...
- 標準ルートは takt: `takt add '#<N>'` で `.takt/workflows/` の専用 workflow（yt-auto-feature / yt-auto-fix / yt-auto-docs / yt-auto-maintenance / yt-auto-audit）を選び、`auto_pr` 有効で `takt run`。使い分けと運用は `docs/takt-operations.md`（既存 `takt:*` ラベルは履歴メタデータのみ — 新規に付与しない）
