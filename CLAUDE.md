...
- worktree で commit / push する場合も、対象 worktree で `nix develop`（または direnv）に入り `lefthook install --force` を通す。診断・再インストール手順は `docs/development.md` の「Git hooks（lefthook）」を参照
