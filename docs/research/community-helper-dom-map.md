...
実画像を添付すると作成状態を変更するため、今回の調査では file の設定自体は実行していない。
ボタン、input、dropzone の DOM path は実測済みだが、native file chooser の起動と
`DataTransfer` + `change` による preview 生成は未検証である。後続の fixture test で event 契約を
固定し、CH-07 実装前に専用テスト画像で添付・preview・削除まで実機確認する。
