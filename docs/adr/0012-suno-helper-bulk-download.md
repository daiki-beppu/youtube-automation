...
- playlist 追加を先に行うことで、DL の成否に関わらず Suno 上のプレイリスト URL を確実に記録できる
- DL がブラウザクラッシュ等で失敗しても、playlist URL さえあれば手動リカバリが可能
...
7. **`suno-playlists.json` 廃止**: 新規 collection では使用しない。既存ファイルは放置し、TS 移植時に完全撤去。参照コード（`read_mapped_slugs` / `write_suno_playlists` / `normalize_suno_title`）は段階的に削除
8. **playlist URL を `workflow-state.json` に統合**: `planning.music.suno_playlist_url` フィールドに記録
...
11. **`GET /collections` レスポンス刷新**:
    ...
    ```json
    {
      "id": "20260611-soulful-grooves",
      "name": "Soulful Grooves",
      "status": "needs_prompts | ready | downloaded",
      "pattern_count": 8,
      "downloaded_count": 8
    }
    ```

    `mapped` / `playlist_name` フィールドは削除。`playlist_name` は拡張側で `${PREFIX} | ${name}` として組み立てる

12. **`POST /collections/<id>/downloaded`（新設、`/suno/download-complete` を置換）**: 冪等な PATCH セマンティクス。playlist 追加完了時と ZIP ダウンロード完了時の 2 段階で POST する。サーバーは来たフィールドで `workflow-state.json` をマージ更新し、`download_path` がある場合だけ ZIP を展開して `02-Individual-music/` へ配置する。
    ...
    `download_path` は Chrome downloads API から得た絶対 ZIP パスで、相対パスや playlist URL なしの指定は 400。`expected_file_count` は Suno が生成した全 clip 数で、サーバーは prompt 数 \* 2 と比較して大きい方を完了判定に使う。ZIP 展開数が期待値未満、または照合できる音声が 0 件なら 500 とし、`workflow-state.json` の `assets.music_downloaded` は更新しない。

13. **`POST /suno/playlists` は legacy 互換として存続**: 新規の playlist URL 記録は `POST /collections/<id>/downloaded` に統合する。既存の `/suno/playlists` route、`write_suno_playlists()`、`--playlist-capture-*` は旧運用・旧テスト互換のため deprecated legacy として残し、下流チャンネルで `suno-playlists.json` 依存が無いことを確認した後に削除する
...
- `extensions/shared/constants.ts` に `DOWNLOADED_ROUTE`, `PHASE.DOWNLOADING`, `DOWNLOAD_FORMAT_DEFAULT` を追加。`SUNO_PLAYLISTS_ROUTE` は deprecated マーク
...
- `src/youtube_automation/scripts/collection_serve.py`: `POST /collections/<id>/downloaded` 追加、`build_collections_index()` を `status` enum 方式に改訂、`read_mapped_slugs` / `write_suno_playlists` を deprecated 化
...
- `suno-playlists.json` 関連コード・ファイルは deprecated legacy とし、下流依存が無いことを確認してから撤去
