...
12. **`POST /collections/<id>/downloaded`（新設、`/suno/download-complete` を置換）**: 冪等な PATCH セマンティクス。playlist 追加完了時に `{ file_count: 0, format, suno_playlist_url }` で 1 回目、DL 完了後に `{ file_count: N, format, suno_playlist_url }` で 2 回目を POST。サーバーは来たフィールドで `workflow-state.json` をマージ更新
    ```json
    {
      "file_count": 8,
      "format": "mp3",
