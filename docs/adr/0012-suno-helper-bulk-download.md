...
5. **インスト vs vocal の分岐**: インストは全曲 `02-Individual-music/` 直接配置、vocal は `.candidates/` に配置して masterup-pairs がキュレーション
...
13. **`POST /suno/playlists` 廃止**: playlist URL の記録は `POST /collections/<id>/downloaded` に統合
...
- TS 移植時に `suno-playlists.json` 関連コード・ファイルを完全撤去
