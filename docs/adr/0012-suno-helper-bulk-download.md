...
7. **`suno-playlists.json` 廃止**: 新規 collection では使用しない。既存ファイルと Python 互換 route（`POST /suno/playlists`、`write_suno_playlists()`、`normalize_suno_title()`）は legacy として残す。残置理由は、下流チャンネル repo の `/wf-batch` 系スキルが `<channel>/config/suno-playlists.json` を入力にしてマスター化〜公開予約を直列実行する旧運用があるため。削除条件と対象は #1301 で追跡する
...
13. **`POST /suno/playlists` は legacy 互換として存続**: 新規の playlist URL 記録は `POST /collections/<id>/downloaded` に統合する。既存の `/suno/playlists` route、`write_suno_playlists()`、`--playlist-capture-*` は、下流チャンネル repo の `/wf-batch` 系スキルが `<channel>/config/suno-playlists.json` を読む旧運用との互換だけに残す。削除条件は #1301（`/wf-batch` が `config/suno-playlists.json` を読まない、旧 operator 手順がない、`--playlist-capture-*` 既存運用がないこと）で追跡する
...
- `src/youtube_automation/scripts/collection_serve.py`: `POST /collections/<id>/downloaded` 追加、`build_collections_index()` を `status` enum 方式に改訂。`POST /suno/playlists` / `write_suno_playlists` は legacy 互換として残す
...
- `suno-playlists.json` 関連コード・ファイルは deprecated legacy とし、下流 `/wf-batch` 依存が無いことを #1301 で確認してから撤去
