...
7. **`suno-playlists.json` 廃止**: 新規 collection では使用しない。playlist URL 記録は `workflow-state.json` に集約し、旧 Python 互換 route（`POST /suno/playlists`、`write_suno_playlists()`、`normalize_suno_title()`）は #1301 で撤去する
...
13. **`POST /suno/playlists` は撤去**: 新規の playlist URL 記録は `POST /collections/<id>/downloaded` に統合する。旧 `/suno/playlists` route、`write_suno_playlists()`、`--playlist-capture-*` は #1301 で削除する
...
- `src/youtube_automation/scripts/collection_serve.py`: `POST /collections/<id>/downloaded` 追加、`build_collections_index()` を `status` enum 方式に改訂。旧 `POST /suno/playlists` / `write_suno_playlists` は #1301 で削除
...
- `suno-playlists.json` 関連コード・ファイルは #1301 で撤去
