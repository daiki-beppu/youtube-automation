...
7. **`suno-playlists.json` 廃止**: 新規 collection では使用しない。旧 Python 互換 route（`POST /suno/playlists`）と関連する playlist URL マッピング関数は #1261 で撤去済み。
...
13. **`POST /suno/playlists` は撤去済み**: 新規の playlist URL 記録は `POST /collections/<id>/downloaded` に統合する。旧 `/suno/playlists` route、`write_suno_playlists()`、`normalize_suno_title()`、`--playlist-capture-prefix` は #1261 で削除済み。
...
- `src/youtube_automation/scripts/collection_serve.py`: `POST /collections/<id>/downloaded` 追加、`build_collections_index()` を `status` enum 方式に改訂。旧 `POST /suno/playlists` / `write_suno_playlists` は #1261 で撤去済み
...
- `suno-playlists.json` 関連コード・ファイルは #1261 で撤去済み
