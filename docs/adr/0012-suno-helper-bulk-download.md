...
7. **`suno-playlists.json` 廃止**: 新規 collection では使用しない。既存ファイルと Python 互換 route（`POST /suno/playlists`、`write_suno_playlists()`、`normalize_suno_title()`）は legacy として残し、下流依存が無いことを確認してから削除
