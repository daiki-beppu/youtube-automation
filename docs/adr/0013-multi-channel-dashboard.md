...
運営者は複数の first-party チャンネルを持ち、各チャンネルの `data/analytics_data_*.json` を個別に確認している。dashboard を開いた時点の最新データを横断表示し、公開予約の在庫をすぐ判断できる必要がある。frontend と別リポジトリの tayk 開発は引き続き分離する。

## Decision

### 責務境界

- Python HTTP server の `yt-dashboard` が channel registry、起動時収集、read model、JSON API、loopback 限定配信を担当する。
...
- 通常起動では registry の全チャンネルを登録順に更新してから配信を開始する。各更新は既存の standard Analytics 収集経路を再利用し、YouTube Data API と YouTube Analytics API を消費する。
...
5. **保存済み snapshot だけを表示（初期決定、#2397 で不採用へ変更）** — 起動は速くクォータを消費しないが、運営判断時の鮮度を保証できない。
...
- 通常起動は登録チャンネル数に比例して API quota と待ち時間を消費し、各チャンネルで read-only OAuth が必要になる。
