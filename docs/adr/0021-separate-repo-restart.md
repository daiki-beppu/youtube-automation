...
本 ADR の「本リポジトリから TS ツールチェーンを削除する」原則は、tayk core と旧 `packages/` に対して引き続き有効とする。一方、収集済み JSON を表示するローカル dashboard は Python 製品の配布 UI であり、次の閉じた境界に限り TypeScript を許可する。

- 許可範囲は `dashboard/` の React + Vite + shadcn/ui source、test、build config と、完成済み asset を収録する `src/youtube_automation/dashboard_dist/`。
- Python が registry/read model/API/loopback HTTP 配信を所有し、frontend は同一 origin の読み取り専用 JSON API だけを利用する。
