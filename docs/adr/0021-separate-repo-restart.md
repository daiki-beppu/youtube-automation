...
2. **Python parity という無限の指摘源** — プロジェクトの用語・設計方針は現行 architecture/ADR 文書へ統合済みだが、Python コードが同居する限り実践は parity 移植に回帰し、レビューは常に旧実装との差分を新たな指摘として生産できた
...
- **物理分離が唯一 parity 回帰を止める**: 紙の上のフレーミング転換では実践が移植に戻ることが実証された。Python コードが見えない別リポでは「旧実装との差分」というレビュー指摘のカテゴリ自体が消滅する
...
- 新リポへ「引き継ぐ決定」を明示する: ADR-0006 (npm 配布) / 0007 (tayk ブランド) / 0009 (JSON-only config) / 0017 (libSQL local store) / 現行 ADR・architecture 文書の MCP tool 2 層・knowledge codec・collection lifecycle 用語系。引き継がない決定: ADR-0002〜0004 (service registry セレモニー) / 0008 (merge 戦略)
...
- 新リポ側の初期 ADR として、薄いアーキテクチャ規約 (1 MCP tool = 実装 1 ファイル + テスト 1 ファイル、registry レス) を起票する。初期内容は本リポの現行 ADR・architecture 文書から MCP tool 2 層 / adapter / knowledge codec / collection lifecycle / データ 4 分類 / read model を移植する

## Related

- ADR-0001 / 0008 / 0015 (superseded・失効) / ADR-0017 (引き継ぎ) / 現行 architecture 文書「データ 4 分類」「read model」「cutover」

## Amendment: dashboard 限定 TypeScript 例外（2026-07-21, #2386）

本 ADR の「本リポジトリから TS ツールチェーンを削除する」原則は、tayk core と旧 `packages/` に対して引き続き有効とする。一方、Python が起動時に収集した JSON を表示するローカル dashboard は Python 製品の配布 UI であり、次の閉じた境界に限り TypeScript を許可する。

- 許可範囲は `dashboard/` の React + Vite + shadcn/ui source、test、build config と、完成済み asset を収録する `src/youtube_automation/dashboard_dist/`。
- Python が registry/起動時収集/read model/API/loopback HTTP 配信を所有し、frontend は同一 origin の読み取り専用 JSON API だけを利用する。
- dashboard は tayk core の実装ではなく、削除済み `packages/`、MCP tool、CLI adapter、旧 TS service を復活させない。
- `extensions/shared-ui` は直接 import せず、dashboard 独立 workspace で Base UI・Tailwind CSS v4・semantic token 契約を適用する。
