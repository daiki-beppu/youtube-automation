...
2. **`bun build` で JS 依存を単一 `dist/cli.js` に bundle**: `workspace:*` 問題は publish 時に消滅し、下流 install は瞬時になる。ただし **`sharp` は native binary のため bundle 不可** — `optionalDependencies` に sharp を配置する (ccusage と違い dependencies 完全空だが optional が 1 件残る)。sharp 欠損時の fallback 方針:
   - サムネイルが YouTube API 上限 (2 MB) 未満なら圧縮不要のため**素通し** — sharp の有無に関わらずアップロードへ進む
   - 圧縮が必要なサイズで sharp が欠損している場合は **fail-loud** (エラー停止 + `bun add sharp` のインストール案内を表示)
   - 「警告して続行」は採らない — upload API の 2 MB 制限エラーまで原因と症状が離れ、デバッグコストが跳ね上がるため

   ffmpeg は従来どおり外部バイナリ前提で変化なし
