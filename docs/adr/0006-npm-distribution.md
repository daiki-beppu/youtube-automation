...
検討時点で npm の `youtube-channels-automation` / `yt-automation` はともに未登録 (404 確認済み)。リポは PUBLIC のため npm 公開に障害はない。

## Decision

**ccusage 型の npm 公開 publish を採用する。**

1. **publish 対象は 1 package**: `youtube-channels-automation` (仮名、ADR 0001 の「同名維持」原則に従う)。`@youtube-automation/core` は bundle に取り込まれるため publish しない。root / cli package の `"private": true` は publish 整備 (#968) で外す
...
3. **bin は単一 `yt`** (ADR 0004 の citty dispatcher)。下流からの canonical 起動文字列は **devDependency install + `bunx yt <cmd>`** に統一する。`.claude/skills/**` の置換 (#965 / #966) はこの文字列を正とする
...
- `.claude/skills/**` の呼び出し置換 (#965 / #966) は canonical 起動文字列 `bunx yt <cmd>` を正として実施する
- package 名は仮置きであり、publish 前 (#968 完了前) であれば改名コストはゼロ。改名する場合は本 ADR を改訂する
