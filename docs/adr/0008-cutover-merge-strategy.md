...
## Amendment: cutover までの同期プロトコル (2026-07-03 追記)

2026-06-17 の merge-base 以降、main の `packages/` に TS fix が直接積まれ（#1330 / #1346 / #1357 等 13 commits）、`feat/ts-rewrite` と main の 2 本で TS 開発ラインが分岐していたことが判明した。一発 merge の前提（branch が TS の唯一の正）を回復するため、以下を決定に追加する:

5. **`feat/ts-rewrite` が TS 実装の唯一の正本**。main 側に入った TS fix は追従 merge で回収する
6. **main → `feat/ts-rewrite` の追従 merge を cutover まで週次で実施**し、一発 merge 時の衝突面積を小さく保つ
7. **main の `packages/` への直コミットは cutover まで禁止**。TS の fix はすべて `feat/ts-rewrite` 宛の PR にする
