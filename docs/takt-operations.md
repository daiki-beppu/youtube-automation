...
- worktree で commit / push する前に、対象 worktree 内で `nix develop`（direnv 利用時は `direnv allow` 後の shell）に入る。shellHook が `lefthook install --force` を実行し、親 checkout と同じ `lefthook.yml` から hook を再生成する。診断は `nix develop --command sh -c 'command -v lefthook && lefthook version'`、再生成は `nix develop --command lefthook install --force`。
