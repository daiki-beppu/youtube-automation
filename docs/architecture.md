...
**ペア**: 同一競合チャンネルに属し、再生数に明確な差がある 2 本の動画。

**伸びた側**: ペアのうち再生数が多い動画。

**伸びなかった側**: ペアのうち再生数が少ない動画。

**ブラインド回答**: 人間が封印分析を見ずに、ペアから観察した事実と解釈を書く回答。

**封印分析**: AI が人間の回答前に生成し、内容の sha256 を訓練記録へ保存して封印したファイル。

**出し切り宣言**: 人間が grilling を終えたと宣言すること。促しへ移る条件となる。

**照合表**: 観点 ID ごとに人間と AI の記述を突き合わせ、`一致`、`人間だけ`、`AI だけ`、`対立`、`事実の相違`の 5 区分で整理した表。

**対立点**: 同じ観点に両者が言及し、再生数差への効きの方向または有無が逆のもの。色名や位置など事実の相違とは区別する。

**試す 1 点**: 訓練セッションの成果として選び、次のサムネイルで変える 1 つの原則または要素。

**訓練記録**: 1 セッションにつき 1 つ作る、人間が直接編集する Markdown。中断・再開を含む進行状態の唯一の正本となる。

**訓練メニュー**: 訓練の種類。初版はサムネイルを扱う `thumbnail` のみ。

**完了セッション**: 訓練記録の `next_try` が非 null となり、「試す 1 点」が決まったセッション。

**未完セッション / 未完了記録**: 訓練記録の `next_try` が null のセッションまたはその記録。同じ状態を指す表記として扱う。

**観点 ID**: 訓練記録、封印分析、照合表を結ぶ `V01`〜`V12` の共通キー。

**母集団**: 自チャンネルの benchmark 競合と、対象となる兄弟チャンネルの benchmark 競合を合わせたペア選定元。first-party チャンネル自身の動画は含めない。

**兄弟チャンネル**: 自チャンネルと benchmark 競合 ID を 1 件以上共有する別の first-party チャンネル。

**走査プール**: 1 競合の走査記録から Shorts とライブを除いた長尺 VOD の集合。

**候補ペア**: 走査プールから選定条件を満たして取り出した、同一競合の長尺 VOD 2 本。伸びた側 / 伸びなかった側の組で承認ゲートに提示する。閾値の正本は skill の `references/pair-selection.md`。

**日差段**: ペアの公開日の差を 5 日、14 日、30 日の順に広げて探索する段階。条件を黙って 30 日より先へ緩めない。

**承認ゲート**: 候補ペアの画像、メタデータ、選定理由を提示し、人間が訓練に使うかを決める 1 回の判断点。

**走査記録**: 競合の直近投稿をフィルタ前に記録した集合。再生数しきい値未満の動画も含む。

**ベンチマーク対象**: 走査記録のうち `min_views` 以上で、競合分析とレポートの対象になる動画。
...
**workspace**: 複数チャンネルを `channels/<slug>/` として同居させる単一リポジトリ。共有物はルートに 1 セット、per-channel 状態は各チャンネル配下に置く。ADR-0029 で deprecated となり、警告リリースで deprecated を告知、削除リリースで経路ごと撤去する（本項も同時に削除）。独立したチャンネルリポジトリへ戻す操作は channel export。
...
**channel export（逆移行）**: workspace の `channels/<slug>/` を独立したチャンネルリポジトリへ copy + 検証で戻す操作。workspace 側は変更せず、git 履歴は引き継がない。ADR-0022 の import と対称。最終段で channel registry の workspace エントリを同位置で戻し先のパスに置換する（一致が無ければ追加、戻し先が既にあれば no-op）。
...
**凍結（workspace slug の）**: export 済みチャンネルの workspace 側 `channels/<slug>/` を書込不可にし、AI 向けの凍結中一覧と launchd 定期収集の停止で二重運用を防ぐ状態。workspace の削除までの暫定。
...
**channel registry**: first-party チャンネルの絶対パス一覧を `~/.config/tayk/channels.json` に JSON 配列で保持するもの。表示名などは各チャンネルの `config/channel/meta.json` から解決する。dashboard の表示対象と fan-out の適格チャンネルの両方を列挙する台帳であり（ADR-0029）、channel export が workspace エントリを同位置で戻し先のパスに置換する（無ければ追加）。
...
**警告リリース**: workspace 経路が動作を維持したまま deprecated を告知するリリース。告知は CHANGELOG の `deprecated` fragment と逆移行ガイドの公開で行い、コード上の警告は出さない。廃止 hook の prune もここに入る。
