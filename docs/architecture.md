...
**封印分析**: 人間の回答前に独立した AI が生成し、内容の sha256 を訓練記録へ保存した比較分析。

**出し切り宣言**: 人間が自発的な観察と grilling を終えたと宣言すること。促しへ移る条件となる。
...
**母集団**: 自チャンネルの benchmark 競合と、対象となる兄弟チャンネルの benchmark 競合を合わせたペア選定元。

**兄弟チャンネル**: 自チャンネルと benchmark 競合 ID を 1 件以上共有する別の first-party チャンネル。

**走査プール**: 1 競合の走査記録から Shorts とライブを除いた長尺 VOD の集合。

**候補ペア**: 走査プール内で日差、公開後日数、再生数比、中央値、最低再生数、既出除外の条件を全て満たすペア。
