...
| 競合の CTR / 視聴維持 / 平均視聴時間は取得不能 | Data API v3 は公開統計（views / likes / comments）のみ | `/channel-research --benchmark`, `/analytics --flop` に明記 |
...
| L2 | CTR | `/thumbnail`（TTP ベース生成）、`/thumbnail --compare`（320px 視認性）、`yt-thumbnail-correlate`（特徴量×CTR 相関）、`/audit --alignment`（サムネ×タイトル×ムード整合）、`/analytics --flop`（CTR 閾値ルーブリック） | Reporting API v1 + サムネ画像特徴量 | ◎ 最厚のレバー |
| L3 | 視聴維持 | retention 収集（`audienceWatchRatio` / `relativeRetentionPerformance`）、`/audit --video`（Gemini によるフック・BGM 展開解析）、`/channel-strategy --scene`（シーン別の最適尺設計） | Analytics API v2 / Vertex AI | ◯ 計測◎、原因照合が手動 |
| L4 | 回遊・セッション | `/publish --playlist`（`yt-playlist-manager`）、`/video --describe`（Complete Collection 導線）、カード指標収集（`cardImpressions/Clicks/ClickRate`）、`/publish --pinned` `/reply` `/publish --community` | Analytics API v2 / Data API v3 | △ 導線は張れるが効果測定が薄い |
...
| L6 | SEO・メタデータ | `/video --describe`（SEO 最適化概要欄）、`/audit --metadata`、`yt-title-duplicate-check`、localizations 同期（`/setup --push`、`yt-shorts-bulk-update-loc`） | ローカル config / Data API v3 | ◯ 生成◎、検索流入との突合なし |
...
横断（レバー非依存）の既存資産: `/analytics --analyze`（全レバーの統合分析・戦略提案）、`/channel-research --voice` + `/channel-strategy --persona`（誰に何を作るかの上流）、`/wf-new`（企画への落とし込み）、`/analytics --flop`（不振動画の切り分け）。
...
- **足りないもの**
  - retention curve × シーン照合の自動化: `audienceWatchRatio`（elapsedVideoTimeRatio 単位）と `/audit --video` のシーンタイムライン・BGM 展開は別々に存在し、「drop 地点 = どのシーン/曲か」の突合が手動 → **新規開発**（G2）
  - 維持率からの逆フィードバック: drop 分析の結果を次コレクションの BGM 構成（`/music --prompt` / `/music --generate` プロンプト）へ反映する定型経路がない → **運用改善**（`/analytics --flop` → `/wf-new` のバトンに含める）
...
│        ↓                                                             │
│  不振動画あり？ ── yes → /analytics --flop  … CTR/Imp/Ret で症状切り分け
│        │                     ↓                                       │
│        │              /thumbnail --compare or /audit --alignment or /audit --video
│        │                     ↓ （原因に応じた個別監査）              │
│        ↓ no                  ↓                                       │
│  /wf-new ←──────────────────┘              … 分析結果を次企画へ反映
│        ↓                                                             │
...
1. **週次ループを止めない**ことが最優先。`/wf-new` は analytics 鮮度切れで停止する設計（`freshness_days` 既定 7 日）なので、収集→分析を週 1 で回すこと自体が制作パイプラインの前提になる
2. CTR 判断は **D+2 ラグ**を踏まえ、公開後 3 日未満の動画では下さない（`/analytics --flop` のルーブリックに従う）
3. ペルソナ系（viewer-voice → channel-strategy --persona → channel-strategy --scene）は硬い依存チェーン。未整備だと `/wf-new` が fallback/minimal mode に劣化するため、新チャンネルでは最初の四半期内に一巡させる
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| G1 | 登録転換分析（動画別転換率 + subscribedStatus） | L5 | 実装済み | 中〜高 | **低** | 完了 | #1813 |
...
- **G1 登録転換分析（#1813 で実装済み）**: `strategic_analytics.py` の video 単位 `subscribersGained` と views から転換率ランキングを算出し、`/analytics --analyze` の「登録を生む動画の型」断面に出力する。`subscribedStatus` dimension の収集 Mixin も追加済み。これにより、転換率と登録済み/未登録視聴者の状態を合わせて確認し、企画選定（`/wf-new`）へ反映できる
- **G2 retention × シーン照合**: `retention_analytics.py` の `audienceWatchRatio`（elapsedVideoTimeRatio）と `data/video_analysis/<slug>/<id>.json` の scene_timeline / bgm_arc を突合し、drop 地点のシーン・曲を特定するレポートを `/analytics --flop` または `/audit --video` に追加。期待効果: 視聴維持の改善が「どの曲・どの展開を変えるか」の具体アクションに変わる
