...
| ID | ギャップ | レバー | 種別 | 効果 | コスト | 優先度 |
| --- | --- | --- | --- | --- | --- | --- |
| G1 | 登録転換分析（動画別転換率 + subscribedStatus） | L5 | 新規開発 | 中〜高 | **低** | ★★★ |
| G2 | retention drop × シーン/BGM 自動照合 | L3 | 新規開発 | 高 | 中 | ★★★ |
| G3 | サムネ A/B（Test & Compare）手動運用の手順化 | L2 | 手動補完 | 高 | **低** | ★★★ |
| G4 | プレイリスト単位 analytics 収集 | L4 | 新規開発 | 中 | 低 | ★★ |
| G5 | 検索流入キーワードレポート | L6 | 新規開発 | 中 | 低〜中 | ★★ |
| G6 | 成長 KPI 定点ビュー（Imp 長期履歴・レバー別週次推移） | L1・横断 | 新規開発 | 中 | 中 | ★★ |
| G7 | TTP トラッキング統合（参照パターン → 企画 → 成果） | L2・横断 | 新規開発 | 高 | 中〜高 | ★★（#1754 で継続検討） |
| G8 | 時間帯・曜日別の公開タイミング分析 | L7 | 新規開発 | 低〜中 | 中 | ★ |
