...
- **パッケージマネージャ**: 両拡張とも `pnpm 9.15.9` 固定（`ni`/`nr` や ambient `pnpm` ではなく、再現可能な検証では `npx -y pnpm@9.15.9` を使う）。各 `package.json::packageManager`、pnpm 9 形式の lockfile、`pnpm.onlyBuiltDependencies` の esbuild approval、CI の pnpm 9 を揃えるための契約である。`.npmrc::package-manager-strict=false` のため別版は自動拒否されない。両拡張共通の install / build / zip コマンドと lockfile 無差分確認は `extensions/README.md::pnpm バージョン契約` を正とする
