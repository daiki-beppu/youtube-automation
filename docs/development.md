...
- **pre-push**:
  ...
  - 広すぎる型注釈ゲート。`origin/main` からの新規追加行だけを対象に、ディレクトリを問わず全 `*.py` / `*.ts` / `*.tsx` の Python の typing module 経由の Any 型（`typing.Any` の修飾形、`from typing import Any` の直接 import 経由で使う裸の `Any` の両方。複数行の括弧 import や `as` alias も Python `ast` で解決するため検出できる）、または TypeScript の any 型注釈（`: any` 直書きに加え、`Array<any>` / `Record<string, any>` のようなジェネリック引数・union / intersection・tuple 要素などの型位置の `any` も検出。型を導入する記号が直前にない英語コメントや文字列リテラル中の "any" は誤検知しない）を検出したら push を止める。既存行は対象外。ロジック本体は `.lefthook/pre-push/any-usage-gate.sh`。Python 側の import 解決には `python3` が必要で、見つからない場合は警告を出して直接 import 検出のみ省略する
