...
| `.claude/skills/setup/references/claude-md-template.md` | upstream リポジトリ名の説明 |
...
- **Terraform drift job**（`.github/workflows/terraform-drift.yml`）: 毎日 / dispatch / gcp 定義の main push で WIF SA が読み取り専用 plan を実行し、drift / apply 待ち / job 失敗を Discord に通知する。required check にはしない。解消は `infra/terraform/gcp/README.md` に従いローカル ADC で apply する。
- **Terraform 静的ゲート**（`.github/workflows/terraform-static.yml`）: `infra/terraform/**` または workflow 自身を変更した PR と main push で、bootstrap / gcp / r2 / streaming の `fmt -check -recursive` / `init -backend=false -input=false` / `validate` と r2 の mock `terraform test` を認証不要で実行する。path filter があるため required check にはせず、既存の `lint` / `test` を維持する。
