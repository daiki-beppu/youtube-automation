...
|---|---|---|---|
| `/collection-ideate` | Analytics × ベンチマーク × ペルソナを統合してペルソナ別 3 企画候補を生成 | コレクションディレクトリは作らない | 提案表示のみ（選択は `/wf-new` 側で） |
| `/wf-new` | 企画選択 → `yt-init-collection` でディレクトリ + `workflow-state.json` 作成 → サムネ・音楽素材生成 | 楽曲の最終マスタリングや動画化はしない | (1) 企画選択 (2) サムネ承認 の 2 箇所 |
...
**Q. ベンチマークが古いと `/collection-ideate` に怒られる**
A. `/benchmark` を先に実行する（`/collection-ideate` 内部で stale 検出時に自動で呼ぶ）。`/analytics-analyze` の stale は AI 推論コスト発生のため自動呼び出しはせず、ユーザーへの案内のみ。
