...
以下のコマンドはすべて v5.5.0 のリリース時点で entry point として登録済みです（uv run yt-config-migrate / uv run yt-channel-status / uv run yt-skills など）。command not found / No module named 相当が出た場合は「ガイドが古い」「コマンドが存在しない」と判断せず、env 側の問題として以下の順で切り分けてください（詳細は Q4 参照）:
...
Q4. uv run yt-config-migrate verify / yt-channel-status / yt-skills 等で command not found / No module named ...
