...
| `wfNextChannelOverride` | `approval_gates.{audio,upload}` | 廃止 | 設定 loader が `ConfigError` を返す。 |
...
正規キーは `true = 承認省略` に統一する。旧 `approval_gates` はロード時に拒否されるため、
`skip_audio_approval` / `skip_upload_approval` へ移行する。
