...
```json
{
  ...
  "$defs": {
    ...
    "approvalGate": {
      ...
      "properties": {
        "skip": {
          "type": "boolean",
          "default": true,
          "description": "Whether the interpreter skips approval immediately before this step."
        },
        "enabled": {
          "type": "boolean",
          "default": false,
          "description": "Deprecated inverse alias. Resolve as skip = not enabled."
        },
        "configPath": {
          "type": "string",
          "minLength": 1,
          "description": "Optional config/channel/workflow.json path that supplies skip."
        }
      }
    },
    "idempotency": {
    ...
    "wfNextChannelOverride": {
      ...
      "additionalProperties": false,
      "properties": {
        "skip_audio_approval": { "type": "boolean", "default": true },
        "skip_upload_approval": { "type": "boolean", "default": true },
        "approval_gates": {
          "type": "object",
          "additionalProperties": false,
          "properties": {
    ...
    "genericChainOverride": {
      ...
      "properties": {
        "skip_approvals": {
          ...
          "description": "Per-step approvalGate.skip overrides for this chain."
        },
        "approval_gates": {
          "type": "object",
          "propertyNames": {
          ...
          "additionalProperties": { "type": "boolean" },
          "description": "Deprecated inverse aliases for per-step approvalGate.skip overrides."
        }
      }
    },
    "channelWorkflowFile": {
...
| `step` | `approvalGate.skip` | Yes (canonical) | `true` なら step 直前の承認を省略。bundled manifest は必ずこの正規 field を明示する。 |
| `step` | `approvalGate.enabled` | Legacy only | 逆向きの後方互換 alias。`skip = not enabled` として解決する。`skip` との同時指定は schema 違反。 |
| `step` | `approvalGate.configPath` | No | `skip` のチャンネル上書き元となる `workflow.json` 内のパス。 |
...
| `channelWorkflowFile` | `workflow.<chain_id>`（`wf_next` 以外） | No | `chainId` と同名の generic chain override。chain 固有の専用 schema がある場合を除き、承認設定だけを許可する。 |
| `genericChainOverride` | `skip_approvals.<step_id>` | No | 対応する同梱 step の `approvalGate.skip` を上書きする正規 boolean。`step_id` は英小文字で始まる kebab-case。 |
| `genericChainOverride` | `approval_gates.<step_id>` | Legacy only | 逆向きの後方互換 alias。同一 step の `skip_approvals` との同時指定は semantic validation で拒否する。 |
| `wfNextChannelOverride` | `skip_audio_approval` | No | 音源確定 step の承認省略。未指定時は `true`。 |
| `wfNextChannelOverride` | `skip_upload_approval` | No | upload step の承認省略。未指定時は `true`。 |
| `wfNextChannelOverride` | `approval_gates.{audio,upload}` | Legacy only | 逆向きの後方互換 alias。対応する `skip_*_approval` との同時指定は拒否する。 |
...
1. 同梱 `bundledManifest` を schema で構造検証し、`steps[*].id` の重複を semantic
   ...
   idempotency script を得る。bundled manifest は正規 `approvalGate.skip` を必ず
   明示する。旧 manifest の `enabled` だけがある場合は `skip = not enabled`
   として解決する。`skip` と `enabled` の両方がある場合は schema 違反で拒否する。
...
3. それ以外の `chainId` は、`workflow.<chain_id>` を
   ...
   `skip_approvals.<step_id>` は、同梱 manifest 内で同じ `id` を持つ step の
   `approvalGate.skip` だけを上書きする。旧 `approval_gates.<step_id>` は
   逆向き alias として反転解決し、同一 step の新旧同時指定は semantic validation で拒否する。
4. override に存在しない field は同梱 manifest の明示値を使う。各 step の
   `approvalGate.skip` は同梱 manifest で必ず明示し、承認なしの既定値には
   `true` を記載する。

したがって下流側に `workflow.json`、`workflow.<chain_id>`、または
`skip_approvals` が存在しなくても、承認なしの既定動作（全自動）が維持される。
...
| `workflow.wf_next.approval_gates.{audio,upload}` | 旧 alias を反転して対応する `approvalGate.skip` へ解決 | 上記 2 step。同一 gate の新旧同時指定は拒否。 | `false` (= 承認なし) |
...
正規キーは `true = 承認省略` に統一する。旧 `approval_gates` は逆向き alias として
ロードし、未指定時の承認なしと既存設定の挙動を維持する。
  ...
  "steps": [
    {
      ...
      "approvalGate": {
        "skip": true,
        "configPath": "workflow.analytics.skip_approvals.collect"
      },
      "idempotency": {
    ...
    },
    {
      ...
      "approvalGate": {
        "skip": true,
        "configPath": "workflow.analytics.skip_approvals.report"
      },
      "idempotency": {
        "script": "references/analytics-chain-state.py"
      }
...
analytics は外部反映をしないため、三 step とも `approvalGate.skip: true` を初期値に
...
`true` のまま、`analyze` だけが `false` になる。`workflow.analytics` が存在しない
ときは、三 step とも同梱 default の `true` を使う。
