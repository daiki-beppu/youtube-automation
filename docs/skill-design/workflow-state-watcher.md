...
- `config/channel/workflow.json::workflow.wf_next.skip_audio_approval` と `.skip_upload_approval` は
...
「`skip_audio_approval` と `skip_upload_approval` が true なら全自動」とは、既知の承認
ゲートを挟まないという意味である。対象 collection を一意に指定でき、playlist 等の外部反映設定が準備済みで、候補
...
- [`src/youtube_automation/configuration/workflow.py`](../../src/youtube_automation/configuration/workflow.py)
- [`src/youtube_automation/configuration/loader.py`](../../src/youtube_automation/configuration/loader.py)
