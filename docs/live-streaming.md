...
配信中のライブチャットへ自動返信したい場合は、`/reply` skill の live mode を組み合わせられます。ライブ配信本体の構築・復旧は `/streaming`、チャット返信の設定と運用は `/reply --live` が担当します。

```text
/reply --live ライブチャットの自動返信を始めたい
```

返信機能は opt-in です。利用するチャンネルのコメント設定と認証情報を準備したうえで有効にしてください。返信 daemon は配信サービスから独立しているため、返信側の停止や再起動で映像配信が止まることはありません。
