...
(`scripts/thumbnail_text.py` が CLI 境界で 1 回だけ解決する)。
"""
...
_FONT_FALLBACK_GUIDANCE = (
    "対処:\n"
    "  1. config/skills/thumbnail.yaml の image_generation.gemini.thumbnail_text.overlay.font.title に\n"
...
def _parse_color(raw: Any, *, default: str, key: str) -> str:
...
def _parse_int(raw: Any, *, default: int, key: str, minimum: int = 0) -> int:
    if raw in (None, ""):
        return default
    try:
        value = int(raw)
...
def overlay_spec_from_skill_config(
    skill_config: dict[str, Any],
    *,
    channel_root: Path,
    with_channel_name: bool,
) -> OverlaySpec:
    """skill-config (deep-merge 済み dict) から OverlaySpec を組み立てる。

    `image_generation.gemini.thumbnail_text.overlay` セクションを読む。
    フォント未設定などの不備は ConfigError (理由 + 代替手順つき) にする。
    """
    gemini = skill_config.get("image_generation", {}).get("gemini", {})
    thumbnail_text = gemini.get("thumbnail_text", {}) if isinstance(gemini, dict) else {}
    overlay = thumbnail_text.get("overlay", {}) if isinstance(thumbnail_text, dict) else {}
    if not isinstance(overlay, dict):
        raise ConfigError("thumbnail_text.overlay はマッピングで指定してください (config/skills/thumbnail.yaml)")

    font_cfg = overlay.get("font", {}) if isinstance(overlay.get("font"), dict) else {}
    title_cfg = overlay.get("title", {}) if isinstance(overlay.get("title"), dict) else {}
    prefix = "thumbnail_text.overlay"

    title_style = _build_text_style(
        title_cfg,
        font_raw=str(font_cfg.get("title") or ""),
    ...
    if with_channel_name:
        channel_cfg = overlay.get("channel_name", {}) if isinstance(overlay.get("channel_name"), dict) else {}
        # channel_name 用フォント未設定時はタイトルフォントを継承する
        channel_font_raw = str(font_cfg.get("channel_name") or font_cfg.get("title") or "")
    ...
    line_spacing_raw = layout.get("line_spacing")
    try:
        line_spacing = 1.15 if line_spacing_raw in (None, "") else float(line_spacing_raw)
    except (TypeError, ValueError) as exc:
        raise ConfigError(f"数値を指定してください: {prefix}.layout.line_spacing = {line_spacing_raw!r}") from exc
    if line_spacing <= 0:
        raise ConfigError(f"{prefix}.layout.line_spacing は正の数を指定してください: {line_spacing}")
