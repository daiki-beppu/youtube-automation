...
_FONT_FALLBACK_GUIDANCE = (
    "対処:\n"
    "  1. config/skills/thumbnail.yaml の thumbnail_text.overlay.font.title に対応する\n"
...
def overlay_spec_from_overlay_config(
    ...
    *,
    ...
    if not isinstance(overlay_config, dict):
        raise ConfigError(f"thumbnail_text.overlay はマッピングで指定してください ({_SKILL_CONFIG_PATH_HINT})")

    font_cfg = _mapping_at(overlay_config, "font", key="thumbnail_text.overlay.font")
    title_cfg = _mapping_at(overlay_config, "title", key="thumbnail_text.overlay.title")
    channel_cfg = _mapping_at(overlay_config, "channel_name", key="thumbnail_text.overlay.channel_name")
