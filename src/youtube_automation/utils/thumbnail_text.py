...
def _mapping_at(parent: dict[str, Any], name: str, *, key: str) -> dict[str, Any]:
...
def _parse_positive_float(raw: Any, *, default: float, key: str) -> float:
    if raw in (None, ""):
        return default
    if isinstance(raw, bool):
...
def overlay_spec_from_skill_config(
    ...
    *,
    ...
    font_cfg = _mapping_at(overlay, "font", key="thumbnail_text.overlay.font")
    title_cfg = _mapping_at(overlay, "title", key="thumbnail_text.overlay.title")
    ...
    if with_channel_name:
        channel_cfg = _mapping_at(overlay, "channel_name", key=f"{prefix}.channel_name")
    ...
    layout = _mapping_at(overlay, "layout", key=f"{prefix}.layout")
