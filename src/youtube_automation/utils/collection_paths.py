...
# YouTube / DistroKid などのアップロード用サムネイル候補。
...
class CollectionPaths:
    ...
    def find_thumbnail(self) -> Path | None:
        """10-assets/ からアップロード用サムネイル画像を探す。

        候補順は ``thumbnail.jpg > thumbnail.png``（``_THUMBNAIL_CANDIDATES``）。
        ``main.png`` / ``main.jpg`` は textless 動画背景なので、サムネイル欠落時に
        fallback 選択しない。動画背景が必要な経路は ``find_main_image()`` を使う。
        """
