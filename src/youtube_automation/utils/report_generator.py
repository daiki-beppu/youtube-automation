...
class ReportGenerator:
    ...
    def __init__(self, output_dir=None):
        ...
        if output_dir is None:
            output_dir = channel_dir() / "reports"
        else:
    ...
    def _generate_strategic_insights(self, analytics_data: Dict, ctr_strategy: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            "competitive_positioning": {
                "unique_strengths": [
                    f"{config.content.genre.context}特化{config.content.genre.primary}",
                    "コレクション完成度",
                    "楽曲資産",
                ],
                "market_opportunity": "CTR改善によるリーチ拡大 → 登録者数成長可能性",
                "differentiation_strategy": (
                    f"{config.meta.channel_short}独自の世界観 + 高品質{config.content.genre.primary}"
                ),
            },
        }
    ...
    def save_report_as_json(self, report_data: Dict, filename: str = None) -> str:
        ...
        if filename is None:
            ...
            filename = f"{config.meta.channel_short.lower()}_report_{report_data['report_type']}_{timestamp}.json"
    ...
    def save_report_as_html(self, report_data: Dict, filename: str = None) -> str:
        ...
        if filename is None:
            config = load_config()
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"{config.meta.channel_short.lower()}_report_{report_data['report_type']}_{timestamp}.html"
    ...
    def run_automated_report_generation(self) -> Dict[str, str]:
        """自動レポート生成実行"""
        config = load_config()
        logger.info(f"{config.meta.channel_short}自動レポート生成システム開始...")
        ...
        try:
            ...
            results["weekly"] = {"json": weekly_json, "html": weekly_html}
            ...
            results["monthly"] = {"json": monthly_json, "html": monthly_html}
...
def main():
    """メイン関数 - スタンドアロン実行用"""
    config = load_config()
    print(f"🎵 {config.meta.channel_name} - 自動レポート生成システム")
    print("=" * 60)
