...
class ChannelConfig:
    ...
    def _resolve_channel_dir(cls) -> Path:
        ...
        for parent in [Path.cwd()] + list(Path.cwd().parents):
            if (parent / "config" / "channel_config.json").exists():
                return parent
        raise ConfigError("CHANNEL_DIR 環境変数を設定するか、チャンネルディレクトリ配下で実行してください")
    ...
    def _validate(cls):
        ...
        if missing:
            raise ConfigError(f"channel_config.json に必須キーがありません: {', '.join(missing)}")
    ...
    def music_engine(self) -> str:
        ...
        if engine not in ("suno", "lyria"):
            logger.warning(
                "channel_config.json の music_engine='%s' は未知の値です。既知の値は 'suno' / 'lyria'",
                engine,
            )
        return engine
    ...
    @property
    def youtube_channel_settings(self) -> dict:
        """チャンネルのブランディング設定（任意セクション）。

        yt-channel-settings コマンドが push/pull する対象。未定義時は空辞書。
        含みうるキー: description, keywords, country, default_language,
        unsubscribed_trailer, made_for_kids
        """
        return dict(self._data.get("youtube_channel", {}))
