...
from youtube_automation.infrastructure.runtime import progress as progress_fmt
...
# 新しい動画タイプは専用 generator を実装し、この registry に登録する。
# エンジン選択（Veo / 将来の別エンジン）は各 generator の内部責務であり、
# video_type の軸とは分離する。
VideoGenerator = Callable[..., bool]
_VIDEO_TYPE_GENERATORS: dict[VideoType, VideoGenerator] = {}


def register_video_generator(video_type: VideoType, generator: VideoGenerator) -> None:
    """動画タイプに generator hook を登録する。"""
    _VIDEO_TYPE_GENERATORS[video_type] = generator


def generate_video(video_type: VideoType | str, /, *args: object, **kwargs: object) -> bool:
    """登録済み hook へ明示的な動画タイプで dispatch する。"""
    resolved = VideoType.parse(video_type, config_path="video_type")
    generator = _VIDEO_TYPE_GENERATORS.get(resolved)
    if generator is None:
        raise ValueError(f"video_type={resolved.value!r} has no registered generator")
...
register_video_generator(VideoType.LOOP, generate_loop_video)
