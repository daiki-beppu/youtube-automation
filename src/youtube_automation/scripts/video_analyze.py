...
def _resolve_benchmark_targets(*, data_dir: Path, channel_slug: str, top: int) -> list[VideoTarget]:
    ...
    vod_videos = [v for v in matched if not is_live_benchmark_video(v)]
    excluded_live = len(matched) - len(vod_videos)
    if excluded_live:
        logger.info(
            "live 配信 %d 本を解析対象から除外し次点 VOD を繰り上げます"
            "（Gemini はライブ配信 URL を取り込めないため）: %s",
            excluded_live,
            ", ".join(str(v.get("video_id")) for v in matched if is_live_benchmark_video(v)),
        )
    if not vod_videos:
        raise ValidationError(f"slug='{channel_slug}' の benchmark 動画は live 配信のみで、解析可能な VOD がありません")

    selected = vod_videos[:top]
    return [
        VideoTarget(
            video_id=v["video_id"],
            slug=channel_slug,
