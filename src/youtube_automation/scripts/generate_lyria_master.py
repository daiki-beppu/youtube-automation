...
5. 全セグメント揃ったら `tayk generate-master` を呼び、
   `01-master/master.wav` を出力
...
- マスタリング実体は TS core service (`master.generate`) に集約し、本 CLI からは
  `tayk generate-master` を呼ぶ。
...
def main() -> int:
    ...
    try:
        ...
        master_path = _run_generate_master(collection_dir)
        print()
