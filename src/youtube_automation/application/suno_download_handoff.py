...
def _download_sources(music_dir: Path) -> tuple[HandoffSource, ...]:
...
class SunoDownloadHandoff:
    ...
    def complete(self, collection_dir: Path) -> SunoDownloadHandoffResult:
        ...
        manifest_key = handoff_media_key(identity, MANIFEST_NAME).as_posix()
        state = read_workflow_state(paths.workflow_state_path)
        completed = _completed_manifest(self._store, state, identity, manifest_key)
        ...
        sources = _download_sources(paths.music_dir)
