...
from youtube_automation.core.time_utils import format_duration_display, format_duration_short, format_timestamp
...
logger = logging.getLogger("youtube_automation.domains.metadata.service")

_WorkflowValue = TypeVar("_WorkflowValue")


def _read_workflow_value(
    collection_path: Path, read_value: Callable[[WorkflowState], _WorkflowValue]
) -> _WorkflowValue | None:
    """Read one optional value, treating unreadable or malformed state as no override."""
    try:
        state = read_workflow_state_or_none(CollectionPaths(collection_path).workflow_state_path)
        return read_value(state) if state is not None else None
    except WorkflowStateError:
        return None


def _load_pattern_names(patterns_path: Path) -> dict[str, str]:
    """Read optional pattern labels and their scene variations from the YAML document."""
...
def _load_metadata_workflow_state(ws_path: Path) -> WorkflowState:
    """Read metadata workflow state with its strict section diagnostics."""
    try:
...
def _metadata_scene_emoji(state: WorkflowState) -> str:
    """Read an optional scene emoji while retaining type-error translation."""
...
def _persist_track_display_names(ws_path: Path, filename_map: dict[str, str]) -> None:
    """Merge display names into authoritative workflow state with metadata diagnostics."""
...
def _collection_display_name(collection_path: Path) -> str:
    """Resolve the display name shared by full and Shorts metadata generation."""
    dir_name = collection_path.name
...
def _build_shorts_metadata(config: ChannelConfig, collection_name: str, theme: str, cc_video_url: str) -> Dict:
    """Build the Shorts fields from channel and collection context only."""
    channel_name = config.meta.channel_name

    # タイトル: 旧版踏襲 "{collection_name} ✦ {channel_name} #Shorts"
    title = build_short_title(collection_name, channel_name)
    ...
    tag_list = build_short_tags(config.content.tags.base, config.content.tags.themes.get(theme, []))
    ...
    return {
        ...
        "tags": tag_list,
        "category_id": config.youtube.api.category_id,
        "privacy_status": "public",
        "language": config.youtube.api.language,
        "localizations": localizations,
    }


def generate_shorts_metadata(collection_path: str, cc_video_url: str) -> Dict:
    """Generate Shorts metadata without loading audio mastering or long-form description settings."""
    config = load_config()
    path = Path(collection_path)
    collection_name = _collection_display_name(path)
    theme = _read_workflow_value(path, lambda state: state.theme) or ""
    return _build_shorts_metadata(config, collection_name, theme, cc_video_url)
...
class BAHMetadataGenerator:
    ...
    def _extract_collection_name(self) -> str:
        """ディレクトリ名からコレクション名を抽出"""
        return _collection_display_name(self.collection_path)
    ...
    def _load_theme_display_names(self) -> Dict[str, str]:
        ...
        def read_patterns(state: WorkflowState) -> dict | None:
            planning = state.planning
            music = planning.music if planning is not None else None
            return music.patterns if music is not None else None

        patterns = _read_workflow_value(self.collection_path, read_patterns) or {}
        result: Dict[str, str] = {}
    ...
    def _load_suno_pattern_name_en(self) -> Dict[str, str]:
        ...
        return _load_pattern_names(patterns_path)
    ...
    def _apply_persisted_display_names(self) -> None:
        ...
        """
        name_map = _read_workflow_value(self.collection_path, lambda state: state.track_display_names)
        if not name_map:
            return
        for track in self.tracks:
    ...
    def apply_track_display_names(self, name_map: Dict[int, str]) -> None:
        ...
        _persist_track_display_names(ws_path, filename_map)
    ...
    def _extract_theme_name(self) -> str:
        ...
        """
        configured_name = _read_workflow_value(self.collection_path, lambda state: state.collection_name)
        return re.sub(r"\s+Collection$", "", configured_name or self.collection_name, flags=re.IGNORECASE)
    ...
    def _get_activity(self) -> str:
        ...
        """
        title_activity = _read_workflow_value(self.collection_path, lambda state: state.title_activity)
        if title_activity:
    ...
    def _load_theme(self) -> str:
        ...
        確定している前提）。
        """
        return _read_workflow_value(self.collection_path, lambda state: state.theme) or ""
    ...
    def generate_shorts_metadata(self, cc_video_url: str) -> Dict:
        ...
        Raises:
            ValueError: 生成タイトルが 100 codepoint を超過したとき（silent slice 禁止、
                plan 補足設計判断 §153）。
        """
        return _build_shorts_metadata(self.config, self.collection_name, self._load_theme(), cc_video_url)
    ...
    def generate_metadata_report(self) -> str:
        ...
        return _render_metadata_report(
            self.config.meta.channel_name,
...
def _render_metadata_report(
    ...
    tracks: list[dict],
    crossfade: float,
    format_duration: Callable[[int | float], str],
) -> str:
    """Render collected track metadata without performing collection or configuration reads."""
    total_duration = sum(track["duration"] for track in tracks) - max(0, len(tracks) - 1) * crossfade

    report_parts = [
        f"📊 {channel_name} メタデータ生成レポート",
        "=" * 60,
        f"🎵 コレクション: {collection_name}",
        f"🎼 ビット深度: {bit_depth}",
        f"📁 パス: {collection_path}",
        f"🎶 楽曲数: {len(tracks)}曲",
        f"⏱️  総再生時間: {format_duration(total_duration)}",
...
def main():
    ...
    if len(sys.argv) != 2:
        print("使用法: python -m youtube_automation.application.metadata.service <collection_directory>")
        sys.exit(1)
