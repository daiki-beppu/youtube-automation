...
logger = logging.getLogger("youtube_automation.domains.uploads._preflight")
...
class PreflightChecker:
    ...
    def check(self, collection_dir: Path) -> None:
        ...
        state = _read_preflight_workflow_state(ws_path)
        ...
        ts_lines = _validated_chapters(description, config.audio.chapter_max)
        ...
        _require_scene_phrase_translations(scene_phrases, config.localizations.supported_languages)
        ...
        _validate_localized_titles(prebuilt["localizations"])

        # タグ件数 / quotation 文字数チェック
        # 本番と同じ validated JSON の tags を検証する。
        _validate_tags_and_duration(
            prebuilt["tags"],
            duration_sec,
            min_tags=config.content.tags.min_count,
            target_min=getattr(config.audio, "target_duration_min", None),
            target_max=getattr(config.audio, "target_duration_max", None),
            allow_duration_outside_target=self.allow_duration_outside_target,
        )

        logger.info(f"✅ preflight OK — title={len(title)}c, chapters={len(ts_lines)}, langs={len(scene_phrases)}")


def _require_scene_phrase_translations(scene_phrases: Mapping[str, object], supported_languages: list[str]) -> None:
    """Require each distinct language only for multilingual publication."""
    missing = missing_scene_phrase_languages(scene_phrases, supported_languages)
    if missing:
        raise ValidationError(
...
def _read_preflight_workflow_state(ws_path: Path) -> WorkflowState:
    """Require persisted workflow state and retain preflight's JSON error contract."""
    if not path_exists(ws_path):
        raise ValidationError(
            f"❌ {ws_path} が存在しません。/wf-new または /video --describe の前提を確認してください。"
        )
    try:
        return read_workflow_state(ws_path)
...
def _validated_chapters(description: str, chapter_max: int) -> list[str]:
    ts_lines = [line for line in description.split("\n") if re.match(r"^\d{1,2}:\d{2}", line.strip())]
    msg = check_chapter_count(len(ts_lines), chapter_max)
    if msg:
...
def _validate_localized_titles(localizations: object) -> None:
...
def _validate_tags_and_duration(
    tags_value: object,
    duration_sec: float,
    *,
    min_tags: int | None,
    ...
    issues: list[str] = []
    for msg in (
        check_tags_count(tags, min_tags),
    ...
    if target_min is not None or target_max is not None:
        ...
        if duration_issue and not allow_duration_outside_target:
            issues.append(
