"""Compose settings, scheduling, tracking, and transport for Shorts uploads."""
...
from youtube_automation.application.metadata.service import generate_shorts_metadata
from youtube_automation.application.uploads.youtube import YouTubeAutoUploader
from youtube_automation.configuration import load_config, load_schedule_config
...
from youtube_automation.domains.uploads.scheduling import get_schedule_timezone
from youtube_automation.domains.uploads.upload_journal import UploadAttempt, UploadJournal
from youtube_automation.infrastructure.filesystem import list_directory, path_exists
from youtube_automation.infrastructure.google.youtube import YouTubeClients

logger = logging.getLogger("youtube_automation.domains.uploads.shorts")
...
def _short_upload_failure(error: str) -> dict:
    """Use the same failure response shape for every non-retryable short upload error."""
    return {"action": ACTION_FAILED, "details": {"error": error}}
...
def _begin_short_upload(
    journal: UploadJournal, ws_path: Path, short_num: int | None
) -> tuple[UploadAttempt, str | None]:
    """Begin an attempt and import a legacy resume URI only when the journal lacks one."""
    ...
    return attempt, resume_session_uri


def _create_short_uploader(collections_root: str, youtube_clients: YouTubeClients | None) -> YouTubeAutoUploader:
    """Compose the default upload transport for short uploads."""
    return YouTubeAutoUploader(collections_root, youtube_clients)


def _persisted_short_upload_dates(
    live_dir: Path, tracking_store: TrackingStore, published_dates: PublishedDatesScheduler
) -> Iterator[datetime]:
    """Read the persisted short-upload timeline with the canonical date parser."""
    for col_dir in list_directory(live_dir):
        ws_path = CollectionPaths(col_dir).workflow_state_path
        if not path_exists(ws_path):
            continue
        state = tracking_store.load_workflow_state(ws_path)
        if state is None:
            continue
        ...
        for entry in shorts:
            ...
            if not uploaded_at:
                continue
            dt = published_dates.parse_persisted_datetime(
                uploaded_at,
                source=ws_path,
...
def _record_completed_short(attempt: UploadAttempt, ws_path: Path, entry: dict) -> bool:
    """Commit the upload journal before projecting the result into workflow state."""

    def project(state: WorkflowState) -> None:
        state.record_short_upload(entry)

    try:
        attempt.complete({"video_id": entry["video_id"]})
...
class ShortUploader:
    ...
    def __init__(
        ...
        *,
        uploader_factory: Callable[[str, YouTubeClients | None], YouTubeAutoUploader] = _create_short_uploader,
    ):
        ...
        self.uploader = uploader_factory(collections_root, youtube_clients)
    ...
    def _short_upload_dates(self, live_dir: Path) -> Iterator[datetime]:
        """Yield parseable persisted upload dates without changing their traversal order."""
        yield from _persisted_short_upload_dates(live_dir, self.tracking_store, self.published_dates)
    ...
    def _check_upload_interval(self) -> tuple[bool, str]:
        ...
        latest_dt = max(self._short_upload_dates(live_dir), default=None)
    ...
    def upload_short(self, collection_path: Path, short_num: Optional[int] = None) -> dict:
        ...
        if not path_exists(tracking_path):
            ...
            return _short_upload_failure(f"tracking missing: {tracking_path}")
        try:
            tracking = self.tracking_store.read_for_upload(collection_path)
        except UploadError:
            logger.error("❌ upload_tracking.json 読み込み失敗")
            return _short_upload_failure(_TRACKING_READ_ERROR)
        ...
        try:
            ...
            return _short_upload_failure("short video not found")

        # 4. メタデータ生成
        try:
            metadata = generate_shorts_metadata(str(collection_path), cc_video_url)
        except AutomationError:
            logger.error("❌ Shorts メタデータ生成失敗")
            return _short_upload_failure("short metadata generation failed")
        ...
        try:
            attempt, resume_session_uri = _begin_short_upload(journal, ws_path, short_num)
        ...
        try:
            video_id = self.uploader.upload_video(
                ...
                on_session_uri_changed=attempt.record_session,
            ...
            return _short_upload_failure(_SHORT_UPLOAD_ERROR)
        if not video_id:
            return _short_upload_failure("upload_video returned None")
        ...
        if not _record_completed_short(attempt, ws_path, entry):
            return _short_upload_failure(_SHORT_UPLOAD_ERROR)
