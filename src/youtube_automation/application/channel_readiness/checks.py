"""Channel readiness checks and their injectable external probes."""
...
def bundled_skill_names() -> list[str]:
    """Return bundled skills; the command boundary injects the package probe."""
...
RunProbe = Callable[[list[str], int], tuple[int, str, str]]


@dataclass(frozen=True)
class ReadinessProbes:
    """External effects used by readiness checks."""

    run: Callable[..., tuple[int, str, str]]


_PROBES: ContextVar[ReadinessProbes | None] = ContextVar("channel_readiness_probes", default=None)


@contextmanager
def use_probes(probes: ReadinessProbes):
    """Inject external probes for one readiness evaluation."""

    token = _PROBES.set(probes)
    try:
        yield
    finally:
        _PROBES.reset(token)
...
def _subprocess_run(cmd: list[str], timeout: int = 30, *, cwd: Path | None = None) -> tuple[int, str, str]:
    try:
...
def _run(cmd: list[str], timeout: int = 30, *, cwd: Path | None = None) -> tuple[int, str, str]:
    probes = _PROBES.get()
    if probes is None:
        return _subprocess_run(cmd, timeout=timeout, cwd=cwd)
    if cwd is None:
        return probes.run(cmd, timeout=timeout)
    return probes.run(cmd, timeout=timeout, cwd=cwd)


def _active_run_probe() -> Callable[..., tuple[int, str, str]]:
    probes = _PROBES.get()
    return probes.run if probes is not None else _subprocess_run
...
def _read_upload_local_state(channel_dir: Path) -> CheckResult | tuple[Path, str]:
    ...
    return token_path, channel_id


def check_upload_ready(channel_dir: Path) -> CheckResult:
    local_state = _read_upload_local_state(channel_dir)
    if isinstance(local_state, CheckResult):
        return local_state
    token_path, channel_id = local_state
