"""Collect channel configuration and artifacts to evaluate setup readiness."""
...
from youtube_automation.domains.channel_readiness.readiness import (
    ReadinessResult,
    _approved_ttp_exception_blocks,
    _line_mentions_approved_exception,
    _line_mentions_ttp_skip,
    _validate_approved_ttp_exception_blocks,
...
def _missing_legacy_persona_readiness_items(path: Path) -> list[str]:
    ...
    missing.extend(_missing_persona_source_annotations(sections, relative))
    return missing


def _missing_persona_source_annotations(sections: dict[str, list[str]], relative: str) -> list[str]:
    """Check source annotations on structured persona list entries."""
    missing: list[str] = []
    for name in _PERSONA_SECTIONS:
...
def _read_json_mapping(path: Path) -> _MappingRead:
    return _read_mapping(path, json.loads)
...
def _read_mapping(path: Path, parse: Callable[[str], object]) -> _MappingRead:
    """Read an optional configuration object with actionable parse/IO diagnostics."""
    if not path.exists():
        return _MappingRead({})
    try:
...
def _missing_thumbnail_ttp_items(channel_dir: Path, approved_exceptions: set[str]) -> list[str]:
    """Check thumbnail configuration and reference requirements for TTP readiness."""
    missing: list[str] = []
...
def _missing_ttp_readiness_items(channel_dir: Path, channels: list[dict[str, object]]) -> tuple[list[str], set[str]]:
    ...
    missing.extend(_missing_thumbnail_ttp_items(channel_dir, approved_exceptions))
...
def _missing_video_analysis_items(channel_dir: Path, approved_slugs: list[str]) -> tuple[list[str], list[str]]:
    ...
    for slug in approved_slugs:
        ...
        if slug_error:
            ...
            continue
        slug_missing, slug_notes = _video_analysis_for_slug(
            slug,
            slug_dir or video_analysis_dir / slug,
            benchmark_by_slug.get(slug, []),
            historical_ids_by_slug.get(slug, set()),
        )
        missing.extend(slug_missing)
        notes.extend(slug_notes)
    return missing, notes


def _video_analysis_coverage(
    slug: str, expected_ids: set[str], done_ids: set[str], required: int
) -> tuple[list[str], list[str]]:
    missing: list[str] = []
    notes: list[str] = []
    done = len(done_ids)
...
def _video_analysis_for_slug(
    slug: str, slug_dir: Path, videos: list[dict[str, object]], historical_ids: set[str]
) -> tuple[list[str], list[str]]:
    missing: list[str] = []
    notes: list[str] = []
    ...
    if not expected_ids:
        ...
        return missing, notes
    historical_ids = historical_ids | expected_ids
    ...
    coverage_missing, coverage_notes = _video_analysis_coverage(slug, expected_ids, done_ids, required)
...
def _eligible_historical_video_id(video: object) -> str | None:
    """Return the ID of a sufficiently viewed, non-live historical benchmark video."""
...
def _collect_historical_channel_ids(channel: object, approved_slugs: set[str], result: dict[str, set[str]]) -> None:
    ...
    for video in channel["videos"]:
        video_id = _eligible_historical_video_id(video)
...
def _historical_benchmark_video_ids_by_slug(
    ...
    for path in _matching_files(channel_dir / "data", "benchmark_*.json"):
        ...
        for channel in payload["channels"]:
            _collect_historical_channel_ids(channel, approved_slugs, result)
    return result
...
def _validate_ttp_seed_confirmation(seed_text: str, channels: list[dict[str, object]]) -> tuple[list[str], set[str]]:
    ...
    for index, channel in enumerate(channels):
        ...
        if not _is_placeholder_relationship(relationship) and relationship.lower() not in candidate_text.lower():
            missing.append(f"ttp-seed-confirmation.md に承認済み TTP 対象の relationship が未記録 ({label})")

    approved_exceptions, exception_missing = _validate_ttp_skip_exceptions(seed_text)
    missing.extend(exception_missing)
    return missing, approved_exceptions


def _validate_ttp_skip_exceptions(seed_text: str) -> tuple[set[str], list[str]]:
    """Check document-wide skipped items against its explicitly approved exception blocks."""
    ...
    has_unapproved_skip = any(
        _line_mentions_ttp_skip(line) and line_number not in approved_exception_line_numbers
        for line_number, line in enumerate(seed_text.splitlines())
    )
    approved_exceptions, missing = _validate_approved_ttp_exception_blocks(exception_blocks)
    if has_unapproved_skip:
        missing.insert(0, "ttp-seed-confirmation.md に未承認の TTP 未反映 / スキップ項目あり")
    return approved_exceptions, missing
...
def _missing_duration_ttp_items(
    ...
    missing.extend(_missing_duration_decision_items(duration_lines))
...
def _missing_duration_decision_items(duration_lines: list[tuple[str, bool, str]]) -> list[str]:
    """Check that duration evidence, range and approval are recorded."""
    missing: list[str] = []
...
def _missing_branding_snapshot_items(
    ...
    snapshot_by_id = _index_branding_items(snapshot_items, "id")
    image_reference_by_id = _index_branding_items(image_references, "channel_id")

    missing.extend(_missing_branding_targets(channels, approved_ids, snapshot_by_id, image_reference_by_id))

    missing.extend(_missing_approved_branding_items(channel_dir, approved_ids, snapshot_by_id, image_reference_by_id))
...
def _index_branding_items(items: list[object], key: str) -> dict[str, dict[str, object]]:
    return {str(item.get(key)): item for item in items if isinstance(item, dict) and str(item.get(key) or "").strip()}


def _missing_branding_targets(
    channels: list[dict[str, object]],
...
def _missing_approved_branding_items(
    channel_dir: Path,
    approved_ids: list[str],
    snapshot_by_id: dict[str, dict[str, object]],
    image_reference_by_id: dict[str, dict[str, object]],
) -> list[str]:
    """Validate snapshot fields and image sources for approved reference channels."""
    missing: list[str] = []
...
def _missing_channel_branding_thumbnail_config(
    ...
    if thumbnail_read.error:
        return []
    reference_images = thumbnail_read.data
    for section_name in ("image_generation", "gemini", "reference_images"):
    ...
    missing = [
        f"thumbnail.yaml の reference_images.channel_branding.{field} が未設定または不正"
        for field, expected in expected_paths
        if channel_branding.get(field) != expected
    ]
...
def _suno_music_readiness(channel_dir: Path, channels: list[dict[str, object]]) -> _MusicReadiness:
    configured = _configured_suno_readiness(channel_dir)
    if configured.ready:
        return configured
...
def _configured_suno_readiness(channel_dir: Path) -> _MusicReadiness:
    errors: list[str] = []
    ...
    return _MusicReadiness(genre_ready, errors)
...
def evaluate_initial_setup_readiness(channel_dir: Path) -> ReadinessResult:
    ...
    issues.extend(_initial_description_issues(channel_dir))
...
def _initial_description_issues(channel_dir: Path) -> list[str]:
    """Validate planning description documents and report legacy migrations."""
    issues: list[str] = []
