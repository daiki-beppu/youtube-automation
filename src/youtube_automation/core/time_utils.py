...
# Preserve the public warning logger used by existing logging filters.
logger = logging.getLogger("youtube_automation.infrastructure.runtime.time_utils")
...
def _is_well_formed_bcp47_tag(locale: str) -> bool:
    ...
    index = _language_prefix_end(subtags)
    return index is not None and _valid_language_suffix(subtags, index)


def _language_prefix_end(subtags: list[str]) -> int | None:
    """Consume the language, extlang, script, and region subtags."""
    language = subtags[0]
...
def _valid_language_suffix(subtags: list[str], index: int) -> bool:
    """Validate variants, extensions, and a trailing private-use section."""
...
def parse_utc_datetime(value: object, field: str, *, context: str | None = None) -> datetime:
    """タイムゾーン付き ISO 日時を検証し UTC に変換する。"""
    suffix = f": {context}" if context is not None else ""
    invalid_message = f"{field} が不正です{suffix}"
    if not isinstance(value, str):
        raise ValidationError(invalid_message)
    try:
        parsed = datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as error:
        raise ValidationError(invalid_message) from error
    if parsed.tzinfo is None:
        raise ValidationError(f"{field} に timezone がありません{suffix}")
