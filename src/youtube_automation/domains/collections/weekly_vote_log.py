...
def _validate_axis(payload: object, *, context: str) -> AxisVote:
...
def append_weekly_vote_entry(
    *,
    channel_dir: Path,
    week_start: str | date,
    axes: Iterable[AxisVote | dict[str, object]],
    notes: str = "",
    path: Path | str | None = None,
    replace: bool = False,
) -> WeeklyVoteLog:
...
def load_weekly_vote_log_schema() -> dict[str, object]:
    ...
    """
    schema_file = resources.files("youtube_automation.domains.collections.schemas").joinpath(
