...
def check_descriptions_md_parseability(desc_md: Path, *, allowed_root: Path | None = None) -> str | None:
    ...
    if path_is_symlink(desc_md) and not path_exists(desc_md):
...
def extract_descriptions_md_tags(desc_md: Path) -> list[str] | None:
    ...
    if not path_exists(desc_md):
        return None
    raw = extract_descriptions_md_section(read_file_text(desc_md), "タグ（YouTube タグ欄）")
    if raw is None:
        return None
...
def _validate_scene_phrases(paths: CollectionPaths, supported_languages: list[str]) -> list[str]:
    ...
    return [f"workflow-state.json.scene_phrases に不足: {', '.join(missing)}"] if missing else []
