...
def _analytics_video_record(row: list, detail: dict) -> dict:
    """Combine a ranked Analytics row with optional video metadata."""
    optional_fields = ("likes", "dislikes", "comments", "shares", "subscribers_gained")
    optional_metrics = dict.fromkeys(optional_fields, 0)
    optional_metrics.update(zip(optional_fields, row[4:9], strict=False))
...
def fetch_top_video_analytics(
    ...
    *,
    ...
    """Fetch ranked pages, retaining completed pages if the API later fails."""
    videos_data = []
    ...
    while remaining_count > 0 and len(videos_data) < top_count:
        ...
        try:
            ...
            for row in response["rows"]:
                videos_data.append(_analytics_video_record(row, video_details.get(row[0], {})))
            ...
            logger.error(f"YouTube API エラー（{error_context}）: {e}")
            break

    return videos_data


def fetch_videos_analytics_parallel(
    videos: List[Dict],
    start_date: str,
    end_date: str,
    section_label: str,
    *,
    get_video_analytics: Callable[[str, str, str], dict],
    add_conversion_rates: Callable[[List[Dict]], None],
) -> List[Dict]:
    ...
    with section(section_label, count=total):
        with ThreadPoolExecutor(max_workers=_MAX_WORKERS) as executor:
            futures = {
                executor.submit(get_video_analytics, video["video_id"], start_date, end_date): video for video in videos
            }
            ...
            for future in as_completed(futures):
                ...
                except (RuntimeError, YouTubeAPIError):
                    logger.exception("動画 %s の analytics 取得に失敗", video["video_id"])
                    continue

                combined_data = {**video, **analytics_data}
                add_conversion_rates([combined_data])
...
def add_subscriber_conversion_rates(videos: List[Dict]) -> None:
...
def build_subscriber_conversion_ranking(videos: List[Dict]) -> List[Dict]:
...
def collect_combined_analytics(
    start_date: str,
    end_date: str,
    top_count: int = 50,
    recent_days: int = 30,
    *,
    get_all_videos: Callable[[], List[Dict]],
    get_top_analytics: Callable[[str, str, int], List[Dict]],
    get_video_analytics: Callable[[str, str, str], Dict],
    add_conversion_rates: Callable[[List[Dict]], None],
) -> Dict:
    """
    ...
    top_videos_data = get_top_analytics(start_date, end_date, top_count)
    for video_data in top_videos_data:
        video_data["is_recent"] = video_data["video_id"] in recent_video_ids
    ...
    for video in recent_videos_info:
        if video["video_id"] not in top_video_ids:
            analytics_data = get_video_analytics(video["video_id"], start_date, end_date)
    ...
    add_conversion_rates(top_videos_data)
    add_conversion_rates(recent_videos_data)
...
def rank_video_analytics(
    ...
    *,
    selection_label: str,
    empty_message: str,
    fetch_parallel: Callable[[List[Dict], str, str, str], List[Dict]],
    add_conversion_rates: Callable[[List[Dict]], None],
) -> List[Dict]:
    """Collect a nonempty selection, rank by views, and report conversion rates."""
    if not videos:
        logger.error(empty_message)
        return []
    logger.info("%s: %s本の動画のAnalyticsデータを取得開始...", selection_label, len(videos))
    videos_data = fetch_parallel(videos, start_date, end_date, section_label)
    videos_data.sort(key=lambda video: video.get("views", 0), reverse=True)
    add_conversion_rates(videos_data)
    logger.info("%sAnalytics取得完了: %s本", selection_label, len(videos_data))
    return videos_data


def collect_strategic_video_analytics(
    start_date: str,
    end_date: str,
    mode: str = "efficient",
    *,
    get_combined: Callable[[str, str, int, int], Dict],
    get_all: Callable[[str, str], List[Dict]],
    get_top: Callable[[str, str, int], List[Dict]],
    get_recent: Callable[[str, str, int], List[Dict]],
    add_conversion_rates: Callable[[List[Dict]], None],
    build_ranking: Callable[[List[Dict]], List[Dict]],
) -> Dict:
    """
    ...
    if mode == "efficient":
        ...
        combined_data = get_combined(start_date, end_date, 50, 30)
        result["top_videos"] = combined_data["top_videos"]
        result["recent_videos"] = combined_data["recent_videos"]

    elif mode == "comprehensive":
        logger.info("包括モード: 全動画")
        result["all_videos"] = get_all(start_date, end_date)

    elif mode == "top_only":
        logger.info("上位のみモード: 上位50本")
        result["top_videos"] = get_top(start_date, end_date, 50)

    elif mode == "recent_only":
        logger.info("直近のみモード: 直近30日投稿")
        result["recent_videos"] = get_recent(start_date, end_date, 30)
    ...
    add_conversion_rates(all_videos)
    result["summary"] = {
    ...
    result["subscriber_conversion_ranking"] = build_ranking(all_videos)
...
class StrategicAnalyticsMixin:
    ...
    _add_subscriber_conversion_rates = staticmethod(add_subscriber_conversion_rates)

    _build_subscriber_conversion_ranking = staticmethod(build_subscriber_conversion_ranking)

    def get_combined_analytics(
        self, start_date: str, end_date: str, top_count: int = 50, recent_days: int = 30
    ) -> Dict:
        return collect_combined_analytics(
            start_date,
            end_date,
            top_count,
            recent_days,
            get_all_videos=self.get_all_channel_videos,
            get_top_analytics=partial(self._fetch_top_video_analytics, error_context="上位動画取得"),
            get_video_analytics=self.get_video_analytics_by_id,
            add_conversion_rates=self._add_subscriber_conversion_rates,
        )

    def get_strategic_video_analytics(self, start_date: str, end_date: str, mode: str = "efficient") -> Dict:
        return collect_strategic_video_analytics(
            start_date,
            end_date,
            mode,
            get_combined=self.get_combined_analytics,
            get_all=self.get_all_video_analytics,
            get_top=self.get_top_video_analytics,
            get_recent=self.get_recent_video_analytics,
            add_conversion_rates=self._add_subscriber_conversion_rates,
            build_ranking=self._build_subscriber_conversion_ranking,
        )
    ...
    def get_all_video_analytics(self, start_date: str, end_date: str) -> List[Dict]:
        ...
        return self._fetch_ranked_video_analytics(
            self.get_all_channel_videos(),
            start_date,
            end_date,
            "strategic_analytics.all_videos_loop",
            selection_label="全動画",
            empty_message="動画リストの取得に失敗しました",
        )
    ...
    def get_top_video_analytics(self, start_date: str, end_date: str, top_count: int = 50) -> List[Dict]:
        ...
        videos_data = self._fetch_top_video_analytics(start_date, end_date, top_count, error_context="バッチ取得")

        logger.info(f"上位動画Analytics取得完了: {len(videos_data)}本")
        self._add_subscriber_conversion_rates(videos_data)
        return videos_data

    def _fetch_top_video_analytics(
        self, start_date: str, end_date: str, top_count: int, *, error_context: str
    ) -> List[Dict]:
        return fetch_top_video_analytics(
    ...
    def get_recent_video_analytics(self, start_date: str, end_date: str, days: int = 30) -> List[Dict]:
        ...
        return self._fetch_ranked_video_analytics(
            self.get_recent_videos(days),
            start_date,
            end_date,
            "strategic_analytics.recent_videos_loop",
            selection_label="直近動画",
            empty_message="直近投稿動画が見つかりません",
        )

    def _fetch_ranked_video_analytics(
        self,
        videos: List[Dict],
        start_date: str,
        end_date: str,
        ...
        *,
        ...
        return rank_video_analytics(
            ...
            fetch_parallel=self._fetch_videos_analytics_parallel,
            add_conversion_rates=self._add_subscriber_conversion_rates,
        )

    def _fetch_videos_analytics_parallel(
        self, videos: List[Dict], start_date: str, end_date: str, section_label: str
    ) -> List[Dict]:
        return fetch_videos_analytics_parallel(
            videos,
            start_date,
            end_date,
            section_label,
