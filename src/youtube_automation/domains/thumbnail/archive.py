...
def archive_approved_thumbnail_transaction(
    collection: Path,
    *,
    archive_config: Mapping[str, object],
    channel_root: Path,
) -> ThumbnailArchiveUpdate | None:
    """設定が有効なら承認済み thumbnail を更新し、復元可能な結果を返す。"""
    if not _archive_enabled(archive_config):
        return None
    ...
    gallery, target = _validate_destination(channel_root, paths.root, source)
    alternate_targets = [
...
def archive_approved_thumbnail(
    collection: Path,
    *,
    archive_config: Mapping[str, object],
    channel_root: Path,
    ...
    update = archive_approved_thumbnail_transaction(
        collection,
