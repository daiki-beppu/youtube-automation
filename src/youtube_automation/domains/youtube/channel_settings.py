...
def fetch_channel(youtube) -> dict[str, Any]:
    ...
    try:
        resp = execute_youtube_request(
            youtube.channels().list(part=_COMBINED_PARTS, mine=True),
    ...
    try:
        items = validate_youtube_response_items(resp, "channels().list")
    ...
    if not isinstance(item, dict):
        raise YouTubeAPIError("channels().list() returned an invalid channel item")

    result = item.copy()
    result["localizations"] = _fetch_localizations(youtube)
    return result
...
def _fetch_localizations(youtube) -> dict[str, Any]:
    ...
    try:
        resp = execute_youtube_request(
            youtube.channels().list(part=_LOCALIZATIONS_PART, mine=True),
            "channels().list(localizations) failed",
        )
    except (TypeError, ValueError, OSError) as e:
        raise YouTubeAPIError(f"channels().list(part={_LOCALIZATIONS_PART}) failed: {e}") from e

    try:
        items = validate_youtube_response_items(resp, "channels().list(localizations)")
    except ValidationError as e:
        raise YouTubeAPIError(str(e)) from e
    if not items:
        return {}
    item = items[0]
    if not isinstance(item, dict):
        raise YouTubeAPIError("channels().list(localizations) returned an invalid channel item")
    localizations = item.get("localizations")
    if localizations is None:
        return {}
    if not isinstance(localizations, dict):
        raise YouTubeAPIError("channels().list(localizations) returned invalid localizations")
