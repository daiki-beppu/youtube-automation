...
`--iterate` は公開済み動画の CTR や比較対象を読み、再利用できる勝ち筋を champion として保存します。通常生成はこの記録を参照してプロンプトへ反映します。分析対象の動画 ID は引数では渡さず、対象 collection の `workflow-state.json` に記録された公開済み動画から自動で特定されるため、対象 collection を 1 件に確定でき、比較期間の Analytics が揃った状態で実行してください。
...
`--loop` は確定済みの textless `main.png` / `main.jpg` を Veo、fal.ai、または Gemini Omni Flash へ渡し、動画本編で繰り返せる `loop.mp4` を作ります。Shorts の 9:16 ループは `yt-generate-shorts-loop --engine fal` でも生成できます。静止画の確定前には実行せず、生成後は継ぎ目と意図しない文字・動きをプレビューしてください。
