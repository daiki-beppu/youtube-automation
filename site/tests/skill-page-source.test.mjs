...
test("実リポジトリでは9カテゴリと全skillを生成する", async () => {
  ...
  assert.equal(result.entries.length, 45);
  assert.equal(skillDirectories?.length, 44);
...
test("production build は一覧と44個の個別ページを公開する", async () => {
  ...
  assert.match(index, /44 個の skill/);
