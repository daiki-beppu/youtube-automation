...
test("実リポジトリでは9カテゴリと全skillを生成する", async () => {
  ...
  assert.equal(result.entries.length, 28);
  assert.equal(skillDirectories?.length, 27);
...
test("production build は一覧と27個の個別ページを公開する", async () => {
  ...
  assert.match(index, /27 個の skill/);
