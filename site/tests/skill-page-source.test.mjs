...
test("実リポジトリでは9カテゴリと全skillを生成する", async () => {
  ...
  assert.equal(result.entries.length, 44);
  assert.equal(skillDirectories?.length, 43);
...
test("production build は一覧と43個の個別ページを公開する", async () => {
  ...
  assert.match(index, /43 個の skill/);
