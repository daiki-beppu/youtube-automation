...
test("実リポジトリでは9カテゴリと全skillを生成する", async () => {
  ...
  assert.equal(result.entries.length, 46);
  assert.equal(skillDirectories?.length, 45);
...
test("production build は一覧と45個の個別ページを公開する", async () => {
  ...
  assert.match(index, /45 個の skill/);
