...
test("実リポジトリでは9カテゴリと全skillを生成する", async () => {
  ...
  assert.equal(result.entries.length, 52);
  assert.equal(skillDirectories?.length, 51);
...
test("production build は一覧と51個の個別ページを公開する", async () => {
  ...
  assert.match(index, /51 個の skill/);
