...
- `build_collections_index(root)`
    status / downloaded_count を含む collection index を返す。mapped / playlist_name は返さない。
- `create_server(..., playlist_capture: tuple[Path, str | None] | None=None)`
    prefix 付き playlist_capture 指定時のみ POST `/suno/playlists` を有効化する。
...
# ---------------------------------------------------------------------------
# _resolve_playlist_capture: root/prefix の意味的不変条件 + env fallback
# 要件 1/2/4: root 指定で capture root 有効、prefix は Suno playlist capture のみ必須。
# ---------------------------------------------------------------------------
...
def test_resolve_playlist_capture_allows_root_only_for_distrokid_capture(monkeypatch):
    """Given root だけ指定・prefix 不在
    When _resolve_playlist_capture を呼ぶ
    Then DistroKid release capture 用に `(root, None)` を返す。
    """
    monkeypatch.delenv("PLAYLIST_CAPTURE_ROOT", raising=False)
    monkeypatch.delenv("PLAYLIST_CAPTURE_PREFIX", raising=False)

    assert _resolve_playlist_capture("/tmp/channel", None) == (Path("/tmp/channel"), None)
...
def test_resolve_playlist_capture_allows_env_root_only_for_distrokid_capture(monkeypatch):
    """Given env に root だけある（prefix は CLI 引数・env とも不在）
    When _resolve_playlist_capture を呼ぶ
    Then DistroKid release capture 用に env root を返す。
    """
    monkeypatch.setenv("PLAYLIST_CAPTURE_ROOT", "/srv/df365-channel")
    monkeypatch.delenv("PLAYLIST_CAPTURE_PREFIX", raising=False)

    assert _resolve_playlist_capture(None, None) == (Path("/srv/df365-channel"), None)
...
# ---------------------------------------------------------------------------
# build_collections_index: status contract
# ---------------------------------------------------------------------------
...
def test_post_suno_playlists_returns_404_when_capture_prefix_absent(tmp_path, serve_capture):
    """Given capture_root はあるが prefix が無い DistroKid-only capture
    When 許可 Origin から POST /suno/playlists する
    Then Suno playlist capture は無効のため 404 を返す。
    """
    base = serve_capture(capture_root=tmp_path / "out", prefix=None)
    payload = [{"title": "df365 | smoke", "url": "https://suno.com/playlist/u1"}]
...
def test_post_suno_playlists_body_too_large_returns_413(tmp_path, serve_capture):
