...
def test_main_nests_upload_settings_in_schedule_config(tmp_path):
    # Given/When: フルパッケージ生成を実行
    rc = main(_required_args(tmp_path))

    # Then: upload_settings は root file ではなく schedule_config.json に内包される
    assert rc == 0
    assert not (tmp_path / "config" / "upload_settings.json").exists()
    schedule = _read_json(tmp_path / "config" / "schedule_config.json")
    assert schedule["upload_settings"] == {
