...
def test_invalid_input_fails_before_upload(
    ...
    overrides: dict[str, object], size: tuple[int, int],
) -> None:
    upload = Mock()
    ...
    assert not generator.generate_loop_video(
        _image(tmp_path / "main.png", size), tmp_path / "loop.mp4", **arguments,
...
def test_submit_uses_one_resized_url_and_records_metadata_only(
    ...
    submit = Mock(return_value={
        "request_id": "request-1", "response_url": "https://queue.fal.run/result",
        "status_url": "https://queue.fal.run/status", "cancel_url": "https://queue.fal.run/cancel",
    })
    get_url = Mock(side_effect=[{"status": "COMPLETED"}, {
        "video": {"url": "https://v3.fal.media/video.mp4"},
        "expanded_prompt": "expanded", "metrics": {"inference_time": 7.5},
    }])
    ...
    assert generator.generate_loop_video(
        image, output, generator.DEFAULT_MODEL, "gentle motion", channel_root=tmp_path,
    )
...
def test_matching_state_resumes_without_submit(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    generator.task_store.save(
        output, image, request_id="request-1", response_url="https://queue.fal.run/result",
        status_url="https://queue.fal.run/status", cancel_url="https://queue.fal.run/cancel",
        submitted_at="2026-09-04T00:00:00Z", model=generator.DEFAULT_MODEL,
        prompt="motion", duration_seconds=5, resolution="768P",
        prompt_expansion_mode="balanced", channel_root=tmp_path,
    )
    submit = Mock()
    monkeypatch.setattr(generator.fal_client, "submit", submit)
    monkeypatch.setattr(generator.fal_client, "upload_file", Mock())
    monkeypatch.setattr(generator.fal_client, "get_url", Mock(side_effect=[
        {"status": "COMPLETED"}, {"video": {"url": "https://v3.fal.media/out.mp4"}},
    ]))
    monkeypatch.setattr(generator.fal_client, "download", Mock(return_value=b"\0\0\0\x18ftypmp42video"))
    monkeypatch.setattr(generator, "smooth_loop", Mock(return_value=True))
    ...
    assert generator.generate_loop_video(
        image, output, generator.DEFAULT_MODEL, "motion", channel_root=tmp_path,
    )
