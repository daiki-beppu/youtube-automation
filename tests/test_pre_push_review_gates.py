...
def test_any_usage_gate_ignores_python_comments_and_strings_with_any_alias(tmp_path: Path) -> None:
    repo = _init_repo_with_origin_main(tmp_path)
    _write(
        repo,
        "src/youtube_automation/prose.py",
        "\n".join(
            [
                "from typing import Any",
                "# Any mention in a comment is not a type annotation.",
                "text = 'Any mention in a string'",
                "",
            ]
        ),
    )
    _commit_all(repo, "add python prose mentioning Any")

    result = _run_gate(repo, _ANY_USAGE_GATE_PATH)

    assert result.returncode == 1
    assert "src/youtube_automation/prose.py:1" in result.stderr
    assert "src/youtube_automation/prose.py:2" not in result.stderr
    assert "src/youtube_automation/prose.py:3" not in result.stderr
