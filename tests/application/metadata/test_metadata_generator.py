...
テスト対象: application/metadata/service.py
...
from youtube_automation.application.metadata import service as metadata_generator_module
from youtube_automation.application.metadata.service import BAHMetadataGenerator
from youtube_automation.configuration import load_config
from youtube_automation.core.errors import ValidationError
from youtube_automation.core.time_utils import format_duration_display, format_localized_duration_display
...
class TestSunoPatternTrackNames:
    ...
    @pytest.mark.parametrize("payload", ["- unexpected", "unexpected", "42"])
    def test_non_object_pattern_document_uses_fallback(self, tmp_path, payload):
        gen = _make_generator()
        gen.collection_path = tmp_path
        docs = tmp_path / "20-documentation"
        docs.mkdir()
        (docs / "suno-patterns.yaml").write_text(payload, encoding="utf-8")
...
@pytest.mark.parametrize(
    ...
    [
        None,
        "{broken",
        json.dumps({"collection_name": 12, "title_activity": [], "theme": {}, "track_display_names": []}),
    ],
)
def test_optional_workflow_metadata_falls_back_without_changing_tracks(tmp_path, content):
    gen = _make_generator("20250907-live-8bit-treasure-collection")
    gen.collection_path = tmp_path
    if content is not None:
        (tmp_path / "workflow-state.json").write_text(content)
    gen.tracks = [{"filename": "track.mp3", "title": "Original"}]
    assert gen._extract_theme_name() == "Treasure"
    assert gen._get_activity() == gen.config.content.title.activity_for_theme("Treasure")
    assert gen._load_theme() == ""
    gen._apply_persisted_display_names()
    assert gen.tracks == [{"filename": "track.mp3", "title": "Original"}]


def test_optional_workflow_metadata_overrides_remain_independent(tmp_path):
    gen = _make_generator()
    gen.collection_path = tmp_path
    (tmp_path / "workflow-state.json").write_text(
        json.dumps(
            {
                "collection_name": "Evening COLLECTION",
                "title_activity": "Reading",
                "theme": "night",
                "track_display_names": {"track.mp3": "Moonlight"},
            }
        )
    )
    gen.tracks = [{"filename": "track.mp3", "title": "Original"}]
    assert gen._extract_theme_name() == "Evening"
    assert gen._get_activity() == "Reading"
    assert gen._load_theme() == "night"
    gen._apply_persisted_display_names()
    assert gen.tracks == [{"filename": "track.mp3", "title": "Moonlight"}]


def test_metadata_report_preserves_crossfade_duration_and_track_order():
    gen = _make_generator()
    gen._crossfade_sec = 2.0
    gen.tracks = [
        {"title": "First", "duration": 60, "timestamp": "00:00"},
        {"title": "Second", "duration": 90, "timestamp": "00:58"},
    ]
    report = gen.generate_metadata_report()
    assert "総再生時間: 02:28" in report
    assert "  01. 00:00 First (01:00)\n  02. 00:58 Second (01:30)" in report
    assert gen.config.meta.channel_name in report
    assert str(gen.collection_path) in report
