...
テスト対象: `src/youtube_automation/application/metadata/service.py::BAHMetadataGenerator.generate_shorts_metadata`
...
@pytest.mark.parametrize("name", ["20250907-live-8bit-adventure-music", "20260909-live-16bit-rainy-ver2", "plain-name"])
def test_direct_shorts_metadata_ignores_unrelated_skill_settings(tmp_path, monkeypatch, name):
    from youtube_automation.application.metadata import service

    collection = _make_collection(tmp_path, name, theme="adventure")
    legacy = _make_generator(collection_path=collection)
    expected = legacy.generate_shorts_metadata("https://youtu.be/example")

    def reject_unrelated_settings(*args, **kwargs):
        raise AssertionError("Shorts metadata must not load audio mastering or long-form video settings")

    monkeypatch.setattr(service, "load_skill_config", reject_unrelated_settings)
    result = service.generate_shorts_metadata(str(collection), "https://youtu.be/example")

    assert result == expected
    assert "#Shorts" in result["title"]
    assert "https://youtu.be/example" in result["description"]
