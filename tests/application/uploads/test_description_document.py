...
from youtube_automation.application.uploads.youtube import CompleteCollectionStrategy
...
def test_upload_loader_reads_only_validated_json_pair(tmp_path: Path) -> None:
    ...
    metadata = CompleteCollectionStrategy.load_description_document(tmp_path)
...
def test_upload_loader_rejects_legacy_markdown_instead_of_parsing_it(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValidationError, match="明示 migration"):
        CompleteCollectionStrategy.load_description_document(tmp_path)
