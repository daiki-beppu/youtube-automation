...
@pytest.mark.parametrize("value", ["2026-09-07T01:00:00Z", "2026-09-07T10:00:00+09:00"])
def test_parse_timestamp_normalizes_offsets(value):
    assert parse_utc_datetime(value, "published_at") == datetime(2026, 9, 7, 1, tzinfo=timezone.utc)


@pytest.mark.parametrize("value", [None, 7, "not-a-date", "2026-99-07T01:00:00Z"])
@pytest.mark.parametrize("context", [None, "", "video-123"])
def test_parse_timestamp_invalid_value_preserves_diagnostic_context(value, context):
    suffix = f": {context}" if context is not None else ""
    with pytest.raises(ValidationError) as error:
        parse_utc_datetime(value, "published_at", context=context)
    assert str(error.value) == f"published_at が不正です{suffix}"


@pytest.mark.parametrize("value", ["2026-09-07", "2026-09-07T01:00:00"])
def test_parse_timestamp_requires_timezone(value):
    with pytest.raises(ValidationError) as error:
        parse_utc_datetime(value, "published_at", context="video-123")
    assert str(error.value) == "published_at に timezone がありません: video-123"
