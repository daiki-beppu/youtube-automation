...
- **takt 設定**: リポジトリ固有 `.takt/config.yaml`（`draft_pr: false`）、グローバル `~/.takt/config.yaml`（`provider: claude`, `language: ja`）。現状 takt は Claude Agent SDK ベースで動作する（Codex 経由で takt を回す前提はない）
...
そのため **skill 関連の修正は takt にやらせず、通常の対話セッション（Claude Code / Codex CLI どちらでも可、cmux pane 等）で直接行う**。対話セッションでは prompt に手動 Allow できるため、`.claude/skills/**` への編集が通る。コミット・PR 作成は `commit-convention` / `pr` スキル経由で実施する。
