...
- commit / push 前に、対象 checkout（親 checkout / worktree のどちらも）で `nix develop` または direnv に入り、`flake.nix` の shellHook による `lefthook install --force` を通す
- `Can't find lefthook in PATH` が出る場合は `docs/development.md` の Git hooks 診断手順に従い、対象 checkout で `nix develop --command lefthook install --force` を実行する
