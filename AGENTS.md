...
- 変更した test file / node ID と、変更に直接必要な `tests/repo/` の contract test を focused test の対象にする。標準形は `UV_NO_SYNC=1 bash .claude/skills/automation/references/pytest-quiet.sh <targets>`（内部の `uv run pytest` に `--no-sync` 相当を適用）
