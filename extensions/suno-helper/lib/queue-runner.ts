...
async function waitForQueueCapacity(
  ...
  options.emitProgress({
    ...
    index,
    total: options.total,
    message: options.hasObservedAnyTraffic()
      ? undefined
...
async function submitQueueEntry(
  ...
  await injectWithVerification({
    inject: () =>
      options.submitEntryToQueue(options.entries[index], index, options.total),
    markBeforeInject: () =>
      markAck({
...
export async function finalizeQueueEntriesYield(
  ...
  const pendingEntries = new Map<
    number,
    { clipIds: string[]; yieldRetryCount: number }
  >();
  ...
  while (pendingEntries.size > 0) {
    ...
    for (const index of options.order) {
      ...
      try {
        attemptResult = {
          ...
          evaluation: evaluateClips(
            entryClipIds.map((id) => ({
              id,
              duration: options.getDuration(id),
            })),
            durationFilter
          ),
        };
      ...
      if (decision.kind === "retry") {
        if (options.durationOutlierStrategy.kind !== "regenerate") {
          throw new Error(
            "duration retain policy returned an invalid retry decision"
          );
        }
    ...
    for (const retry of retries) {
      try {
        const regeneratedClipIds =
          await durationOutlierStrategy.regenerateEntry(
            retry.index,
            retry.yieldRetryCount
          );
        if (options.isAborted?.()) {
        ...
        console.warn(
          `[suno-helper] entry ${retry.index} の duration 再生成に失敗しました: ${regenerationMessage}`
        );
        options.emitProgress({
          phase: PHASE.ENTRY_FAILED,
          index: retry.index,
          ...
          log: {
            kind: "skip",
            entryName: entryDisplayName(options.entries[retry.index]),
          },
        });
      }
    }
    ...
    const completionResults = await Promise.all(
      submittedRetries.map(async (retry) => {
        try {
          await durationOutlierStrategy.waitForRegeneratedClips(
            retry.regeneratedClipIds
          );
    ...
    for (const completion of completionResults) {
      if (completion.error !== undefined) {
        ...
        console.warn(
          `[suno-helper] entry ${completion.index} の duration 再生成に失敗しました: ${completion.error}`
        );
        options.emitProgress({
          phase: PHASE.ENTRY_FAILED,
          ...
          log: {
            kind: "skip",
            entryName: entryDisplayName(options.entries[completion.index]),
          },
        });
        continue;
      }
      options.dropSubmittedIds(completion.previousClipIds);
      options.clipIdsByEntry.set(
        completion.index,
        completion.regeneratedClipIds
      );
...
export async function submitQueueEntries(
  options: QueueSubmissionOptions
  ...
  for (const [orderPosition, index] of options.order.entries()) {
    ...
    const result = await runEntryWithRetry({
      ...
      wasSubmitted: (error) =>
        ...
        !(error instanceof InjectNotAcknowledgedError),
      isFatal: (error) => error instanceof FatalRunError,
      maxRetry: options.preset.maxEntryRetry,
      retryDelayMs: () =>
        options.applyJitter(
          options.preset.interCreateDelayMs,
          options.preset.jitterMs
        ),
      onRetry: (attempt, max) =>
        options.emitProgress({
          phase: PHASE.WAITING_SLOT,
    ...
    if (result.outcome === "failed") {
      ...
      failedIndices.push(index);
      console.warn(
        `[suno-helper] entry ${index} をスキップして続行します: ${message}`
      );
      options.emitProgress({
        phase: PHASE.ENTRY_FAILED,
        index,
      ...
      if (result.outcome === "presumed-done") {
        const message =
          result.error instanceof Error
            ...
            : String(result.error);
        console.warn(
          `[suno-helper] entry ${index} は投入済みのため生成済み扱いで続行します: ${message}`
        );
      }
      clipIdsByEntry.set(
        index,
        getNewSubmittedIds(submittedBeforeEntry, options.getSubmittedIds())
      );
      options.emitProgress({
...
export async function waitForSubmittedClipsComplete(
  ...
  while (!options.isAborted()) {
    ...
    const observedSubmittedCount = new Set([
      ...options.previousSubmittedClipIds,
      ...submittedIds,
    ]).size;
    const pendingSubmittedIds = Array.from(
      new Set([
        ...options.getPendingIdsByIds(options.previousSubmittedClipIds),
        ...options.getPendingSubmittedIds(),
      ])
    );
    if (
    ...
    const pendingCountDecreased =
      ...
      pendingSubmittedIds.length < lastPendingCount;
