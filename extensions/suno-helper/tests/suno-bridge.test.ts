...
describe("suno-bridge fetch interceptor", () => {
  it("Given feed v3 POST response When fetch resolves Then FEED_CLIPS を postMessage する", async () => {
    const originalFetch = vi.fn(async () =>
      jsonResponse({ clips: [{ id: "clip-1", status: "complete" }] })
    );
  ...
  it("Given generate response When fetch resolves Then GENERATE_CLIPS は従来通り postMessage する", async () => {
    const originalFetch = vi.fn(async () =>
      jsonResponse({ clips: [{ id: "clip-1", status: "submitted" }] })
    );
    vi.stubGlobal("fetch", originalFetch);
    const postMessage = vi
      .spyOn(window, "postMessage")
    ...
    await loadBridge();
    await window.fetch(`${SUNO_API_ORIGIN}${GENERATE_ENDPOINT_PATH}`, {
      method: "POST",
    });
    await flushObservedFetch();
  ...
  it("Given auth 捕捉済み When active poll request を受ける Then feed v3 POST で duration 付き clip を返す", async () => {
    const originalFetch = vi.fn(async () =>
      jsonResponse({
        clips: [
          { id: "clip-1", status: "complete", metadata: { duration: 121.5 } },
        ],
      })
    );
    vi.stubGlobal("fetch", originalFetch);
  ...
  it("Given 対象 clip が先頭ページにない When active poll request を受ける Then cursor を辿って全対象を返す", async () => {
    ...
    const originalFetch = vi
      ...
      .mockResolvedValueOnce(
        jsonResponse({
          clips: [{ id: "new", status: "complete" }],
          next_cursor: "cursor-2",
        })
      )
      .mockResolvedValueOnce(
        jsonResponse({ clips: [{ id: "old", status: "complete" }] })
      );
    vi.stubGlobal("fetch", originalFetch);
    const postMessage = vi
      .spyOn(window, "postMessage")
