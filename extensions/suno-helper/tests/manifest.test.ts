...
//
// 契約 (#893 で `tabs`、#1146 で `downloads`、#1251 で `debugger` を追加):
//   storage / activeTab / tabs / downloads / debugger。
// `tabs` は自動 POST trigger、`downloads` は Suno playlist の ZIP ダウンロード監視、
// `debugger` は trusted Cmd+P dispatch に必要。
...
const EXPECTED_PERMISSIONS = ["storage", "activeTab", "tabs", "downloads", "debugger"];
