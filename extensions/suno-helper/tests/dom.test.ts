...
// placeholder は UI ロケールで変わる（英語 "Exclude styles" / 日本語「スタイルを除外」、#1840）。
function addExcludeInput(opts: { visible?: boolean; placeholder?: string } = {}): HTMLInputElement {
  const input = document.createElement("input");
  input.type = "text";
...
describe("resolveAdvancedFields: More Options 3 フィールドの解決 (#900, fail-soft)", () => {
  ...
  it("Given 日本語 UI placeholder「スタイルを除外」の input When 解決する Then excludeStyles として返す（#1840）", () => {
    // Suno の表示言語が日本語だと placeholder が「スタイルを除外」になり、英語決め打ち
    // セレクタでは未検出 → 誤案内の FatalRunError で run が停止していた回帰の再現ケース。
  ...
  it("Given 日本語 UI placeholder「スタイルを除外」の textarea When 解決する Then excludeStyles として返す（#1840）", () => {
