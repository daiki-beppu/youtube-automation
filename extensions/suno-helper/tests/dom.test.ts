...
function addInput(
  opts: { placeholder?: string; visible?: boolean } = {}
): HTMLInputElement {
...
function addCreateFormMode(
  selectedName: "Simple" | "Advanced" | "Sounds",
  groupLabel = "作成フォームモード"
): void {
  addModeGroup(
    "tablist",
    "tab",
...
describe("resolveFields: data-testid ベースの Style / Lyrics 解決 (#807)", () => {
  it("Given lyrics-textarea と別の visible textarea When 解決する Then data-testid で lyrics を、残りを style とする", () => {
    const lyrics = addTextarea({
      ...
      placeholder: "What do you want your lyrics to be about?",
    });
  ...
  it("Given 日本語 UI (style placeholder がジャンル語彙) When 解決する Then placeholder 非依存で正しく振り分ける", () => {
    ...
    const lyrics = addTextarea({
      testId: "lyrics-textarea",
      placeholder:
        "What do you want your lyrics to be about? Suno will write new lyrics every generation.",
    });
    const style = addTextarea({
      placeholder:
        "地下の罠, コントラルト, リズミカルなベース, ソフトパンク, メリスマ",
    });
  ...
  it("Given data-testid 無しの visible textarea 1 枚 (instrumental) When 解決する Then style に解決し lyrics は null", () => {
    const only = addTextarea({
      placeholder: "地下の罠, コントラルト, リズミカルなベース",
    });
...
describe("setLyricsValue: textarea / Lexical contenteditable 両対応の Lyrics 注入 (2026-07 UI 改装)", () => {
  ...
  function applyLexicalBeforeInput(el: HTMLElement): void {
    el.addEventListener("beforeinput", (e) => {
      ...
      el.textContent =
        ev.inputType === "deleteContentBackward" ? "" : (ev.data ?? "");
      e.preventDefault();
    });
  }
  ...
  it("Given contenteditable div When 注入する Then focus → selectAll → (selection 同期待ち) → paste の順で dispatch する", async () => {
    ...
    expect(pastes).toEqual([
      { text: "verse one", bubbles: true, cancelable: true },
    ]);
  ...
  it("Given paste 後に Lyrics が反映されない When Lexical lyrics へ注入する Then fail-loud する", async () => {
    ...
    const injected = setLyricsValue(div, "verse one");
    const rejection = expect(injected).rejects.toThrow(
      "Lyrics 欄への paste 反映に失敗しました"
    );
    await vi.advanceTimersByTimeAsync(400);
...
describe("detectRecaptcha: 可視なチャレンジのみ検知 (#810)", () => {
  ...
  describe("title 判定による active challenge 検知の 4 組合せ (#875, #924)", () => {
    ...
    const HCAPTCHA_CHALLENGE_SRC =
      "https://hcaptcha-assets-prod.suno.com/captcha/v1/x#frame=challenge";
