...
// Suno Exclude styles の native text input を模す（英語 UI を既定にし、ローカライズ表記も指定可能）。
function addExcludeInput(opts: { visible?: boolean; placeholder?: string; ariaLabel?: string } = {}): HTMLInputElement {
  const input = document.createElement("input");
  input.type = "text";
  input.placeholder = opts.placeholder ?? "Exclude styles";
...
describe("resolveAdvancedFields: More Options 3 フィールドの解決 (#900, fail-soft)", () => {
  ...
  it.each([
    { element: "input", attribute: "placeholder", label: "スタイルを除外" },
    { element: "textarea", attribute: "placeholder", label: "スタイルを除外" },
    { element: "input", attribute: "aria-label", label: "除外するスタイル" },
    { element: "textarea", attribute: "aria-label", label: "除外するスタイル" },
  ])(
    "Given 日本語 UI の $element[$attribute='$label'] When 解決する Then excludeStyles として返す (#1840)",
    ({ element, attribute, label }) => {
      const exclude = document.createElement(element) as HTMLInputElement | HTMLTextAreaElement;
      exclude.setAttribute(attribute, label);
      document.body.appendChild(exclude);
      setRect(exclude, VISIBLE_RECT);
  ...
  it("Given 日本語 UI の Exclude Styles 欄 When advanced 値を注入する Then 欄へ値が反映される (#1840)", async () => {
    const exclude = addExcludeInput({ placeholder: "スタイルを除外" });
    const inputListener = vi.fn();
    exclude.addEventListener("input", inputListener);

    await injectAdvancedFields({ exclude_styles: "hyperpop, edm" }, resolveAdvancedFields());
