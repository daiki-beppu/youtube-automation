  ...
  await page.evaluate(() => {
    ...
    function setSliderValue(slider: HTMLElement, target: number): void {
      ...
      for (let i = 0; i < steps; i++) {
        slider.dispatchEvent(new KeyboardEvent("keydown", { key, bubbles: true, composed: true }));
      }
    }

    const exclude = document.querySelector('input[placeholder="Exclude styles"]') as HTMLInputElement;
    setNativeValue(exclude, "hyperpop, edm");
    setSliderValue(document.querySelector('[role="slider"][aria-label="Weirdness"]') as HTMLElement, 30);
    setSliderValue(document.querySelector('[role="slider"][aria-label="Style Influence"]') as HTMLElement, 85);
  });
...
test("Lexical Lyrics mock (2026-07 UI) で instrumental 空文字を selectAll → delete でクリアする", async ({ page }) => {
  await page.setContent(MOCK_SUNO_HTML_LEXICAL);
