...
function getInFlightClipCountInPage(): number {
  ...
  const isDocumentRoot = (root: HTMLElement): boolean =>
    root === document.body || root === document.documentElement;
  const hasExactlyOne = (root: HTMLElement, selector: string): boolean =>
  ...
  const hasWaveformActions = (root: HTMLElement): boolean =>
    hasExactlyOne(root, SELECT_CLIP_SELECTOR) &&
    hasExactlyOne(root, REMIX_BTN_SELECTOR);
  const hasGridActions = (root: HTMLElement): boolean =>
    hasExactlyOne(root, REMIX_BTN_SELECTOR);
  const isAlternateViewCardBoundary = (root: HTMLElement): boolean =>
    ...
    while (current) {
      if (
        ...
        (hasListActions(current) ||
          (isAlternateViewCardBoundary(current) &&
            (hasWaveformActions(current) || hasGridActions(current))))
      ) {
...
function getInstalledInFlightClipCountInPage(): number {
  return (
    window as unknown as InFlightCounterWindow
  ).__sunoHelperE2EGetInFlightClipCount();
}
...
test("11 件目は in-flight 上限 (20 clip) で待機し、1 clip 完了で投入が再開する", async ({
  page,
}) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    document
      .querySelector<HTMLButtonElement>(REMIX_BTN_SELECTOR)
    ...
    return {
      before,
      blockedWhileFull,
      resolvedAfterFree: resolved,
      after: getInFlightClipCount(),
    };
  });
...
test("Waveform 風の Remix 0 DOM は明示的な生成中シグナルだけを in-flight として数える", async ({
  page,
}) => {
  ...
  const remixCount = await page
    .locator('button[aria-label="Remix clip"]')
    .count();
...
test("Grid 風の Remix 0 DOM は signal 欠落 clip 候補を fail-loud にする", async ({
  page,
}) => {
  ...
  await expect(
    page.evaluate(getInstalledInFlightClipCountInPage)
  ).rejects.toThrow(/in-flight 検知シグナル/);
});
...
test("queue 上限 toast 検知中は空きスロットでも待機し、toast 消失で投入を再開する (#847)", async ({
  page,
}) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    const isQueueLimitErrorVisible = (): boolean =>
      Array.from(
        ...
        (el) =>
          ...
          (el.textContent ?? "")
            .toLowerCase()
            .includes("generation in progress")
      );
    const sleep = (ms: number) => new Promise((r) => setTimeout(r, ms));
    ...
    const slotFreeButToastUp =
      getInFlightClipCount() < 20 && isQueueLimitErrorVisible();
    ...
    return {
      slotFreeButToastUp,
      blockedWhileToast,
      resolvedAfterToastGone: resolved,
    };
  });
...
test("abortableSleep は中断フラグで長い待機の途中でも 3 秒以内に抜ける (#847)", async ({
  page,
}) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    const pending = abortableSleep(10000, () => aborted).then(
      () => Date.now() - start
    );
    setTimeout(() => {
