...
// 初期値を最初の ready/downloaded にする」契約を mock DOM へ inline 再現して検証する (#1216)。
...
test("collection ドロップダウンは populate され、needs_prompts は disabled・初期値は最初の有効 entry", async ({
  ...
  const result = await page.evaluate(() => {
    ...
    // pickInitialCollectionId と同ルール: 最初の status !== "needs_prompts"。
    const initialId = collections.find((c) => c.status !== "needs_prompts")?.id ?? "";
