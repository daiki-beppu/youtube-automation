...
test("11 件目は in-flight 上限 (20 clip) で待機し、1 clip 完了で投入が再開する", async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    const isArticleBoundary = (root: HTMLElement): boolean => root.matches("article");
    ...
    const hasAlternateActions = (root: HTMLElement): boolean =>
      isArticleBoundary(root) &&
      (hasExactlyOne(root, REMIX_BTN_SELECTOR) ||
    ...
    const findCardRoot = (anchor: HTMLElement): HTMLElement => {
      ...
      while (node) {
        if (!isDocumentRoot(node) && (hasListActions(node) || hasAlternateActions(node))) {
          return node;
        }
        node = node.parentElement;
      }
    ...
    const collectCardsFromAlternateSignals = (): Set<HTMLElement> => {
      const anchors = document.querySelectorAll<HTMLElement>(
        `${SELECT_CLIP_SELECTOR}, ${DESELECT_CLIP_SELECTOR}, [aria-busy="true"], [role="progressbar"]`,
      );
      const cards = new Set<HTMLElement>();
      for (const anchor of anchors) {
        const card = findArticleCardRoot(anchor);
        if (card && hasAlternateInFlightSignal(card)) cards.add(card);
      }
      return cards;
    };
    ...
    const getInFlightClipCount = (): number => {
      ...
      for (const card of collectCardsFromAlternateSignals()) cards.add(card);
      if (anchors.length === 0 && cards.size === 0) {
        if (detectSunoViewMode() !== "unknown") return 0;
...
test("Waveform/Grid 風の Remix 0 DOM は明示的な生成中シグナルだけを in-flight として数える", async ({ page }) => {
  ...
  const result = await page.evaluate(() => {
    ...
    return {
      count: getInFlightClipCount(),
      remixCount: document.querySelectorAll('button[aria-label="Remix clip"]').length,
    };
  });
