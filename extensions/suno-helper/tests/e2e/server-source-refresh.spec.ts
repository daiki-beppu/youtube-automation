...
test("最初の開操作では停止済み候補を提示せず、検出完了後に更新済み listbox を開く", async () => {
  ...
  try {
    const makeLiveServer = async (label: string): Promise<{ info: Record<string, unknown> }> => {
    ...
    const storedOptions = page.locator('[data-suno-control="server-url"] option');
    await expect(trigger).toContainText("YouTube Automation (default)");
    await expect(storedOptions).toContainText(["YouTube Automation (default)", "Old"]);

    active = newServer;
    delayNextResponse = true;
    await trigger.click();
    await expect(trigger).toBeDisabled();
    await expect(page.getByRole("listbox", { name: "ローカル配信元" })).toHaveCount(0);
