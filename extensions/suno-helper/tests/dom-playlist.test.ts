...
//   - selectRecentClips(count: number): HTMLElement[]  // scroller 不在 / row 0 件で fail-loud throw (#881)
 ...
 * selectRecentClips は button の `closest('.multi-select-button').parentElement`（= per-clip div）
 * を row として導出するため、返り値 `row` はその per-clip div になる。
 ...
 */
function addClipRow(opts: { selectLabel?: string; visible?: boolean } = {}): {
  row: HTMLElement;
...
describe("selectRecentClips: scroller 配下の clip row を per-clip 粒度で先頭から count 件取得", () => {
  it("Given scroller 内 5 row When count=40 Then 5 row を DOM 順で返す", () => {
    const rows = Array.from({ length: 5 }, () => addClipRow().row);

    expect(selectRecentClips(40)).toEqual(rows);
  });

  it("Given 単一中間ラッパ配下に複数 per-clip div When 取得する Then 各 clip を別 row に分離する（1 row に collapse しない）", () => {
    // 実機 (order.md L26) は scroller 直下が単一中間ラッパで、per-clip div はその配下に並ぶ。
    // `:scope > div`（素朴実装）だとこの単一ラッパ 1 件に collapse し、全 clip が 1 row に潰れて
    // multiSelectClips が先頭 1 ボタンしか click できない。ボタン基点の per-clip 導出は分離を保証する。
    const rows = Array.from({ length: 3 }, () => addClipRow().row);

    // fixture が「scroller 直下 = 単一中間ラッパ 1 件」構造であることを明示（collapse の前提条件）。
    const scroller = getOrCreateScroller();
    expect(scroller.querySelectorAll(":scope > div").length).toBe(1);

    const selected = selectRecentClips(40);
    expect(selected).toEqual(rows); // 3 row に分離（collapse なら length 1 で落ちる）
    expect(new Set(selected).size).toBe(3); // 重複排除後も per-clip で 3 件
  });

  it("Given 未選択 (Select clip) と選択済み (Deselect clip) row が混在 When 取得する Then どちらも row として返す", () => {
    // multi-select ボタンの aria-label は Select/Deselect いずれでも clip row とみなす（#881）。
    // 生成中 / 完了で挙動を変えないため、状態を問わず multi-select 対象に含める。
    ...
    expect(selectRecentClips(40)).toEqual([a, b, c]);
  });

  it("Given scroller 直下に multi-select を持たない div が混在 When 取得する Then その div は row 採用しない", () => {
    const clip = addClipRow().row;
    // multi-select ボタンを含まない装飾 div（header / spacer 等）は clip row ではない。
    const decoration = document.createElement("div");
    markBbox(decoration, 200, 20);
    getOrCreateScroller().appendChild(decoration);

    expect(selectRecentClips(40)).toEqual([clip]);
  });

  it("Given scroller 内 5 row When count=3 Then 先頭 3 row だけ返す", () => {
    const rows = Array.from({ length: 5 }, () => addClipRow().row);

    expect(selectRecentClips(3)).toEqual(rows.slice(0, 3));
  });

  it("Given 非可視 row が混在 When 取得する Then strict isVisible で除外する", () => {
    const visible = addClipRow().row;
    addClipRow({ visible: false });

    expect(selectRecentClips(40)).toEqual([visible]);
  });

  it("Given .clip-browser-list-scroller が存在しない When 取得する Then fail-loud で throw する", () => {
    // scroller 不在 = Suno の clip list コンテナ自体が変わった。空配列で素通しせず即 throw（#881）。
    expect(() => selectRecentClips(40)).toThrow(/clip row が見つかりません/);
  });

  it("Given scroller はあるが clip row が 0 件 When 取得する Then fail-loud で throw する", () => {
    // scroller は健在だが multi-select を持つ row が 1 件も無い = selector 廃止等の UI 変更。
    // 空配列を返すと後段 multiSelectClips([]) が silent resolve するため、検出境界で throw（#881）。
    getOrCreateScroller();
...
describe("multiSelectClips: click + selected 状態への遷移を verify", () => {
  ...
  it("Given 空の rows When multiSelectClips Then 内部不変条件違反として throw する（0>=0 の silent resolve 防止）", async () => {
    // selectRecentClips が row 0 件で先に throw する前提だが、万一 [] で到達したら silent resolve させない（#881）。
