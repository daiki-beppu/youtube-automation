...
//   - clip-row は `[data-testid="clip-row"]` で識別（生成中含め status は問わない、#862 で status フィルタ撤去）
...
//   - CLIP_ROW_SELECTOR: string = '[data-testid="clip-row"]'
...
//   - selectRecentClips(count: number): HTMLElement[]
//   - multiSelectClips(rows: HTMLElement[]): Promise<void>  // click 後 selected 遷移を poll verify、不在/不遷移で fail-loud (#878)
...
describe("playlist-dom セレクタ定数: 実機 DOM 検証で確定した安定識別子", () => {
  it("Given CLIP_ROW_SELECTOR When 読む Then 生成中含む clip-row 全般の data 属性セレクタである", () => {
...
describe("selectRecentClips: clip-row を先頭から count 件取得（生成中含む）", () => {
  ...
  it("Given 完了 3 + streaming 2 When 取得する Then complete と streaming を区別せず 5 件返す（status フィルタなし）", () => {
    // #859 修正: data-clip-status="complete" フィルタを外し、生成中 clip も multi-select 対象に含めることで、
    // 生成完了マーク反映ラグによる 0 件選択を防ぐ。playlist 追加自体は生成中でも可能で、未完了分は Suno 側で
    // 生成完了時に playlist へ自動反映される。
    const c1 = addClipRow({ status: "complete" }).row;
    const s1 = addClipRow({ status: "streaming" }).row;
    const c2 = addClipRow({ status: "complete" }).row;
    const s2 = addClipRow({ status: "streaming" }).row;
    const c3 = addClipRow({ status: "complete" }).row;

    expect(selectRecentClips(40)).toEqual([c1, s1, c2, s2, c3]);
  });

  it("Given 全 row が streaming のみ（生成完了マーク未付与） When 取得する Then 0 件にならず全件返す", () => {
    const rows = Array.from({ length: 3 }, () => addClipRow({ status: "streaming" }).row);
  ...
  it("Given clip-row が無い When 取得する Then 空配列を返す", () => {
    expect(selectRecentClips(40)).toEqual([]);
  });
});
