...
describe("extractPlaylistName: 正常系（<channel> | <theme> 形式で導出）", () => {
  it("Given 単一 word channel + 複数 word theme When 抽出する Then `rjn | dawn-cloud-fold` を返す", () => {
    expect(extractPlaylistName("20260601-rjn-dawn-cloud-fold-collection", "dawn-cloud-fold")).toBe(
      "rjn | dawn-cloud-fold",
    );
  });

  it("Given 複数 word channel (soulful-grooves) + 複数 word theme When 抽出する Then channel もハイフン保持して `|` で分離する", () => {
    expect(extractPlaylistName("20260520-soulful-grooves-midnight-mood-collection", "midnight-mood")).toBe(
      "soulful-grooves | midnight-mood",
    );
  });

  it("Given `-collection` 無しの id When 抽出する Then 接尾辞剥がし無しで同じ結果を返す", () => {
    expect(extractPlaylistName("20260601-rjn-dawn-cloud-fold", "dawn-cloud-fold")).toBe("rjn | dawn-cloud-fold");
  });
...
describe("extractPlaylistName: 異常系（fail-loud で throw）", () => {
  it("Given 空の theme When 抽出する Then throw する", () => {
    expect(() => extractPlaylistName("20260601-rjn-dawn-cloud-fold-collection", "")).toThrow();
  });

  it("Given id が theme と不整合 When 抽出する Then throw する（末尾照合失敗）", () => {
    // theme が "dawn-cloud-fold" のはずなのに id 末尾が "wrong-theme" 系
    expect(() => extractPlaylistName("20260601-rjn-other-theme-collection", "dawn-cloud-fold")).toThrow(
      /theme と collection id/,
    );
  });

  it("Given 先頭が 8 桁日付でない id When 抽出する Then throw する（日付なし）", () => {
    expect(() => extractPlaylistName("rjn-dawn-cloud-fold-collection", "dawn-cloud-fold")).toThrow();
  });
  ...
  it("Given theme 剥がし後に channel が空 When 抽出する Then throw する", () => {
    ...
    expect(() => extractPlaylistName("20260601-dawn-cloud-fold-collection", "dawn-cloud-fold")).toThrow();
  });

  it("Given 日付が 8 桁でない (7 桁) id When 抽出する Then throw する（境界の桁数厳密）", () => {
    expect(() => extractPlaylistName("2026060-rjn-dawn-cloud", "dawn-cloud")).toThrow();
  });

  it("Given 日付に英字混入の id When 抽出する Then throw する（^\\d{8}$ の数値限定）", () => {
    expect(() => extractPlaylistName("2026060a-rjn-dawn-cloud", "dawn-cloud")).toThrow();
  });
