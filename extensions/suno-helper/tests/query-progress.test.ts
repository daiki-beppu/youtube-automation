...
describe("initSnapshot: 連続実行開始時の初期スナップショット", () => {
  ...
  it("Given option 未指定 When initSnapshot Then 異常値再生成を ON で保持する", () => {
    expect(
      initSnapshot(makePromptEntries(1), snapshotOptions())
...
describe("nextItemStates: itemStates の遷移ロジック (useSunoRunner live handler と同一)", () => {
  ...
  it("Given 全 idle When INJECTING index=0 Then index を active にする", () => {
    expect(
      nextItemStates(base, { phase: PHASE.INJECTING, index: 0, total: 3 })
    ).toEqual(["active", "idle", "idle"]);
  });

  it("Given 直前 active が残る When 別 index を INJECTING Then 旧 active は idle へ戻す", () => {
    const prev = nextItemStates(base, {
  ...
  it("Given done を含む When 別 index を INJECTING Then done は維持し active のみ移す", () => {
    ...
    expect(
      nextItemStates(done0, { phase: PHASE.INJECTING, index: 1, total: 3 })
    ).toEqual(["done", "active", "idle"]);
  });

  it("Given queue ACK 後 When SUBMITTED index=0 Then index を submitted にし DONE とは区別する", () => {
    expect(
      nextItemStates(base, { phase: PHASE.SUBMITTED, index: 0, total: 3 })
    ).toEqual(["submitted", "idle", "idle"]);
  });

  it("Given 全 idle When DONE index=0 Then index を done にする", () => {
    expect(
      nextItemStates(base, { phase: PHASE.DONE, index: 0, total: 3 })
    ).toEqual(["done", "idle", "idle"]);
  });
  ...
  it("Given prev When nextItemStates Then 新しい配列を返す（React state 更新のため非破壊）", () => {
    const result = nextItemStates(base, {
      phase: PHASE.INJECTING,
      index: 0,
      total: 3,
    });
...
describe("applyProgress: progress 受信でスナップショットを更新する", () => {
  ...
  it("Given 実行中 snap When FINISHED を適用 Then entries / itemStates を破棄せず保持する（再 open 表示用）", () => {
    const entries = makePromptEntries(2);
    const running = applyProgress(initSnapshot(entries, snapshotOptions()), {
      phase: PHASE.DONE,
      index: 0,
      total: 2,
    });

    const finished = applyProgress(running, {
