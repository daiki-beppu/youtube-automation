...
describe("content.ts: STOPPED phase は resume state を保存する (#898 要件1/2/3/7)", () => {
  ...
  it("Given content.ts When PHASE.STOPPED emit を数える Then 正確に 8 箇所（download flow は performDownload に集約、retryPlaylist 1 + retryDownload 1 を含む）", () => {
  ...
  it("Given playlist / download phase STOPPED 3 箇所 When 直前を読む Then persistInterruptState(total) が隣接する（全 entry done 後 + download 中断, 要件7b, #1146 → performDownload 集約で 4→3）", () => {
    const playlistStops =
