...
describe("content.ts: STOPPED phase は resume state を保存する (#898 要件1/2/3/7)", () => {
  ...
  it("Given ループ内 STOPPED のうち未 click 箇所 When 直前を読む Then persistInterruptState(i) が隣接する（ループ先頭 / waitForQueueSlot 後の 2 箇所, #924 で 3→2 に変更）", () => {
    // #924 修正: injectWithVerification 後の STOPPED は interruptIndex（resolveInterruptIndex 補正済み）を使う。
    // ループ先頭 / waitForQueueSlot 後はまだ Generate を click していないため i のまま (#924)。
    const loopStops =
      contentSource.match(
        /persistInterruptState\(i\);\s*emitProgress\(\{ phase: PHASE\.STOPPED, index: i, total \}\)/g,
      ) ?? [];

    expect(loopStops).toHaveLength(2);
  });
