...
describe("URL 判定: 観測対象 endpoint の識別", () => {
  ...
  it("Given 別オリジンの同パス URL When 判定する Then すべて false（オリジン縛り）", () => {
    ...
    expect(
      isFeedRequest(`https://evil.example.com/api/feed/v3`, FEED_V3_METHOD)
    ).toBe(false);
  });
  ...
  it("Given Suno origin に似た別 origin When 判定する Then すべて false", () => {
    ...
    expect(
      isFeedRequest(
        `https://studio-api-prod.suno.com.evil.example${FEED_V3_PATH}`,
        FEED_V3_METHOD
      )
    ).toBe(false);
  });
  ...
  it("Given feed/v3 に似た path や query 内一致 When 判定する Then isFeedRequest は false", () => {
    expect(
      isFeedRequest(`${SUNO_API_ORIGIN}/api/feed/v30`, FEED_V3_METHOD)
    ).toBe(false);
    expect(
      isFeedRequest(`${SUNO_API_ORIGIN}/api/feed/v3extra`, FEED_V3_METHOD)
    ).toBe(false);
    expect(
      isFeedRequest(
        `${SUNO_API_ORIGIN}/api/not-feed?next=${encodeURIComponent(FEED_V3_PATH)}`,
        FEED_V3_METHOD
      )
    ).toBe(false);
  });
  ...
  it("Given generate endpoint に似た path や query 内一致 When 判定する Then isGenerateRequest は false", () => {
    expect(
      isGenerateRequest(`${SUNO_API_ORIGIN}${GENERATE_ENDPOINT_PATH}extra`)
    ).toBe(false);
    expect(
      isGenerateRequest(
        `${SUNO_API_ORIGIN}/api/not-generate?next=${encodeURIComponent(GENERATE_ENDPOINT_PATH)}`
      )
    ).toBe(false);
  });
});
...
describe("resolveRequestMethod: fetch の実効 method 解決", () => {
  ...
  it("Given Request.method When init.method 不在 Then Request 側の method を返す", () => {
    expect(
      resolveRequestMethod(new Request(FEED_V3_URL, { method: "POST" }))
    ).toBe("POST");
  });
...
describe("extractAuthHeader: Authorization の抽出", () => {
  it("Given record 形式の headers When 抽出する Then 値を返す（大文字小文字非依存）", () => {
    expect(
      extractAuthHeader(GENERATE_URL, {
  ...
  it("Given Headers / 配列形式 When 抽出する Then 値を返す", () => {
    expect(
      extractAuthHeader(GENERATE_URL, {
        headers: new Headers({ authorization: "Bearer h" }),
      })
    ).toBe("Bearer h");
    expect(
      extractAuthHeader(GENERATE_URL, {
  ...
  it("Given Request オブジェクトに headers When init 不在 Then Request 側から抽出する", () => {
    const req = new Request(GENERATE_URL, {
...
describe("parseClipsFromGenerateResponse: 生成投入レスポンスの解析 (fail-soft)", () => {
  ...
  it("Given 形が崩れた JSON When 解析する Then null（throw しない）", () => {
    ...
    expect(
      parseClipsFromGenerateResponse({ clips: [{ id: 1, status: 2 }] })
    ).toBeNull();
    expect(
      parseClipsFromGenerateResponse({
        clips: [{ id: "c1", status: "submitted", duration: "241.2" }],
      })
    ).toBeNull();
  });
});
...
describe("parseClipsFromFeedResponse: feed レスポンスの解析 (両形対応・fail-soft)", () => {
  it("Given {clips: [...]} 形 When 解析する Then ObservedClip[] を返す", () => {
    expect(
      parseClipsFromFeedResponse({
        clips: [{ id: "c1", status: "streaming", duration: 182.4 }],
      })
    ).toEqual([{ id: "c1", status: "streaming", duration: 182.4 }]);
  });
  ...
  it("Given 素の配列形 When 解析する Then ObservedClip[] を返す", () => {
    ...
    ).toEqual([{ id: "c1", status: "complete", duration: 0 }]);
  });
  ...
  it("Given 形が崩れた JSON When 解析する Then null（throw しない）", () => {
    ...
    expect(
      parseClipsFromFeedResponse([
        { id: "c1", status: "complete", duration: "241.2" },
      ])
    ).toBeNull();
    expect(
      parseClipsFromFeedResponse([
        { id: "c1", status: "complete", duration: -1 },
      ])
    ).toBeNull();
  });
});
