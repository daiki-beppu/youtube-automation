...
describe("Suno popup compatibility check", () => {
  ...
  it("完全自動生成の FINISHED snapshot をリロード後に復元して同じ clip ID で Download を再開する", async () => {
    const autoRunClipIds = ["auto-clip-a", "auto-clip-b"];
    messagingMocks.sendMessage.mockImplementation(
      (message: string, payload?: Record<string, string>) => {
        if (message === "queryProgress") {
          return Promise.resolve({
            ...
            itemStates: ["done"],
            isRunning: false,
            progress: { phase: PHASE.FINISHED, total: 1 },
            playlistName: "clm | theme-a",
