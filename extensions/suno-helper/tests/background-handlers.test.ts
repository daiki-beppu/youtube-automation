...
async function loadBackground(opts?: {
    ...
    search: vi.fn(
      (
        ...
        const configuredResults =
          opts?.searchResultsById?.[query.id] ?? opts?.searchResults;
  ...
  vi.stubGlobal("chrome", {
    ...
    storage: {
      session: {
        get: vi.fn(
          (key: string, cb: (items: Record<string, unknown>) => void) => {
            if (typeof opts?.sessionGetDelayMs === "number") {
              setTimeout(
                () => cb({ [key]: sessionStore[key] }),
                opts.sessionGetDelayMs
              );
              return;
            }
            cb({ [key]: sessionStore[key] });
  ...
  vi.doMock("../lib/overlay-relay", () => ({
    ...
    requireRelayTab: vi.fn(
      (sender: { tab?: { id?: number } }, action: string) => {
...
describe("background read API handlers: localhost read を extension origin 境界へ委譲する", () => {
  ...
  it("collections/collection prompts/version を background fetch で取得する", async () => {
    ...
    expect(fetchImpl).toHaveBeenNthCalledWith(
      1,
      "http://localhost:7873/collections"
    );
    expect(fetchImpl).toHaveBeenNthCalledWith(
      2,
      "http://localhost:7873/collections/20260601-clm/suno/prompts.json"
    );
    expect(fetchImpl).toHaveBeenNthCalledWith(
      3,
      "http://localhost:7873/version"
    );
  });
...
describe('background onMessage("discoverServerSources"): shared discovery boundary', () => {
  ...
  it("should expose the shared discovery client to the overlay through a typed handler", async () => {
    ...
    await expect(
      handlers.get("discoverServerSources")!({
        data: {},
        sender: { tab: { id: 42 } },
      })
    ).resolves.toEqual([
  ...
  it("should return a rejected handler promise when shared discovery fails", async () => {
    const { handlers, discoverServerSourcesMock } = await loadBackground();
    discoverServerSourcesMock.mockRejectedValueOnce(
...
describe('background onMessage("startDownload"): 非 .zip ダウンロードは無視する', () => {
  ...
  it("Given .mp3 ダウンロード完了 When listener 発火 Then downloadComplete を送信しない", async () => {
    const {
      ...
      searchResults: [
        { filename: "track.mp3", startTime: new Date().toISOString() },
      ],
    });
    ...
    expect(
      sentMessages.filter((m) => m.type === "downloadComplete")
    ).toHaveLength(0);
...
describe('background onMessage("startDownload"): 監視開始前の .zip は無視する (#1217)', () => {
  ...
  it("Given 古い ZIP 後に新しい ZIP が完了 When listener 発火 Then 新しい ZIP だけ完了通知する", async () => {
    ...
    const listener = downloadListeners[0];
    createdListeners[0](
      freshZip(7, { filename: "old.zip", startTime: oldStart })
    );
    listener({ id: 7, state: { current: "complete" } });
    await flushPromises();
    expect(removedDownloadListeners).toHaveLength(0);
    ...
    createdListeners[0](
      freshZip(8, { filename: "new.zip", startTime: freshStart })
    );
  ...
  it("Given onCreated を取り逃した fresh Suno ZIP When complete event だけ届く Then 対象確定して完了通知する", async () => {
    const freshStart = new Date().toISOString();
    const { handlers, sentMessages, downloadListeners, sessionStore } =
      await loadBackground({
        searchResultsById: {
          33: [
            {
              filename: "missed-created.zip",
              startTime: freshStart,
              url: "https://suno.com/api/download/zip",
            },
          ],
...
describe("background downloads listener: service worker restart 後も session watcher を復元する (#1217)", () => {
  ...
  it("Given 保存済み watcher When background が再初期化され complete event を受ける Then downloadComplete を中継する", async () => {
    const freshStart = new Date().toISOString();
    const { sentMessages, downloadListeners, sessionStore } =
      await loadBackground({
        sessionState: {
          tabId: 42,
...
describe('background onMessage("startDownload"): 同時監視を排他する (#1217)', () => {
  ...
  it("Given 監視中 When 別タブが startDownload Then 新しい監視を作らず失敗通知する", async () => {
    const { handlers, sentMessages, downloadListeners } =
      await loadBackground();
    ...
    expect(result).toEqual({
      ok: false,
      message: expect.stringContaining("監視が進行中"),
    });
    expect(
      sentMessages.filter((m) => m.type === "downloadFailed")
    ).toHaveLength(0);
...
describe('background onMessage("cancelDownload"): active watcher を解除する (#1217)', () => {
  ...
  it("Given startDownload 済み When 別タブから cancelDownload Then watcher を維持し次の startDownload を拒否する", async () => {
    const { handlers, downloadListeners, removedDownloadListeners } =
      await loadBackground();
