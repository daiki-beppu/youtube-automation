...
test("トラック数 select でアルバム構造になり、SELECT/file 注入が成立する (#888)", async ({
  ...
  const uuids = await page.evaluate(() =>
    Array.from(
      document.querySelectorAll<HTMLInputElement>('[name^="title_"]')
  ...
  await page.evaluate(() => {
    ...
    dt.items.add(file);
    const upload =
      document.querySelector<HTMLInputElement>("#js-track-upload-1")!;
...
test("AI 開示 modal は full check で persona radio を dynamic inject し、保存で全 track へ伝播する (#888)", async ({
  ...
  await modal
    .locator('.distroAiRecordingScope[track="1"][value="full"]')
    .check();
...
test("Apple Music「クレジットを追加」click で全 track の credit 入力欄が visible 化し、注入できる (#888)", async ({
  ...
  for (const n of [1, 2]) {
    await expect(page.locator(`#track-${n}-performer-1-name`)).toHaveValue(
      artist
    );
    await expect(page.locator(`#track-${n}-producer-1-name`)).toHaveValue(
      artist
    );
  }
