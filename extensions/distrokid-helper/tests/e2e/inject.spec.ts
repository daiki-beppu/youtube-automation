...
async function setNativeValue(page: import("@playwright/test").Page, selector: string, value: string): Promise<void> {
  await page.evaluate(
    ({ selector, value }) => {
...
test("トラック数 select でアルバム構造になり、SELECT/file 注入が成立する (#888)", async ({ page }) => {
  ...
  const uuids = await page.evaluate(() =>
    Array.from(document.querySelectorAll<HTMLInputElement>('[name^="title_"]')).map((el) => el.getAttribute("name")),
  );
  ...
  const noChecked = await page.evaluate(
    () => document.querySelector<HTMLInputElement>('[name="previouslyReleased_uuid-track-1"][value="0"]')?.checked,
  );
  ...
  await expect(page.locator('[name="songwriter_real_name_first1"]')).toHaveValue("Jane");
  const songFileName = await page.evaluate(
    () => document.querySelector<HTMLInputElement>("#js-track-upload-1")!.files?.[0]?.name,
  );
  expect(songFileName).toBe("track-01.mp3");
  const changed = await page.evaluate(() => (window as unknown as { __changed: string[] }).__changed);
...
test("AI 開示 modal は full check で persona radio を dynamic inject し、保存で全 track へ伝播する (#888)", async ({
  ...
  await expect(modal.locator('.distroAiRecordingScope[track="1"][value="full"]')).toHaveCount(1);
  ...
  await expect(page.locator(".ai-credits-swal-modal")).toHaveCount(0);
  await expect(page.locator('[name="ai_gate_uuid-track-1"][value="1"]')).toBeChecked();
  await expect(page.locator('[name="ai_gate_uuid-track-2"][value="1"]')).toBeChecked();
...
test("配信先ストア check（除外 2 ストア uncheck 保証）+ upsell uncheck + credits 可視化 + areyousure check (#923 / #928)", async ({
  ...
  const allStoreIds = await page.evaluate(() =>
    Array.from(document.querySelectorAll<HTMLInputElement>('input[type="checkbox"][name="store"][id^="chk"]')).map(
      (cb) => cb.id,
    ),
  );
  ...
  await page.evaluate(() => {
    document
      .querySelectorAll<HTMLInputElement>('input[type="checkbox"][name="store"]:not([id^="chk"])')
      .forEach((cb) => {
        if (cb.checked) cb.click();
      });
    document.querySelectorAll<HTMLInputElement>('input[type="checkbox"][name="extras"]').forEach((cb) => {
      if (cb.checked) cb.click();
    });
  });

  // Then: shazam / audiomack は unchecked、配信先は維持
  const shazamChecked = await page.evaluate(
    () => document.querySelector<HTMLInputElement>('input[name="store"][value="shazam"]')?.checked,
  );
...
test("release_date（YYYY-MM-DD）を #release-date-dp に注入できる（#932）", async ({ page }) => {
...
test("checkAllStores は事前 checked の chksnap / chkroblox を uncheck する（#928 デフォルト全 check からの除外保証）", async ({
  ...
  await page.evaluate(() => {
    ...
    document.querySelectorAll<HTMLInputElement>('input[type="checkbox"][name="store"][id^="chk"]').forEach((cb) => {
      cb.checked = true;
    });
  });
