...
function buildModal(uuid: string, opts: { album: boolean }): ModalRefs {
  ...
  const lyrics = withClass(
    makeCheckbox(details, { name: `ai_lyrics_${uuid}` }),
    "distroAiLyrics"
  );
  const music = withClass(
    makeCheckbox(details, { name: `ai_music_${uuid}` }),
    "distroAiMusic"
  );
  ...
  scopeFull.addEventListener("change", () => {
    ...
    if (personaContainer.querySelector('[name^="ai_artist_persona_"]') !== null)
      return;
    setTimeout(() => {
      if (
        personaContainer.querySelector('[name^="ai_artist_persona_"]') !== null
      )
        return;
...
const SAMPLE_SONGWRITER: SongwriterName = {
  first: "Jane",
  middle: null,
  last: "Doe",
};
...
describe("TRACK_COUNT_SELECTOR / APPLE_CREDIT_SELECTORS（#888 実 DOM 準拠）", () => {
  ...
  it("performer/producer の name / role は #track-{N}-{role}-1-{kind}（1-indexed）を返す", () => {
    ...
    expect(APPLE_CREDIT_SELECTORS.performerNameByTrack(3)).toBe(
      "#track-3-performer-1-name"
    );
    expect(APPLE_CREDIT_SELECTORS.performerRoleByTrack(3)).toBe(
      "#track-3-performer-1-role"
    );
    expect(APPLE_CREDIT_SELECTORS.producerNameByTrack(3)).toBe(
      "#track-3-producer-1-name"
    );
    expect(APPLE_CREDIT_SELECTORS.producerRoleByTrack(3)).toBe(
      "#track-3-producer-1-role"
    );
  });
});
...
describe("injectProfile（language/main_genre 必須・sub_genre 任意・isVisible 排除）", () => {
  ...
  it("main_genre の change 後に sub_genre select 自体が mount される場合も待って選択する", async () => {
    ...
    // Then
    expect(
      document.querySelector<HTMLSelectElement>("#subGenrePrimary")!.value
    ).toBe("53");
  });
...
describe("AI_DISCLOSURE_SELECTORS / AI_MODAL_SELECTORS（実 DOM 準拠 #877 / #888）", () => {
  ...
  it("modal / save / apply-all は実 DOM の固定セレクタを指す", () => {
    ...
    expect(AI_MODAL_SELECTORS.saveButton).toBe(
      "button.swal2-confirm.ai-modal-btn-save"
    );
    expect(AI_MODAL_SELECTORS.applyAll).toBe("#ai-apply-all-1");
  });

  it("lyrics / music は modal を開いた track の uuid で selector を返す", () => {
    expect(AI_MODAL_SELECTORS.lyricsByUuid("abc")).toBe(
      '[name="ai_lyrics_abc"]'
    );
  ...
  it("partialAudioTypeByUuid は vocals / instruments radio selector を返す", () => {
    ...
    expect(
      AI_MODAL_SELECTORS.partialAudioTypeByUuid("abc", "instruments")
    ).toBe('[name="ai_partial_audio_type_abc"][value="instruments"]');
  });
...
describe("waitForElement / waitForRemoval（MutationObserver ベース）", () => {
  it("既に存在する要素は即解決する", async () => {
    ...
    await expect(waitForElement(document, "#already-here", 1000)).resolves.toBe(
      el
    );
  });
...
describe("waitForElementCount（#888 track 行生成待ち）", () => {
  it("既に count 個あれば即解決する", async () => {
    ...
    await expect(
      waitForElementCount(document, '[name^="title_"]', 2, 1000)
  ...
  it("制限時間内に達しなければ TrackCountTimeoutError で fail-loud", async () => {
    mountInput({ name: "title_a" });
    await expect(
      waitForElementCount(document, '[name^="title_"]', 3, 20)
    ).rejects.toBeInstanceOf(TrackCountTimeoutError);
  });
});
...
describe("setTrackCount（#888 トラック数 select + 行生成待機）", () => {
  ...
  it("select が無ければ FieldNotFoundError で fail-loud", async () => {
    await expect(setTrackCount(document, 2)).rejects.toBeInstanceOf(
      FieldNotFoundError
    );
  });
});
...
describe("injectAppleMusicCredits（#888 / #919 Apple Music クレジット・role 含む）", () => {
  ...
  it("trigger を 1 回 click し、全 track の name / role を profile.artist で注入する", async () => {
    // Given
    const { getTriggerClicks } = mountCreditDom(3, {
      artist: "Soulful Grooves",
    });
    ...
    for (let n = 1; n <= 3; n += 1) {
      expect(
        document.querySelector<HTMLInputElement>(
          `#track-${n}-performer-1-name`
        )!.value
      ).toBe("ABYSS MI");
      expect(
        document.querySelector<HTMLSelectElement>(
          `#track-${n}-performer-1-role`
        )!.value
      ).toBe("Audio");
      expect(
        document.querySelector<HTMLInputElement>(`#track-${n}-producer-1-name`)!
          .value
      ).toBe("ABYSS MI");
      expect(
        document.querySelector<HTMLSelectElement>(
          `#track-${n}-producer-1-role`
        )!.value
      ).toBe("Producer");
    }
  });
  ...
  it("role select の change event が dispatch される（独自 UI 同期のため）", async () => {
    ...
    const perfRole = document.querySelector<HTMLSelectElement>(
      "#track-1-performer-1-role"
    )!;
    const prodRole = document.querySelector<HTMLSelectElement>(
  ...
  it("trigger が無ければ FieldNotFoundError", async () => {
    ...
    // Then: async function → rejected promise
    await expect(
      injectAppleMusicCredits(document, 1, "X", SAMPLE_CREDITS)
    ).rejects.toBeInstanceOf(FieldNotFoundError);
  });
  ...
  it("performer name 欄が無ければ FieldNotFoundError", async () => {
    ...
    document.body.appendChild(trigger);
    mountInput({ id: "track-1-producer-1-name", name: "producer-name" });
    mountSelectWithOptions("track-1-producer-1-role", [
      { value: "Producer", text: "プロデューサー" },
    ]);
  ...
  it("role が option に無ければ OptionNotFoundError", async () => {
    ...
    await expect(
      injectAppleMusicCredits(document, 1, "X", {
        performer_role: "NonExistent",
        producer_role: "Producer",
      })
...
describe("uncheckUpsells（#919 オプション強制 $0 保証）", () => {
  ...
  it("セレクタ定数は実機 DOM の name 属性を保証する（#923 chk* 除外版）", () => {
    expect(UPSELL_SELECTORS.store).toBe(
      'input[type="checkbox"][name="store"]:not([id^="chk"])'
    );
    expect(UPSELL_SELECTORS.extras).toBe(
...
describe("checkAllStores（#923 / #928 配信先ストア check・除外 2 ストア uncheck 保証）", () => {
  it("除外 2 ストア（chksnap / chkroblox）以外の chk* を check する", () => {
    // Given
    const cb1 = makeCheckbox(document.body, {
      id: "chkspotify",
      name: "store",
    });
    const cb2 = makeCheckbox(document.body, {
      id: "chkapplemusic",
      name: "store",
    });
    const cbSnap = makeCheckbox(document.body, {
      id: "chksnap",
      name: "store",
    });
    const cbRoblox = makeCheckbox(document.body, {
      id: "chkroblox",
      name: "store",
    });
  ...
  it("chksnap / chkroblox が事前に checked の場合は uncheck される（DistroKid デフォルト全 check からの配信外保証）", () => {
    ...
    const cbSpotify = makeCheckbox(document.body, {
      id: "chkspotify",
      name: "store",
    });
  ...
  it("chk* id なし（shazam / audiomack）は check しない", () => {
    ...
    const real = makeCheckbox(document.body, {
      id: "chkspotify",
      name: "store",
    });
...
describe("uncheckUpsells（#923 chk* 配信先を巻き込まない回帰）", () => {
  it("chk* id を持つ store は uncheck しない（#923 回帰防止）", () => {
    ...
    const spotify = makeCheckbox(document.body, {
      id: "chkspotify",
      name: "store",
    });
...
describe("VisibilityTimeoutError / waitForElementVisible（#923）", () => {
  ...
  it("制限時間内に可視化されなければ VisibilityTimeoutError", async () => {
    ...
    // Then
    await expect(waitForElementVisible(el, 20)).rejects.toBeInstanceOf(
      VisibilityTimeoutError
    );
  });
  ...
  it("RELOAD_GUIDANCE が ModalTimeoutError / TrackCountTimeoutError / VisibilityTimeoutError の message に含まれる", () => {
    ...
    expect(new TrackCountTimeoutError("#foo", 2).message).toContain(
      RELOAD_GUIDANCE
    );
    expect(new VisibilityTimeoutError("#foo").message).toContain(
      RELOAD_GUIDANCE
    );
  });
});
...
describe("injectAiDisclosure（#877 / #888 modal フロー・段階的 trigger・async）", () => {
  it("title input (track) が無ければ FieldNotFoundError（uuid 解決不可）", async () => {
    await expect(
      injectAiDisclosure(document, SAMPLE_AI)
    ).rejects.toBeInstanceOf(FieldNotFoundError);
  });
  ...
  it("modal 内に保存ボタンが無ければ FieldNotFoundError で fail-loud", async () => {
    ...
    yes.addEventListener("click", () => {
      setTimeout(() => {
        ...
        const details = document.createElement("div");
        withClass(
          makeCheckbox(details, { name: "ai_lyrics_uuid-a" }),
          "distroAiLyrics"
        );
        withClass(
          makeCheckbox(details, { name: "ai_music_uuid-a" }),
          "distroAiMusic"
        );
