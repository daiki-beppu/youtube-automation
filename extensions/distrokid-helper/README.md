...
```bash
npx -y pnpm@9.15.9 install --frozen-lockfile --ignore-workspace
```
...
```bash
npx -y pnpm@9.15.9 build  # .output/chrome-mv3/ に Manifest V3 拡張を生成
npx -y pnpm@9.15.9 zip    # 配布用 zip を生成
```

型チェック（WXT 型生成 + tsc）:

```bash
npx -y pnpm@9.15.9 compile
```

## unpacked ロード

1. `npx -y pnpm@9.15.9 build` で `.output/chrome-mv3/` を生成する。
...
```bash
npx -y pnpm@9.15.9 test                              # Vitest（API client / DOM 注入 / messaging / storage）
npx -y pnpm@9.15.9 exec playwright install chromium  # 初回のみ
npx -y pnpm@9.15.9 test:e2e                          # Playwright（distrokid.com/new モックへの注入スモーク）
```
