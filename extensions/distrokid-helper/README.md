...
データ供給源は `tayk collection-serve` の `GET /distrokid/release.json` / `GET /distrokid/assets/<path>`。
...
1. コレクションルートを指定してサーバーを起動:
   ```bash
   tayk collection-serve <collections_root> --distrokid-state-root <channel_root>
   # 例: tayk collection-serve collections/ --distrokid-state-root .
...
リリース日の供給元はコレクションの `workflow-state.json::planning.publish_target_at`。`tayk collection-serve` が ISO 8601 形式（`"2026-03-22T08:00:00+09:00"` のような full datetime、または `"2026-03-22"` のような date のみ）を `YYYY-MM-DD` へ正規化し、`GET /distrokid/release.json` の `release.release_date` として配信する（`publish_target_at` 未設定時は `null` で、注入はスキップされフォームは空のまま）。
...
サーバー（`tayk collection-serve`）は CORS をデフォルトで `chrome-extension://` オリジンと helper サイト web origin（`https://distrokid.com` / `https://www.distrokid.com`）に許可する（#896）。`--allow-origin <origin>` で単一 origin の完全一致に固定もできる。
