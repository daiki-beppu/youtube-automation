...
- **AI 開示**（#866 で実機 DOM 再検証）は **track 単位** で `ai_gate_<uuid>` radio（value=0「いいえ」/ value=1「はい」）と `ai_lyrics_<uuid>` / `ai_music_<uuid>` checkbox、`ai_partial_audio_type_<uuid>` radio（value=vocals/instruments）が並ぶ。同じ UUID を `title_<uuid>` 等と共有するため、既存の `resolveTrackUuids` で uuid を解決して各 track にループで一括注入する（実 DOM に `apply_to_all` checkbox は存在しないため、ループによる一括適用で代替）。歌詞/作曲 checkbox と partial_audio_type radio は初期状態で `display:none` の親に隠れているが、`el.click()` 自体は React 制御コンポーネントに届くため `MutationObserver` での展開待ちは不要（sync 注入）。100% AI 楽曲（Suno 等）は `partial_audio_type: null` で part 系 radio に触れない。
  - **設計の変更経緯**: PR #813 では `#ai-yes` / `#ai-modal` / `[name^="ai_lyrics_"]` / `#ai-apply-all-1` / `#ai-save` を想定していたが、#866 の実機検証で **モーダルは存在せず inline 展開**、`apply_to_all` checkbox / `save` ボタンも実 DOM に **無い** ことが判明したため、uuid-driven 関数群（`AI_DISCLOSURE_SELECTORS.gateByUuid` / `lyricsByUuid` / `compositionByUuid` / `partialAudioTypeByUuid`）に刷新した。実 DOM が変われば `tests/distrokid-injector.test.ts` と `tests/e2e/inject.spec.ts` のセレクタ固定テストで検知できる。
