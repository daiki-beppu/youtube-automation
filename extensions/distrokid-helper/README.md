...
サーバー（`yt-collection-serve`）は read-only route の CORS をデフォルトで `chrome-extension://` オリジンと helper サイト web origin（`https://distrokid.com` / `https://www.distrokid.com`）に許可する（#896）。配信済み記録を書き込む `POST /distrokid/releases` は、DistroKid page origin からの書き換えを防ぐため、`--allow-origin chrome-extension://<EXTENSION_ID>` で拡張 origin へ明示 lock した場合だけ有効になる。さらに `X-Serve-Token` ヘッダが必須（#1360）: popup は直接 POST せず background service worker へ委譲し、background が `GET /auth/token`（要 extension lock）で取得した serve token を付けて POST する。token が stale（サーバー再起動後）で 403 になった場合は token を再取得して 1 回だけ retry する（suno-helper の downloaded POST と同じ書き込み境界）。
