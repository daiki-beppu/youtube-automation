...
export function durationFilterOrDefault(
...
/** prompts.json を取得して collection 単位 metadata を含む PromptResponse を返す (#1259)。 */
export async function fetchPromptResponse(
  baseUrl: string,
): Promise<PromptResponse> {
  return fetchPromptResponseBody(`${baseUrl}${PROMPTS_ROUTE}`);
}
...
export async function fetchCollectionPromptResponse(
  ...
  return fetchPromptResponseBody(`${baseUrl}${collectionPromptsRoute(id)}`);
}
