      ...
      in
      {
        ...
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            ...
            # （入場をブロックしない）。fail-closed に同期したい場合は
            # `nix develop --command uv sync` を明示実行する。
            # 品質ゲート（ruff / CHANGELOG / any 型）はローカル git hook ではなく
            # CI で担保する（issue #2534 で lefthook を廃止）
