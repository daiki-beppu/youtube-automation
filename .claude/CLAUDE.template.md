...
| **Fail Fast** | 設定ミスや欠損データはエラーで早期に止める。フォールバックで握りつぶさない（`infrastructure/errors.py` のドメイン例外を使う） |
...
| スキル実行中の不具合・摩擦・改善案に遭遇した | `/feedback` を案内し、`data/feedback/feedback-log.jsonl` に構造化記録する |
...
> **使い分けに迷ったら → [`docs/workflow-cheatsheet.md`](../docs/workflow-cheatsheet.md)**（判定フロー / `workflow-state.json` を手で触ってよいかの基準 / 3 フェーズ遷移図）。全 skill のカタログは [`docs/features.md`](../docs/features.md) を参照。

### 3.2 音源生成

| コマンド | 用途 | 経路 |
|---|---|---|
| `/lyria` | Vertex AI Lyria 3 で API 自動生成（プロンプト + API 入力パラメータ設計） | API → WAV 直接出力 |
| `/suno` | SunoAI V5.5 向けプロンプト生成（ボーカルは `/suno-lyric` で歌詞生成）→ Suno UI で人手生成 → `/masterup` でクロスフェード | UI 経由 → MP3 → マスター化 |
...
| `/feedback` | スキル実行中の不具合・摩擦・改善案を `data/feedback/feedback-log.jsonl` に構造化記録 |
| `yt-stock-list` / `yt-stock-preview` / `yt-stock-prune` | `assets/stock/<theme>/` のボツ画像一覧・プレビュー・整理（`/thumbnail` と `/collection-ideate` のクリーンアップで自動退避された画像） |

### 3.7 長時間処理時の挙動（共通プロトコル）

ffmpeg / Veo / Lyria / 画像生成 API を呼ぶスキル（`/videoup` / `/masterup` / `/loop-video` / `/lyria` / `/short` / `/short-release` / `/short-thumbnail` / `/thumbnail`）は、**必ず Bash ツールを `run_in_background=true` で起動し、spawn 直後に「⏳ 処理中。完了まで他の質問にもお答えできます。」テキストを返す**。完了は Claude Code が自動でメッセージ通知するため、`sleep` ループや `until` での自前ポーリングは禁止。cmux 環境下（`$CMUX_WORKSPACE_ID` あり）でのみ `cmux set-status` / `cmux notify` を補助で使う。詳細は各スキルの「長時間処理の取り扱い」セクションを参照。
...
|---|---|---|---|---|
| `lyria` | `/lyria` でプロンプト + API 入力パラメータ設計 | Vertex AI API で自動生成 | API が WAV 直接出力（`/masterup` 不要） | `/videoup` |
...
> **Claude へ**: ユーザーがセッション内で初めて `/wf-*` 系（`/wf-new` `/wf-next` `/wf-status`）または `/collection-ideate` を呼ぼうとした、あるいは「次なに作る？」「制作どこまで進んだ？」「`workflow-state.json` 触っていい？」のような **workflow 起点の質問** をしてきたら、skill 実行に進む前に [`docs/workflow-cheatsheet.md`](../docs/workflow-cheatsheet.md) の判定フローと `workflow-state.json` の扱いを **1 回だけ提示する**（同一セッション内で繰り返し提示しない）。全 skill のカタログを聞かれたら [`docs/features.md`](../docs/features.md) を案内する。
...
- `infrastructure/errors.py` のドメイン例外を使用
