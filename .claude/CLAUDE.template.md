...
> **使い分けに迷ったら → [`docs/workflow-cheatsheet.md`](https://github.com/daiki-beppu/youtube-automation/blob/main/docs/workflow-cheatsheet.md)**（判定フロー / `workflow-state.json` を手で触ってよいかの基準 / 3 フェーズ遷移図）。全 skill のカタログは [`docs/features.md`](https://github.com/daiki-beppu/youtube-automation/blob/main/docs/features.md) を参照。
...
> **Claude へ**: ユーザーがセッション内で初めて `/wf-*` 系（`/wf-new` `/wf-next` `/wf-status`）または `/collection-ideate` を呼ぼうとした、あるいは「次なに作る？」「制作どこまで進んだ？」「`workflow-state.json` 触っていい？」のような **workflow 起点の質問** をしてきたら、skill 実行に進む前に [`docs/workflow-cheatsheet.md`](https://github.com/daiki-beppu/youtube-automation/blob/main/docs/workflow-cheatsheet.md) の判定フローと `workflow-state.json` の扱いを **1 回だけ提示する**（同一セッション内で繰り返し提示しない）。全 skill のカタログを聞かれたら [`docs/features.md`](https://github.com/daiki-beppu/youtube-automation/blob/main/docs/features.md) を案内する。
