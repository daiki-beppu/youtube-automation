...
- **popup 画像探索順**: 絶対パス → `10-assets/<image>` → `<collection-dir>/<image>` → `<channel-root>/<image>`。`<channel-root>` は `CHANNEL_DIR` または canonical `config/channel/youtube.json` の配置先から確定するため、`branding/subscribe-popup.png` を指定すると選択チャンネルのリポジトリ配下の画像を使う。見つからない場合はエラー終了し、popup を欠いた動画を生成しない。
