...
# stream copy は出力尺にほぼ比例する disk I/O 律速。#4757 の実測
# (3h13m の出力を 18分08秒 = 約 10.6 倍速) を基準に、10–15 倍速で見積もる。
STREAM_COPY_FAST_RATE=900
STREAM_COPY_SLOW_RATE=600
# 呼び出し側の foreground timeout は 10 分。disk I/O のブレを考慮し、
# 上限見積もりがその半分に達した時点で background 実行を勧める。
FOREGROUND_TIMEOUT_MINUTES=10

stream_copy_estimate_bounds() {
    awk -v duration="${FULL_VIDEO_DURATION:-0}" \
        -v fast="$STREAM_COPY_FAST_RATE" \
        -v slow="$STREAM_COPY_SLOW_RATE" 'BEGIN{
            lower = int((duration + fast - 1) / fast)
            upper = int((duration + slow - 1) / slow)
            print (lower < 1 ? 1 : lower), (upper < 1 ? 1 : upper)
        }'
}
...
print_full_output_outlook() {
    local route="$1"
    local estimate="$2"
    local stream_copy_upper="${3:-}"
    [[ "$PREVIEW_MODE" -eq 1 ]] || return
    echo "  Full output outlook:"
    echo "    Route   : ${route}"
    echo "    Estimate: ${estimate}"
    # 上限を見積もれない full re-encode は常に background を勧める。stream copy
    # だけは実測由来の上限が foreground timeout の半分に達した時点で切り替える。
    if [[ -z "$stream_copy_upper" ]] || (( stream_copy_upper * 2 >= FOREGROUND_TIMEOUT_MINUTES )); then
        echo "    Execution: background recommended (foreground timeout is about ${FOREGROUND_TIMEOUT_MINUTES} minutes)"
    fi
    echo "    Full file is not generated by --preview."
}

stream_copy_estimate() {
    local lower="$1"
    local upper="$2"
    local span
    if (( lower != upper )); then
        span="${lower}–${upper} minutes"
    elif (( lower == 1 )); then
        span="1 minute"
    else
        span="${lower} minutes"
    fi
    printf 'roughly %s for %s output (duration-based; disk I/O varies)' \
        "$span" "$(format_duration "${FULL_VIDEO_DURATION:-0}")"
}

print_stream_copy_outlook() {
    local route="$1"
    local suffix="${2:-}"
    local lower upper
    [[ "$PREVIEW_MODE" -eq 1 ]] || return
    read -r lower upper <<<"$(stream_copy_estimate_bounds)"
    print_full_output_outlook \
        "$route" \
        "$(stream_copy_estimate "$lower" "$upper")${suffix}" \
        "$upper"
}
...
if [[ "$OVERLAYS_ENABLED" -eq 1 ]]; then
    ...
    if [[ "$av_enabled" == "true" && -n "$av_fill_type" ]]; then
        ...
        if command -v yt-audio-visualizer-fill &>/dev/null; then
            ...
            AV_FILL_COMMAND=(python3 -m youtube_automation.commands.media.audio_visualizer_fill)
        else
    ...
    if [[ -n "$STREAM_SOURCE" ]]; then
        ...
        if [[ "$EFFECT" != "none" ]]; then
            print_stream_copy_outlook \
                ...
                " (plus first bake: about 10–40 seconds)"
        else
            print_stream_copy_outlook "loop stream copy"
        fi
