...
各表示値は `generate_videos.sh` が同じ実行で解決・出力した値を渡し、別に設定を再解決しない。HTMLは `tmp/reviews/master-video-preview.html` / `master-video-full.html` へatomic overwriteし、標準video player、duration、resolution、codec、filesize、背景経路、effect、overlay、Full output outlookを表示する。previewは短尺確認、fullは完成動画と明示する。
