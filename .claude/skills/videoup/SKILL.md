...
| スクリプト | 役割 | 場所 |
|-----------|------|------|
| `generate_master.sh` | 個別 MP3 → クロスフェード結合 → マスター MP3 | `automation/generate_master.sh` |
| `generate_videos.sh` | 音声 + サムネイル → MP4 動画 | `automation/generate_videos.sh` |

## Quick Reference

| コマンド | 説明 |
|---------|------|
| `bash automation/generate_master.sh` | CWD のコレクションでマスター音源生成 |
| `bash automation/generate_master.sh <path>` | 指定コレクションでマスター音源生成 |
| `bash automation/generate_videos.sh` | CWD のコレクションで全動画生成（コレクションディレクトリ内で実行） |
| `bash automation/generate_videos.sh <path>` | 指定コレクションで全動画生成 |
