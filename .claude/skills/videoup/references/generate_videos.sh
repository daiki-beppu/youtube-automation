...
trap 'rm -f "$PROGRESS_FILE" "$AV_FILL_ASSET"' EXIT
...
if [[ "$OVERLAYS_ENABLED" -eq 1 ]]; then
    ...
    # 入力配列: [0]=背景, [1]=master audio, [2]=fill PNG (任意), [2/3]=popup PNG (任意)
    INPUTS=()
    if [[ -n "$LOOP_VIDEO" ]]; then
    ...
    if [[ "$sp_enabled" == "true" ]]; then
        ...
        if [[ -z "$sp_path" || ! -f "$sp_path" ]]; then
            ...
            if [[ -n "$av_fill_input_idx" ]]; then sp_input_idx=3; else sp_input_idx=2; fi
        fi
    fi
    ...
    if [[ "$av_enabled" == "true" ]]; then
        ...
        if [[ -z "$av_fill_type" && "$av_mirror_center" != "true" && "$av_symmetric_vertical" != "true" && -z "$av_rounding_blur" ]]; then
            ...
            # showfreqs の黒背景を alpha と誤認しないよう、白バーの輝度を明示的に mask 化する。
            ...
            if [[ "$av_fill_type" == "gradient" || "$av_fill_type" == "rainbow" ]]; then
                ...
                # fill 未指定で effects のみ指定した場合も従来 colors を維持する。
