...
`/automation-release` スキル（本リポジトリのプロジェクトスコープ、リリース実施 = prepare + publish 担当）とは責務が異なる。本スキルは **publish 後の運営者向けノート生成 + 影響波及対応** に特化。グローバル `/release` は Node.js / npm 向けで本リポジトリでは使わない。
...
- `/automation-release` — 本リポジトリのリリース実施（prepare + publish）。本スキルの前工程
- `/release` — グローバル Node.js 向け（このリポジトリでは使えない、参考のみ）
