...
1. **チャンネル運営者向けアップグレードガイド** `docs/upgrades/v<ver>.md`（非エンジニア向け、TL;DR + カテゴリ別変更 + 影響表 + 実行手順 + Q&A）
...
- `docs/upgrades/v<ver>.md` のスタイルは **必ず `references/release-notes-template.md` のテンプレ構造を踏襲**（読者の認知負荷を一定に保つため）
