...
| `uv run yt-collection-serve <collections-root> --distrokid-capture-root <channel-root> --allow-origin chrome-extension://<EXTENSION_ID> --port 7874` | distrokid-helper 拡張向けに DistroKid dir mode サーバーを起動し、配信済み記録の POST も有効化 |
...
uv run yt-collection-serve "$CHANNEL_DIR/collections/planning" --distrokid-capture-root "$CHANNEL_DIR" --allow-origin chrome-extension://<EXTENSION_ID> --port 7874

# CHANNEL_DIR が未設定、またはチャンネル外の CWD から起動する場合
CHANNEL_DIR=/path/to/channel uv run yt-collection-serve /path/to/channel/collections/planning --distrokid-capture-root /path/to/channel --allow-origin chrome-extension://<EXTENSION_ID> --port 7874
```
