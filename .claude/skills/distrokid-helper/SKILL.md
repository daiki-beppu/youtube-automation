...
| サブコマンド | 説明 |
|---|---|
| `bunx tayk distrokid-prepare plan <collection> [--discs N] [--max-per-disc 35] [--output PATH]` | MP3 を列挙して均等分割し draft spec.json を出力。重複素タイトルには `needs_unique: true` が付く |
...
| `bunx tayk collection-serve <collections-root> --distrokid-capture-root <channel-root> --allow-origin chrome-extension://<EXTENSION_ID> --port 7874` | distrokid-helper 拡張向けに DistroKid dir mode サーバーを起動し、配信済み記録の POST も有効化 |
...
```bash
bunx tayk distrokid-prepare plan <collection>
# disc 数を明示したい場合:
bunx tayk distrokid-prepare plan <collection> --discs 2
# 上限を変えたい場合:
bunx tayk distrokid-prepare plan <collection> --max-per-disc 30
```
...
```bash
# リリース日が決まっている場合（推奨）
bunx tayk distrokid-prepare build \
...
```bash
# 正方形の場合
bunx tayk distrokid-prepare cover \
...
```bash
bunx tayk distrokid-prepare verify <collection>
```
...
bunx tayk collection-serve "$CHANNEL_DIR/collections/planning" --distrokid-capture-root "$CHANNEL_DIR" --allow-origin chrome-extension://<EXTENSION_ID> --port 7874

# CHANNEL_DIR が未設定、またはチャンネル外の CWD から起動する場合
CHANNEL_DIR=/path/to/channel bunx tayk collection-serve /path/to/channel/collections/planning --distrokid-capture-root /path/to/channel --allow-origin chrome-extension://<EXTENSION_ID> --port 7874
```
...
`--distrokid-capture-root` は distrokid-helper の配信済み記録 `POST /distrokid/releases` に必要。DistroKid dir mode では必ずチャンネルルートを指定する。`--allow-origin` は実際の distrokid-helper 拡張 ID に置き換え、DistroKid page origin からの write POST を許可しない。
