...
- **各作業は必ず適切な専用スキルを使用する**
- **最新情報は `python3 get_channel_status` で取得**
...
- **統計・コレクション一覧**: `python3 get_channel_status` で動的取得
...
ruff check automation/ --fix --unsafe-fixes  # 自動修正

# テスト
python3 -m pytest automation/tests/

# 動画アップロード
python3 automation/video_uploader.py
```
...
```
{{DIR_NAME}}/
├── get_channel_status     # チャンネル状況取得（拡張子なし executable）
...
```
データ収集は `daily_collect.sh`（launchd 毎朝 9:00）で自動実行。分析は `/ideate` 内部で実施。
...
- **データドリブン**: 重要判断の前に `get_channel_status` で最新情報確認
...
- **`get_channel_status`**: 拡張子なし executable（各チャンネルディレクトリに個別コピー）
