...
| thumbnail | `references/config-template/skills/thumbnail.yaml` | `image_generation.provider`（GCP 課金なしなら `codex` を優先案内）/ `image_generation.gemini.brand_background` / `composition_rules.*` / `reference_images.default`（TTP サムネ）/ `diff_prompt_template` |
...
`config/skills/thumbnail.yaml::reference_images.default` の欠落を解消してから次へ進む。
