...
- ssh-agent に秘密鍵を登録済み（`ssh-add ~/.ssh/yt_stream_key`）。`null_resource.deploy.connection` は `agent = true` で ssh-agent 経由に接続するため、未登録だと apply 時に `Permission denied (publickey)` で失敗する。`ssh-add -l` で登録済み鍵を確認できる
- 1Password に以下が登録済み:
...
|------|----------|
| 初回構築 | §1 |
| 動画差し替え | `scripts/streaming/swap_video.sh ./new_video.mp4` |
...
| `scripts/streaming/swap_video.sh` | `terraform plan` → `apply` の 1 コマンドラッパー |
...
scripts/streaming/swap_video.sh ./new_video.mp4              # 対話確認あり
scripts/streaming/swap_video.sh --auto-approve ./new_video.mp4  # 非対話
```
