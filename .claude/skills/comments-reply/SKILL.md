...
`rules` は廃止済みで、設定ファイルに存在するとロード時にエラーになる。keywords / pattern の未マッチで skip される経路はない。
...
- `comments.rules`: 廃止済み。設定に残っている場合は `ConfigError` で停止するため、`comments.json` から削除する
- `fallback_on_error`: `skip` / `retry`
- **破壊的変更**: 旧 `comments.generator.type`、`comments.templates`、`comments.rules`、`fallback_on_error: "template"` は廃止
