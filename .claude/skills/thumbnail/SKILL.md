...
3. 背景候補を `open` と `uv run yt-thumbnail-check <collection-path>/10-assets/main-v1.png --json` で確認し、ユーザー承認後に `cp main-v1.png main.png` で `10-assets/main.png` または `10-assets/main.jpg` として確定する。
...
5. `/thumbnail-compare` で 320px 視認性検証後にユーザー承認し、`cp thumbnail-v1.jpg thumbnail.jpg` でテキスト付き最終サムネを `10-assets/thumbnail.jpg` として確定し、`uv run python .claude/skills/thumbnail/references/archive-approved-thumbnail.py <collection-path>` を実行する。
