...
1. ベンチマーク参照画像から、文字・ロゴ・タイポグラフィを入れない textless 背景候補を `10-assets/main-v1.png` に生成する。
2. 承認後、`cp 10-assets/main-v1.png 10-assets/main.png` で textless 動画背景として確定する。
3. 承認済み `main.png` を参照画像にして、YouTube 用のテキスト付き候補を `10-assets/thumbnail-codex-v1.png` に生成する。
4. `/thumbnail-compare` で 320px 視認性検証後に承認し、PNG 候補を JPEG に変換して `10-assets/thumbnail.jpg` として確定する（例: `sips -s format jpeg 10-assets/thumbnail-codex-v1.png --out 10-assets/thumbnail.jpg`）。`thumbnail.png` のまま確定する場合も、YouTube アップロード用の文字入りサムネであり、動画背景には使わない。

既定テンプレート:

```text
TTP this reference thumbnail into a stronger original textless background for {title}.
Keep the winning layout, character scale, color mood, lighting, texture, and energy.
Remove all text, typography, logos, watermarks, brand marks, and channel badges.
Do not add any title text yet; the approved background will be used for the final thumbnail.
```
...
| モード | 説明 |
|---|---|
| `single_step`（**デフォルト**・TTP 推奨）| テキスト付き参照画像から構図・色・質感だけを参照し、textless 背景候補を先に生成。承認済み背景を参照して YouTube 用のテキスト付きサムネ候補を生成する、ベンチマーク模倣（TTP）の標準実装 |
...
2. 背景候補を `open` と `bunx tayk thumbnail-check` で確認し、ユーザー承認後に `10-assets/main.png` または `10-assets/main.jpg` として確定する。
...
4. `open` でプレビュー → `bunx tayk thumbnail-check <collection-path>/10-assets/main-v1.png --json` → ユーザー承認 → `cp main-v1.png main.png`（JPEG で確定する運用では `main-v1.jpg` → `main.jpg` に揃える。拡張子を偽装しない）
...
既存 `main.png/jpg` は企画参照または過去背景として扱い、`thumbnail.jpg` と同一画像で代用しない。Two-Phase でも Phase 1 の背景承認を先に行い、承認済み `main.png/jpg` を Phase 2 の参照画像として使う。
...
3. `open` でプレビュー → `/thumbnail-compare` で 320px 視認性検証 → 承認 → `cp thumbnail-v1.jpg thumbnail.jpg`

この経路では承認済み `main.png` に決定的テキストを合成して `thumbnail.jpg` を作るため、背景先行のファイル契約をそのまま維持できる。
...
TTP 参照画像が固定されているチャンネルでは、候補生成後のユーザー承認を省略し、`bunx tayk thumbnail-auto-select` で `10-assets/thumbnail.jpg` を自動確定できる（#1370）。`auto_selection.enabled` が false / 未設定のチャンネルでは何も変わらず、従来の手動承認フローを使う。
...
```bash
bunx tayk thumbnail-auto-select <collection-path> --dry-run
```

2. 問題なければ apply で確定する（`--json` で選択理由を構造化出力できる）:

```bash
bunx tayk thumbnail-auto-select <collection-path> --apply
```
