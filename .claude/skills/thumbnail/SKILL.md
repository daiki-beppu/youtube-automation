...
2. 背景候補を `open` と `yt-thumbnail-check` で確認し、ユーザー承認後に `10-assets/main.png` または `10-assets/main.jpg` として確定する。
...
4. `open` でプレビュー → `yt-thumbnail-check <collection-path>/10-assets/main-v1.png --json` → ユーザー承認 → `cp main-v1.png main.png`（JPEG で確定する運用では `main-v1.jpg` → `main.jpg` に揃える。拡張子を偽装しない）
...
3. 生成: `yt-generate-image --reference <参照画像> --prompt <プロンプト> --output 10-assets/main-v1.png -y`
...
`yt-thumbnail-check` は `main-v1.png` / `main-v1.jpg` のような **テキストなし背景候補**を対象にする。Gemini Vision で `collection-ideate.yaml` の `objects.fixed` と
...
- [ ] 新しい文字や記号が追加されていないか
- [ ] `yt-thumbnail-check <collection-path>/10-assets/main-v1.png --json` を通したか（JPEG 候補なら `main-v1.jpg` を指定）
