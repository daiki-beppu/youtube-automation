...
| 経路 | 仕組み | フォント再現性 |
|---|---|---|
| **AI プロンプト経路**（既定） | プロンプトで書体の雰囲気を指示（`thumbnail_text.font` / `single_step.typography_clause`） | **保証されない**。AI 画像生成はフォント名を厳密に再現できず、同じ指示でも生成ごとに書体が揺れる |
| **決定的合成経路**（`yt-thumbnail-text`） | textless 背景に実フォントファイル（.ttf/.otf/.ttc）を Pillow で描画 | **完全に安定**。同一の背景・テキスト・設定なら常に同一出力 |

### AI プロンプト経路でのフォント指示

- **single_step（TTP）**: 参照画像のタイポグラフィ「感」がデフォルトで継承されるが、書体は生成ごとに揺れる。書体の一貫性を高めたい場合は `diff_prompt_template` に `${typography_clause}` を展開し、`{font_description}` を `thumbnail_text.font.copy` の値で置換する
...
この経路では `thumbnail.jpg` が textless 背景 + 決定的テキストで完結するため、「承認済み thumbnail.jpg から textless 版を AI 再生成する」工程は不要（背景がそのまま `main.png` になる）。
