...
読み込み後は `youtube_automation.utils.skill_config.load_skill_config("video-description")` と同じ deep-merge 前提で、チャンネル上書きを優先して扱う。存在しない override は未設定として扱い、勝手に作成しない。
...
前工程は `/videoup`（マスター動画生成）。`/wf-next` の公開フローでは `/videoup` と並列に起動される。
