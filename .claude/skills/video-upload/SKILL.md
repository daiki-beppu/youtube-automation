...
- `yt-upload-collection --plan` は collection 専用の事前確認であり、single_release 型では使わない。公開時刻は `yt-upload-auto` の実アップロード経路（metadata の `publish_at` または `config/channel/youtube.json` の既定時刻 fallback）に従う
...
`content_model.type = "single_release"` のときは `yt-upload-auto` を使い、言語ごとに別動画をアップロードする。`yt-upload-collection --plan` は collection uploader の公開予定計算なので、この分岐では実行しない。公開時刻を案内する場合は、`yt-upload-auto` が読む metadata の `publish_at` と `config/channel/youtube.json` の既定時刻 fallback に基づくことを明示し、collection 用 plan 結果を流用しない。
