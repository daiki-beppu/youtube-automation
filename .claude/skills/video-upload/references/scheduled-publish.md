...
### 方法 3: チャンネル既定時刻（簡易 fallback）

曜日 cadence までは不要で、チャンネルごとに毎回同じ時刻へ予約したいだけなら、`config/channel/youtube.json` に既定時刻を保存できる:

```json
{
  "youtube": {
    "default_publish_time": "20:00",
    "default_publish_timezone": "Asia/Tokyo"
  }
}
```

- `schedule_config.json` の `schedule.cadence` / `schedule.publish_time` / `auto_schedule_enabled: true` がある場合は、従来の schedule 計算を優先する
...
`--plan` はアップロード API は叩かないが、予約日時計算のため YouTube read API を呼ぶ場合がある。

```bash
uv run yt-upload-collection --plan -c <COLLECTION_NAME>
```
