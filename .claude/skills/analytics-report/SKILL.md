...
読み込み後は `youtube_automation.utils.skill_config.load_skill_config("analytics-report")` と同じ deep-merge 前提で、チャンネル上書きを優先して扱う。存在しない override は未設定として扱い、勝手に作成しない。HTML レポートの KPI カードは `html.kpi_cards`、Shorts 除外キーワードは `html.exclude_title_keywords` を参照する。
...
`reports/` ディレクトリから更新時刻が最新のレポートファイル（`ls -t reports/ | head -1` で取得できるもの）を検出して内容を表示する。
...
`data/` 配下の **全 analytics スナップショット** と `benchmark` データを集約し、視覚的な HTML レポートを `reports/` に生成する（KPI カード・Shorts 除外は「設定読み込みゲート」で読んだ skill-config を使う）。
