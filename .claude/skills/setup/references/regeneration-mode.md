...
- [ ] 投稿時刻・投稿頻度（collected-analysis branch で既に取得済みなら `docs/channel-research.md` を参照）
- [ ] サムネテンプレ・タイトルテンプレ（既存の collected-analysis 成果物 + 競合 uploads playlist のサンプル）
...
生成ルールは **`.claude/skills/setup/references/config-generation-rules.md`** を参照（tags / descriptions / title / suno の書き方、および TTP 路線時の競合転写ルール）。
雛形は `.claude/skills/setup/references/config-template/*.json`（責務別 5 ファイル: meta / content / youtube / analytics / audio）。
...
`/setup --channel` が作成した初期 config を完全版に拡張。`.claude/skills/setup/references/config-template/` の各ファイルを
...
| lyria（`music_engine: lyria` のとき）| `.claude/skills/music/config.default.yaml::generate.lyria` を参照 | プロンプト系・尺・track 戦略 |
...
**fail-fast 動作**: `/thumbnail` `/music --prompt` `/music --generate` 等の下流 skill は、関連 config が空のまま
...
JSON 構文検証・config ロードテスト・channel_id 自動取得コマンドは **`.claude/skills/setup/references/verification.md`** を参照。
...
5. **任意のパイロット検証**: 色味・構図・ムード・テンポを先に確認したい場合は、仮コレクションで `/thumbnail` → `/thumbnail --compare`、および `music_engine` が `suno` なら `/music --prompt` → `/music --generate`、`lyria` なら `/music --generate` を実行し、OK/NG を判断する。NG なら `config/skills/thumbnail.yaml` / `config/skills/music.yaml::prompt` / `config/skills/lyria.yaml` を調整して再試作する。OK なら仮コレクションを削除するか、既存 `collections/planning/` として `/wf-next` で継続する
