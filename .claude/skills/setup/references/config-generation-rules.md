...
| `hashtags` | **5 個** 程度（`config/channel/content.json::descriptions.hashtags` が単一ソース。`/video --describe` と実装 `application/metadata/service.py` はこの値をそのまま出力する。YouTube は最初の 3 個をタイトル下に表示） |
