...
- **設定管理**: `load_config()`（`youtube_automation.configuration`、依存パッケージ `youtube-channels-automation` 内。frozen dataclass を返す）
...
```
`/wf-new` 1コマンドで企画から公開まで一気通貫。ミキシング工程なし。音楽生成は `config/channel/youtube.json` の `music_engine` に応じたスキル（`/music --prompt` or `/music --generate`）が使われる。
...
- **`load_config()`**: frozen dataclass を返す。テスト時は `youtube_automation.configuration.reset()` でシングルトン state をリセット
