...
8. 冒頭の「完了条件」に従い、条件を満たすなら「運用設定インタビュー」を実行してから「完了時」の報告を行う
9. 「完了条件」を満たさないなら、未完了の check に対応する手順 (§Steps) に進む
...
利用者が "done" と返すまで、次の Bash ツール呼び出しはしない。返ってきたら該当 step に後処理があれば先に実行し、`uv run yt-doctor --json` を再実行して結果を確認する（`client_secrets` は `yt-doctor --fix-client-secrets` を先に実行する）。
...
iCloud Drive 等のクラウド同期コンフリクトで `.venv/bin/` や `.claude/skills/` に `<名前> 2` 形式の重複ファイルが生成されたケース。`yt-doctor` の `next_action.instructions` に従って重複ファイルを削除する（`.venv` 側は `rm -rf .venv && uv sync`、skills 側は `uv run yt-skills sync --asset skills --force` で復元できる）。
...
ペアが stale の場合は、`yt-doctor` の message で stale を表示したうえで setup のブロッカーにしない。`next_action.kind == "human"` でも `[HUMAN STEP]` として `/analytics-analyze` の実行を利用者へ依頼せず、後続の `/collection-ideate` が同じセッションで自動更新する旨を案内する。`summary.next_check_id == "analytics_report"` のままでも `checks` 配列の後続 check を確認し、ほかの未完了 check があればその check の手順へ進む。
...
"done" の後に `uv run yt-doctor --json` を再実行する。
...
選択した対処の完了後に `uv run yt-doctor --json` を再実行し、ID 一致を確認する。
...
再試行条件が整った後だけ `uv run yt-doctor --json` を再実行する。
