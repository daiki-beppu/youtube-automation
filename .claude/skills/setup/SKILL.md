...
Markdown があるのに同日付 JSON がない、または validator が失敗する場合は fallback せず `/analytics-analyze` 再実行を案内する。ペアが stale の場合は `/collection-ideate` が `freshness.stale_action` に従い、コスト見積もり付きの `ask`、上限内の `auto`、従来どおりの `manual` を処理する。`/wf-new` はこの stale 判定を重ねない。
