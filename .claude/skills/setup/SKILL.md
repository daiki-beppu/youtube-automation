...
| `data` | `/wf-new` の入力モード判定データ（analytics_report / benchmark_data）。minimal mode / benchmark fallback mode は setup のブロッカーにしない |
...
4. `uv run yt-skills sync --asset skills --force` と `uv run yt-skills sync --asset claude-md` を Bash で実行する
5. `uv run yt-doctor --json` を Bash で実行し、結果を読む
6. `summary.next_check_id` が `null` なら「全 check 緑です。`/setup` は完了済みです」と報告して終了
7. `null` でないなら、その check に対応する手順 (§Steps) に進む
8. 1 ステップ完了したら、必ず `uv run yt-doctor --json` を再実行して進捗確認してから次の `next_check_id` に移る
...
>   5. 作成直後に表示される client secret を控えるか JSON をダウンロードして、以下のパスに配置:
>      <channel_dir>/auth/client_secrets.json
>      作成直後の画面を閉じて secret を見失った場合は、
>      「Clients」→対象 client→「Client secrets」→「Add secret」で新しい secret を発行し、
>      JSON を再ダウンロードしてください。JSON ダウンロードが表示されない場合は
>      auth/client_secrets_template.json の client_id / project_id / client_secret を手入力します。
>
> 完了したら "done" と返してください。
```
...
AI は config をここで生成しない。`yt-doctor` の `message` に含まれるエラー詳細をそのまま利用者に示し、どちらのルートかを確認してから案内する。
