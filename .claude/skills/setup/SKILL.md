...
責務は「automation ツールが動き、API 認証とアップロード前提が通る状態」まで。新規チャンネルの config 生成、TTP ベンチマーク収集、ペルソナ、branding は `/channel-new` が担当する。
...
- `/channel-new`: 新規チャンネルの TTP 収集、config 生成、ペルソナ、branding (`channel_config` fail・新規チャンネルの場合)
