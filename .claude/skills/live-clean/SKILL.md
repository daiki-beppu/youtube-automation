---
name: live-clean
description: Use when live コレクションの大容量メディアファイルを削除してディスク容量を回復したいとき。「容量」「クリーンアップ」「掃除」「ディスク」「live 整理」「マスターファイル削除」「容量確認」「でかいファイル」など、公開済みコレクションの不要ファイル削除に関わる場面で必ず使用すること
---
