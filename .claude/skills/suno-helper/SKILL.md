...
3. サーバー出力の `detected extension: suno-helper -> <id> (chrome-extension://<id>)` を確認し、`curl -s -H "Origin: chrome-extension://<id>" http://localhost:7873/auth/token | python3 -m json.tool` が `{ "token": "..." }` を返す
