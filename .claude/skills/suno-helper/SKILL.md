...
5. `[data-suno-control="server-url"]` で Step 1 で確認した URL の配信元候補を選ぶ。
...
7. 配信元または collection の選択後に自動取得が完了し、`role="status"` の live region が `N パターンを取得しました。` になること、`data-suno-entry-count` が 1 以上で `[data-suno-entry-list]` に entry 行が並ぶことを確認する。最新データを取り直す場合はページを再読み込みする。
...
| 自動取得 | 初回表示・配信元選択・collection 選択 | サーバーから prompts JSON を fetch して一覧表示し、配信元 URL を自動保存。再取得はページを再読み込み |
| 連続実行 | 実行時 | 開始 |
| 停止 | 実行中のみ有効 | 任意中断 |
...
| `[data-suno-control="server-url"]` / `[data-suno-control="collection-select"]` | 登録済みローカル配信元の選択 / collection 選択 |
