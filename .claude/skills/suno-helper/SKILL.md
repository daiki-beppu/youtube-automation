...
| 役割 | コマンド |
|---|---|
| サーバー起動（dir mode = 推奨） | `uv run yt-collection-serve "$CHANNEL_DIR/collections/planning"` |
| サーバー起動（single collection 限定） | `uv run yt-collection-serve "$CHANNEL_DIR/collections/planning/<YYYYMMDD>-<channel>-<theme>-collection"` |
...
複数 collection を切り替えながら回す可能性があるなら **dir mode**（`collections/planning` を path に渡す）で起動する。collection が 1 件しかなく今後切替えない場合のみ single file mode（collection dir そのものを path に渡す）。

dir mode で読まれるのは **`-collection` suffix を持つ dir のみ**。それ以外（例: `01-master` や雑多ファイル）は無視される。

判別: 起動後に `curl http://localhost:7873/collections` が JSON array を返せば dir mode、404 なら single file mode。
...
| Collection 選択 | dir mode 時のみ | ドロップダウンから対象 collection を選ぶ。選択した瞬間に下に "Playlist 名" が auto derive される |
...
| ⚖️ Balanced（デフォルト） | 20-30 entries の標準 collection | 10s ±3s |
...
- **single file mode で起動すると playlist phase がスキップされる**。`/collections` 404 が返り、popup 側で derivedPlaylistName が undefined になり playlist phase に分岐しない。**collection mode を使うなら必ず dir mode で起動する**。
...
- collection mode を使うときは必ず dir mode で起動する（playlist phase を生かすため）
