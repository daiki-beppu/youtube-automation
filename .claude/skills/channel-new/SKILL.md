...
- **完了条件**: Step R7 の検証（`uv run yt-config-migrate verify` 成功）を経て、Step R8 の次ステップ案内を提示した時点で完了
...
- **完了条件**: `config/channel/*.json` の生成、`uv run yt-config-migrate verify` の成功、OAuth 認証、`channel_id` の `config/channel/meta.json::channel.channel_id` 保存、次ステップ案内まで到達した時点で完了。新規開設モードの TTP 完了条件（`benchmark.channels` / `ttp-seed-confirmation.md` / branding snapshot / `ttp_wf_new_readiness`）は取り込みモードには適用しない
