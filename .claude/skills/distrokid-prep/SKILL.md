...
- **disc 分割** — 1 アルバム 35 曲上限を考慮して均等分割
- **spec.json** — 分割計画・メタデータの機械可読 JSON（LLM がタイトルユニーク化・アルバム名決定を担当）
- **metadata.md** — DistroKid Web フォーム転記用テンプレート（各 disc に生成）
...
`--force` は spec 記載の disc dir のみを再生成する（cover_art_3000.jpg と spec.json は不可触）。既存 disc dir がある場合は `--force` なしでは停止する。
...
| 既存 30-distrokid がある | build 時に「disc dir already exists」エラー | `build --force` で spec 記載の disc だけ再生成される。`cover_art_3000.jpg` と `spec.json` は `--force` でも上書きされない（cover は `cover --force` で別途上書き） |
