...
欲求語彙の選択、欠落時の継続条件、`推定` と根拠の記録は `references/desire-vocabulary.md` をそのまま適用する。
...
`benchmark_json`、`comments_json`、`benchmark_report` がすべて空でないことを確認する。`thumbnail-text-profile.md` は前回の channel-research 成果物であり、個別レポートの存在判定には数えない。
...
抽出したコメント由来の欲求を Step 2〜4 の勝ちパターンへ接続する。タイトル・サムネイル・楽曲 / 音楽性の各要素について `勝ちパターン X ← 欲求 Y（根拠: コメント / タイトル語彙）` を作り、欲求、`推定` / `判定不能`、根拠には `references/desire-vocabulary.md` の適用結果をそのまま記録する。
