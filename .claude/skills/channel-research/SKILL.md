...
- `docs/benchmarks/*.md` — 各チャンネルの個別レポート（`thumbnail-text-profile.md` は含めない。除外後に 0 件なら先に `/benchmark` を案内して停止）
- `docs/benchmarks/thumbnails/` — サムネイル画像（ある場合。無ければ Step 4 は個別レポート内の `## サムネイル分析` または `## サムネイル分析（Gemini API）` 参照に切り替え）
