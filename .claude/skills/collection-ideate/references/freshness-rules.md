...
`/collection-ideate` の Phase 1 を開始する前に、前提スキルの出力が最新であることを確認する。
...
|---|---|---|---|---|
| 1a | `/analytics-analyze` | `reports/analysis_*.md` | 最新 `data/analytics_data_*.json` のファイル名日付 (YYYYMMDD) より古ければ stale | `/collection-ideate` を中断し、ユーザーに `/analytics-analyze`（必要なら `/analytics-collect` 先行）の実行を案内。**自動呼び出し不可**（AI 推論コスト発生のため） |
...
| 2 | `/audience-persona` | `docs/channel/personas/persona-definition.md` | 存在すれば OK（mtime 比較なし。更新タイミングは戦略判断のため人間が決める） | ユーザーに `/audience-persona` 実行を案内（自動呼び出しはしない — ペルソナ選択に `AskUserQuestion` が必要なため） |
...
|---|---|
| `planning` | benchmark / persona / viewing-scene は `/collection-ideate` セッション内で最新化される |
...
if [ -z "$LATEST_REPORT" ]; then
  echo "analyze 未生成 → /collection-ideate 中断、/analytics-collect → /analytics-analyze 先行を案内"
  exit 1
fi
...
if [ ! -f docs/channel/personas/persona-definition.md ]; then
  echo "persona 未定義 → /audience-persona を案内"
fi
