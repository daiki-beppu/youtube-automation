...
3. `reports/analysis_*.md` が stale（最新 `data/analytics_data_*.json` より古い、または最新 `data/analytics_data_*.json` のファイル名日付が実行日から `freshness_days`（既定 7 日、`config/skills/collection-ideate.yaml` で上書き可）を超えて古い）場合は fallback せず、`/analytics-analyze` 再実行を案内して停止。絶対鮮度 stale では収集データ自体が古いため `/analytics-collect` → `/analytics-analyze` の順で案内する
