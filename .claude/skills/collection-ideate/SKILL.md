...
- `true`: `differentiation_axes` の掛け合わせをスキップし、競合の高再生コレクションから抽出した構造・パターン・型を直接転写する。競合カバー済みの実績あるテーマを優先し、各企画に転写元（競合名と高再生コレクションまたは勝ちパターン）を明記する
...
- Markdown / JSON ペアが stale → fallback せず `references/freshness-rules.md` の stale action とコスト承認へ分岐する。`ask`（未設定時の既定）は見積もり後に `自動実行する` / `案内のみ（従来動作）` / `中断` の 3 択、`manual` は従来の中断 + 案内、`auto` は既知の見積額が任意の上限以下の場合だけ確認なしで実行する。見積不能または上限超過の `auto` は `ask` へフォールバックする。
- 承認済みの自動実行は Skill ツールで `/analytics-analyze`（絶対 stale は `/analytics-collect` を先行）を呼び、完了後に Markdown / JSON ペア、validator、鮮度を再検証する。成功時だけ同じセッションで Phase 1-2 へ進む。失敗時はエラーと再開手順を示して停止し、企画生成へ進まない。
...
JSON ペア検証 Hard Gate、入力モード判定、相対比較・絶対鮮度の stale 判定、stale action、既定 `freshness_days`、workflow-state との同期は `references/freshness-rules.md` を正とする。validator 失敗は停止し、stale は同 reference の承認フローが成功した場合だけ Phase 1 を続行する。
...
- `reports/` の Markdown / JSON ペアが stale（最新 `data/analytics_data_*.json` のファイル名日付より古い、または収集データ自体が実行日から `freshness_days` を超えて経過）→ fallback せず stale action とコスト承認へ分岐。自動分析後の再検証成功時は同じセッションで続行し、それ以外は停止
...
- `ttp_mode: true`: 競合の高再生コレクションを順位付けし、実績あるテーマと、その勝ちパターン（構造・パターン・型）を `preview.candidate_count` 件以上抽出する。未開拓性や差別化可能性では絞り込まず、競合カバー済みテーマを優先候補として残す。各候補には競合名と対象コレクションまたは勝ちパターンを紐づける
...
`ttp_mode: true` では差別化戦略を立案せず、Phase 1-4 で順位付けした高再生パターンを企画へ直接転写する戦略を作る。
...
`ttp_mode: true` では `differentiation_axes` を候補生成へ使わない。各候補を別々の高再生パターンに対応させ、転写元と転写する構造・パターン・型を明記する。
...
benchmark fallback mode / minimal mode では停止せず、入力モードごとの材料から初回仮説の視聴者像を明記する既存挙動は、minimal mode では `ttp_mode: false` の場合だけ適用する。`ttp_mode: true` の minimal mode はベンチマーク生成まで停止する:

- benchmark fallback mode: ベンチマークで反応が強い視聴シーンと `config/channel/content.json` の genre / tags から仮説ペルソナを作る
- minimal mode（`ttp_mode: false` のみ）: ユーザーが入力したテーマ / ジャンル / 雰囲気と `config/channel/meta.json` / `config/channel/content.json` から仮説ペルソナを作る
...
`ttp_mode: true` では高再生タイトルだけでなく、高再生コレクションの実績テーマを優先する。転写対象は競合固有のタイトルや表現そのものではなく、そこから抽出した構造・パターン・型とする。
...
`/collection-ideate` は `docs/channel/personas/persona-definition.md` の **第一ペルソナ 1 人** に絞って、`preview.candidate_count` 個の企画候補を生成する。複数ペルソナをローテーションせず、`ttp_mode: false` では同じ人物の別シーン・別感情・別利用文脈、`true` では別々の高再生パターンから企画の幅を出す。
...
4. benchmark fallback mode / minimal mode で persona 文書が存在しない場合は、入力モードごとの材料から作る初回仮説の視聴者像を今回のターゲットペルソナとして扱う既存挙動を、minimal mode では `ttp_mode: false` の場合だけ適用する。`ttp_mode: true` の minimal mode は `/benchmark` を案内して停止する
   - benchmark fallback mode: ベンチマークデータ + config
   - minimal mode: ユーザー直接入力（テーマ / ジャンル / 雰囲気）+ config（`ttp_mode: false` のみ）
