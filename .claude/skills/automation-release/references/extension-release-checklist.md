...
```bash
pnpm -v
# → 9 系（各拡張の package.json::packageManager の pin に従う）
```

無ければ導入を案内して abort。Node は 22 系（`release-extensions.yml` と同契約）。
...
**対応**: 停止して原因を特定する。典型は `--frozen-lockfile` / `--ignore-workspace` を付けない install による `pnpm-lock.yaml` 書き換わり・root への lockfile / workspace 設定の混入、`pnpm add` の誤実行。復旧は `git checkout -- <file>` で差分破棄 → 正しいフラグで verify 再実行。`.output/` / `.wxt/` / `node_modules/` が `git status` に出る場合は `.gitignore` の破損なので、リリースを中断して先に修正する。
