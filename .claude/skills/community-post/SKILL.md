...
Step 6 のユーザー案内（クリップボードコピー済み + Studio での貼り付け・添付・投稿手順）を提示した時点で完了。実際の投稿はユーザーの Studio 手動操作であり、完了条件に含まない。`pbcopy` / `open` が失敗した場合も、stdout フォールバック（テンプレ本文と Studio URL の表示）まで到達すれば完了扱いとする。
