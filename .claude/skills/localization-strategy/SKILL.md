...
**Top 10 CPM 国の主要言語は必須対応**: `MANDATORY_LANGUAGES = {en, de, no}` (CPM 上位 AU/US/CA/NZ/GB/CH/DE/NO/IE/SG の主要言語) は `recommend_supported_languages` が常に `add` または `keep` に含めるため、削除候補にならない。Top 10 構成が変わったら `TOP_CPM_COUNTRIES` を更新すれば自動で `MANDATORY_LANGUAGES` も追従する。
