...
| 引数 | 説明 |
|------|------|
| `/report latest` | 最新の分析レポートを表示 |
| `/report html` | HTML ビジュアルレポートを生成（全履歴データ集約） |
| `/report list` | 全レポートファイル一覧表示 |
| `/report` | 引数なし = 最新レポート表示 |

## Instructions

### `/report latest` / `/report`（デフォルト）
...
3. `config/channel_config.json` からチャンネル情報を取得
4. 各スナップショットから以下を抽出:
...
3. **動画別パフォーマンス表**
   ...
   - Complete Collection のみ表示（Shorts を除外）
...
- **ファイル名**: `reports/{channel_slug}_analytics_YYYYMMDD.html`（`channel_slug` は `channel_config.json` の `channel.short` を小文字化したもの、日付は実行日）
...
- Shorts は動画パフォーマンス表から除外（タイトルに `#Shorts` を含む）
