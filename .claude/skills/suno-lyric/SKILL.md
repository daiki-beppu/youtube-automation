...
1. `suno-patterns.yaml` から最終 entry name を作る。`/suno` と同じく `{name_jp} — {name_en}`、複数 scene の場合は ` (Variation N)` を付ける。ボーカルモードでは `config/skills/suno.yaml::tracks_per_pattern` 分だけ展開し、各 entry に ` (Take N)` も付ける
...
5. Lyrics は V5.5 向けに section tags を明示する。基本形は `[Intro]`, `[Verse 1]`, `[Pre-Chorus]`, `[Chorus]`, `[Verse 2]`, `[Instrumental]`, `[Bridge]`, `[Final Chorus]`, `[Extended Outro]`, `[Outro]`。`[Verse]` / `[Chorus]` だけでなく `[Intro]` `[Pre-Chorus]` `[Bridge]` `[Extended Outro]` `[Outro]` も曲ごとの scene / persona に合わせて書き分け、他の曲から本文を流用しない（Suno は歌詞テキストに強く追従するため、これらが同一だと全曲の入り・終わりが似通う）
...
- `name` は `/suno` の最終 prompt entry name と完全一致させる。`tracks_per_pattern > 1` の場合は ` (Take N)` 付きの展開後 name を全件出力する
...
- `suno-lyrics.json` の各 `name` が `suno-patterns.yaml` と `tracks_per_pattern` 由来の展開後 entry name と一致する
...
- 成果物チェックを実行して exit 0 を確認する: `uv run yt-suno-verify <collection>`
