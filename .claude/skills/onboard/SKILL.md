...
| `data` | /wf-new のハード前提データ（analytics_report / benchmark_data）（2 check） |
| `upload` | upload 必須 scope 充足・channel_id 設定済み（1 check） |

全 check 緑 = `/wf-new` が即中断せず走り出せ、かつ動画アップロードまで通る状態。
...
`/wf-new` は内部で `/collection-ideate` を呼び、そこで analytics レポートとベンチマークデータをハード要求する。これらが揃っていないと `/wf-new` が即中断するため、本カテゴリで事前確認する。

### `analytics_report` — analytics レポート未生成

**ハイブリッド方針 (AI 推論コストのため案内に留める)**:

利用者に以下を案内する（AI が直接実行しない）:

```
> [HUMAN STEP]
> analytics レポートが未生成です。以下の順で実行してください:
>   1. /analytics-collect  （YouTube Analytics API からデータ収集）
>   2. /analytics-analyze  （収集データを分析してレポート生成）
> /analytics-analyze は AI 推論コストが発生するため手動実行をお願いします。
> 完了したら "done" と返してください。
```

### `benchmark_data` — ベンチマークデータ未生成

**AI 主導 (ai-exec)**:

AI が以下を Bash で直接実行できる:

```bash
# /benchmark スキルを実行
```

`yt-doctor` の `next_action.cmd` を参照して実行する。
...
- `/analytics-collect`: YouTube Analytics データ収集 (`analytics_report` fail 時に実行)
- `/analytics-analyze`: Analytics データ分析・レポート生成 (`analytics_report` fail 時に実行)
- `/benchmark`: ベンチマークデータ生成 (`benchmark_data` fail 時に実行)
