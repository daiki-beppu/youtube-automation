...
| 4 | `/thumbnail` または preview 採用 | `main.png` / `thumbnail.jpg` を確定する | `10-assets/thumbnail.jpg` |
...
| 7 | `/loop-video` | 承認済み `main.png` から loop video を生成 | `10-assets/loop.mp4` |
...
1. **企画成果物を collection に固定**:
   - 選択した企画のプレビュー画像を `10-assets/main.png` にコピー
   - Phase 1 の企画候補一覧と選択結果を `20-documentation/` に保存
   - プレビューディレクトリの自セッション分を削除

2. **サムネイル skill を順番に処理**:
   - `single_step` モードまたは `image_generation.provider: codex` の場合: `/collection-ideate` の preview が完成サムネイルなので `/thumbnail` は呼ばない。Read ツールで `main.png` を等倍確認し、手・指、テキスト、署名 / 透かし / ロゴの QA が OK の場合のみ以下を実行する:
     ```bash
     cp <collection-path>/10-assets/main.png <collection-path>/10-assets/thumbnail.jpg
     ```
   - それ以外のモード: `/thumbnail <theme>` を Skill ツールで実行し、`10-assets/thumbnail.jpg` を生成する
...
サムネイル承認後、`/loop-video` を Skill ツールで実行する（`main.png` → `loop.mp4`）。
