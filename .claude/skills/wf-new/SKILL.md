...
4. **analytics input gate**: 入力モード判定、同日付 JSON、validator、stale action、コスト承認は `/collection-ideate` に一元化する。`/wf-new` は stale 判定や AskUserQuestion を先行実行せず、ダイアログを二重表示しない。subagent は `/collection-ideate` の `references/freshness-rules.md` と `references/freshness_action.py` で判定し、`outcome: "ask"` の場合は選択肢をメインへ返す。メインがその選択肢を 1 回だけ AskUserQuestion で提示し、回答後に同じ subagent 作業を再開する。`yt-doctor` の `analytics_report` は予備確認にだけ使い、analytics mode の最終判定には使わない。
...
1. **入力ファイルの予備確認** — メインは以下の候補だけを確認する。入力モード、JSON ペア、validator、stale 判定・承認は `/collection-ideate` が 1 回だけ確定する。subagent が `outcome: "ask"` を返した場合に限り、メインが返却された見積・理由・choices を AskUserQuestion に一度提示し、その回答を再開入力として渡す
