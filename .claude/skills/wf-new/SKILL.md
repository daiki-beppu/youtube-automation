...
4. **analytics input gate**: 入力モード判定、同日付 JSON、validator、stale action、コスト承認は Skill ツールで呼ぶ `/collection-ideate` に一元化する。`/wf-new` は stale 判定や AskUserQuestion を先行実行せず、ダイアログを二重表示しない。`yt-doctor` の `analytics_report` は予備確認にだけ使い、analytics mode の最終判定には使わない。
...
1. **入力ファイルの予備確認** — 以下の候補を確認する。入力モード、JSON ペア、validator、stale 判定・承認は `/collection-ideate` が 1 回だけ確定する

| モード | 判定条件 | `/collection-ideate` の入力 |
|---|---|---|
| analytics mode | 同日付 report ペア候補が存在する（最終判定は `/collection-ideate`） | 日次収集データ + 構造化分析 JSON + ベンチマーク + config |
...
入力モード、JSON ペア検証、stale 判定、stale action、コスト承認の完全な定義は freshness-rules.md を正とし、`/collection-ideate` が適用する。`.claude/skills/collection-ideate/config.default.yaml` + `config/skills/collection-ideate.yaml` の deep-merge も同 skill に委譲し、ここでは判定ロジックを再定義しない。
