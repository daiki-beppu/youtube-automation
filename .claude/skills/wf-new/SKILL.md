...
4. **analytics input gate**: メインが `/collection-ideate` の `references/freshness-rules.md` と `references/freshness_action.py` を使って入力モード、同日付 JSON、validator、stale action を判定する。`outcome: "ask"` と画像プレビューのコスト確認を含む AskUserQuestion はメインが完了し、必要な `/analytics-collect` / `/analytics-analyze` / `/benchmark` 実行後に鮮度を再検証する。承認済みの入力と生成条件が確定するまで `/collection-ideate` subagent を起動しない。`yt-doctor` の `analytics_report` は予備確認にだけ使い、analytics mode の最終判定には使わない。
...
- **順次実行**: Agent ツールで起動する subagent は必ず上から一作業ずつ呼ぶ。並列 Agent は使わない
...
1. **入力ファイルと承認の確定** — メインが以下の候補を確認する。入力モード、JSON ペア、validator、stale 判定は `references/freshness-rules.md`、stale action は `references/freshness_action.py` を使って確定し、`outcome: "ask"` なら helper の見積・理由・choices を AskUserQuestion に提示して回答を `--choice` に渡す。`outcome: "execute"` の前提 skill を実行した後は同 reference の判定を先頭から再実行する
...
入力モード、JSON ペア検証、stale 判定、stale action、コスト承認の完全な定義は `/collection-ideate` の `references/freshness-rules.md` を正とする。メインは `.claude/skills/collection-ideate/config.default.yaml` + `config/skills/collection-ideate.yaml` を `load_skill_config("collection-ideate")` と同じ規則で deep-merge し、解決値を helper と承認文面に使う。minimal mode のテーマ / ジャンル / 雰囲気と、プレビューを生成する場合の候補・枚数・コスト承認もメインが確定する。
...
このステップはサムネイル候補の生成までを行う。候補の承認と確定は Phase 2d の承認ゲートに一元化する。
...
#### 2d. サムネイル承認・確定 + 音楽素材生成

このステップにテキスト付き候補の承認ゲートを一元化する。承認後にだけ textless 候補生成を subagent へ委譲する。
...
2. `/thumbnail-compare` の 320px 視認性検証後、AskUserQuestion でテキスト付き候補の承認を求める:
   ```
