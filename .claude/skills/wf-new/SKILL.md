...
Suno チャンネルではプロンプト生成後、`suno-helper` 用の `bunx tayk collection-serve` 起動と疎通確認まで行い、Chrome 拡張でのブラウザ実行だけを user に引き継ぐ。
...
| 8 | `bunx tayk collection-serve`（Suno のみ） | suno-helper 用 server 起動と疎通確認まで行う | `http://localhost:<PORT>` |
...
3. **音楽 skill を順番に処理**:
   - Suno: `/suno <theme>` を Skill ツールで実行する。`/suno` 呼び出し前提条件チェック（`config/skills/suno.yaml::genre_line` または `data/video_analysis/<slug>/*.json`）を満たさない場合は `/suno` を起動せず、`bunx tayk video-analyze --source benchmark --channel <slug> --top 5` を案内して停止する。AI が `genre_line` を手書きで埋めて続行することは禁止
...
`/suno` が生成した `20-documentation/suno-prompts.json` を Chrome 拡張へ配信するため、`bunx tayk collection-serve` を **dir mode + 拡張 origin lock** で起動する。これは `/wf-new` の責務に含める。Suno UI での連続生成、playlist 追加、ZIP 一括 DL は引き続き `/suno-helper` の user 操作に委ねる。
...
3. **バックグラウンド起動**:
   ...
   PORT=7873
   mkdir -p .tmp/logs
   nohup bunx tayk collection-serve "$CHANNEL_DIR/collections/planning" \
