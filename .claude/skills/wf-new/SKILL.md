...
1. **Skill ツールで `/ideate` を実行** — 日次収集データ + ベンチマークを基に分析 + 5つの企画候補をプレビューサムネイル付きで生成

`/ideate` の出力（5候補 A〜E）が表示された後、**ユーザーに企画選択を求める**:
- 選択肢: A〜E のいずれか
- トラック数: 24（固定。4パターン × 3回生成 × 2曲）
- **音楽エンジン**: Suno（固定。`channel_config.json` の `lyria._disabled: true` のため）
- **ここでフローが一時停止し、ユーザーの入力を待つ**（企画選択のみ。トラック数・エンジンは質問しない）
