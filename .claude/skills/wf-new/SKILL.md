...
- **停止点**: user 入力で止めるのは原則として (1) 企画選択 (2) サムネイル承認。thumbnail の `mode: full` では (2) を省略する。`ttp_mode: false` の minimal mode のみ企画前にテーマ / ジャンル / 雰囲気の直接入力確認を追加し、`true` の minimal mode は `/benchmark` の案内で停止する
