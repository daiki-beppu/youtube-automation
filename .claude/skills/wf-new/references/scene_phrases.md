...
`/wf-new` の Phase 2 ステップ 2b から自動で呼ばれるが、以下のケースでは単体実行する:
...
- メタデータ生成: `application.metadata.service.BAHMetadataGenerator._load_scene_phrases` が `workflow-state.json` から読み込んでタイトル・localizations を生成する
