...
2. 並列 A 完了後:
   ...
   - HTML生成直後、メイン（親 orchestrator）は `descriptions.html` の絶対pathをユーザーが開けるMarkdown linkで提示し、title、概要欄、tag、localizationを確認対象として要約する。自動進行でもこのlinkと要約を完了報告へ残す。`skip_upload_approval = false` では、この概要欄確認を後述のアップロード承認ゲート 3-B に含め、確認完了まで `assets.description` と phase を更新しない
...
3. **アップロード承認ゲート 3-B（`skip_upload_approval = false` のとき）**:
   ...
   - full review PASS 後、ユーザーに公開方法を提示する前に必ず `uv run yt-upload-collection --plan [-c <collection-name>] [--allow-duration-outside-target]` を実行し、`config/schedule_config.json` / `config/channel/youtube.json` を反映した実際の公開タイミングを確定する
