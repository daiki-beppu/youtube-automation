...
- **`yt-thumbnail-correlate --metric views`**: サムネ画像の特徴量 (brightness/contrast/saturation/dominant_hue/colorfulness) と CTR/views/engagement の Pearson 相関。**動画別 CTR は YouTube Analytics API 仕様上取得不可** (`videoThumbnailImpressions*` は `dimensions=video` と組み合わせ不可) のため、`--metric ctr` は既存の `click_through_rate` フォールバックが効かない限り動作しない。実運用では `--metric views` / `engagement` をデフォルト選択すること。次回サムネ制作の方向性。
