...
collection 型では、アップロード実行前にユーザーに公開方法を提示するための公開タイミングを必ず確定する。release 型（単曲リリース）では `uv run yt-upload-collection --plan` を使わず、`uv run yt-upload-auto` の公開時刻決定に従う。
...
- [ ] plan 結果が `📅 公開設定: 非公開でアップロード（即時公開は行いません）` の場合、予約設定または YouTube Studio での手動公開を案内する
- [ ] plan 結果が `📅 公開設定: 限定公開 (unlisted)` / `📅 公開設定: 非公開 (private)` の場合は、その公開範囲を明示した
...
- [ ] Studio の「チェック」が完了するまで待った
- [ ] 「制限」欄で Content ID の申し立て有無を確認した
- [ ] 申し立てがある場合、申立人・対象曲・対象区間を控え、公開 / 異議申し立て / 差し替えを選択した
- [ ] 予約公開の場合、確認時間を確保し、未確認なら公開予定を延期した
