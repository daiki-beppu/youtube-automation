...
### 1. 企画段階（planning/）
1. `/wf-new` で分析フェーズを開始
2. `/status` → `/collect` → `/analyze` でチャンネル分析
3. `/ideate` で企画候補生成（プレビューサムネイル付き）→ ユーザーがテーマを選択
4. テーマ確定後にディレクトリ作成・`workflow-state.json` 初期化
5. `/thumbnail` で本番サムネイルプロンプト生成・`generate_thumbnail.py` で画像生成 → `10-assets/thumbnail.jpg`
...
2. `/description <path>` で概要欄作成
3. `/upload <path>` で YouTube アップロード実行
...
- [ ] ハッシュタグ確認（`channel_config.json` の `descriptions.hashtags` 参照）
- [ ] YouTube 投稿設定
- [ ] プレイリスト設定
