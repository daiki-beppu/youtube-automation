...
`/analyze` と `/benchmark` は独立・並列で鮮度判定（stale 検出）、`/persona` と `/viewing-scene` は
存在チェックのみ（更新タイミングは戦略判断のため人間が決める）。

- `/analyze` が未生成 or stale → ユーザーに `/analyze` 実行を案内（必要なら `/collect` 先行）。**自動呼び出し不可**（AI 推論コスト発生のため）
- `/benchmark` が stale → Skill ツールで実行（内部で差分更新）
- `/persona` が未生成 → ユーザーに案内（更新タイミングは戦略判断のため人間が決める）
...
- `reports/analysis_*.md` が存在しない → **即中断**。ユーザーに `/collect` → `/analyze` の先行実行を案内
- `reports/` が stale（最新 `data/analytics_data_*.json` のファイル名日付より古い）→ 中断。`/analyze` 再実行を案内
