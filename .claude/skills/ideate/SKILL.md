...
Phase 1-2 に入る前に benchmark / persona / viewing-scene の状態を**上から順に**確認する。
`/benchmark` は鮮度判定（stale 検出）、`/persona` と `/viewing-scene` は
...
`references/freshness-rules.md` を参照。stale または未生成を検出したら Phase 1-2 を中断して該当スキルの実行を促すこと。
