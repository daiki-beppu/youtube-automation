...
Phase 1-2 に入る前に benchmark / persona / viewing-scene の鮮度を**上から順に**確認する。
順序依存（persona はベンチマーク由来、viewing-scene は persona 由来）があるため並列実行は禁止。

- `/benchmark` が stale → Skill ツールで実行（内部で差分更新）
- `/persona` が未生成 or stale → ユーザーに案内（自動呼び出し不可。ペルソナ選択で AskUserQuestion が必要）
- `/viewing-scene` が未生成 or stale → ユーザーに案内（シーン選択で AskUserQuestion が必要）

判定ルール（mtime 比較の詳細・擬似コード・workflow-state との同期）は
`references/freshness-rules.md` を参照。stale を検出したら Phase 1-2 を中断して該当スキルの実行を促すこと。
