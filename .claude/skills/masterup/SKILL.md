...
動作パラメータは skill-config (`.claude/skills/masterup/config.default.json`) で管理。
チャンネル側で上書きする場合は `config/skills/masterup.json`:
...
| `bunx tayk generate-master --shuffle` | 連結前に MP3 リストをシャッフル（OS entropy で seed 自動生成、text 出力では `shuffleSeed: <N>`、JSON 出力では `shuffleSeed` を出力） | `bunx tayk generate-master --shuffle` |
| `bunx tayk generate-master --shuffle-seed N` | シャッフル順を固定（`--shuffle` を暗黙有効化、再現性検証用） | `bunx tayk generate-master --shuffle-seed 42` |
| `bunx tayk generate-master --pin-first FILE` | 先頭固定する MP3 ファイル名を順番指定（複数指定は `--pin-first` を繰り返す。`--shuffle` 併用時も pin の順序は保持） | `bunx tayk generate-master --pin-first 00-hook.mp3 --shuffle` |
...
| (skill-config) `audio.target_duration_min` | CLI フラグ未指定時のデフォルト目標尺（分）。`config/skills/masterup.json` で設定 | `"target_duration_min": 120` |
...
2. **本 SKILL.md の Step 2 / Step 3 を書き換え**、WebFetch + CDN curl の経路を `yt-suno-fetch` 呼び出しに置換する。skill-config の `suno_download.cdn_url_template` は deprecated として `config.default.json` に deprecation note を残し、しばらくは「API 障害時の緊急 fallback」として併存させる。
...
bunx tayk generate-master --pin-first 00-hook.mp3 --pin-first 01-theme.mp3 --shuffle  # 複数曲を順番固定
...
**シャッフル時の注意**: `--shuffle` はループ展開の**前**に 1 回だけ実行され、シャッフルされた順序がループごとに同じ並びで N 回繰り返される（ループごとに独立してシャッフルし直すわけではない）。再現性が必要な場合は `--shuffle-seed N` を指定するか、`--shuffle` 単独実行時に text 出力の `shuffleSeed: <N>`、または `--json` 出力の `shuffleSeed` を控えておけば後で同じ並びを再現できる。

**先頭固定時の注意**: `--pin-first FILE` は 1 flag = 1 file として指定し、複数曲を固定する場合は `--pin-first` を繰り返す。指定順を保持して先頭に固定する。`--pin-first-count N` は `02-Individual-music/` のソート済み先頭 N 件を固定する。両者は mutually exclusive（同時指定で validation error）。`--shuffle` 併用時は pin された曲は順序固定のまま、残りのみシャッフルされる（要件: retention に強いフック曲を冒頭に置きつつ後半の類似イントロクラスタ化を回避）。`--target-duration` / `--loop` 併用時もループ展開の前段で先頭固定処理が適用される。`--pin-first` 指定ファイルが `02-Individual-music/` に存在しない場合は fail-loud で停止する。スキル設定 `audio.pin_first_count` を `1` 以上にしておけば、CLI フラグなしでもチャンネル単位のデフォルトとして自動適用される。
...
**CLI 単体実行は引き続き残す**: バッチ補修や CI から呼ぶ用途では `/masterup` を経由せず `bunx tayk fix-timestamps` を直接実行できる。
