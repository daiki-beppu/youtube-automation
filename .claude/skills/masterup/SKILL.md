...
| `audio.target_duration_min` | (未設定) | `tayk generate-master` で `--loop` / `--target-duration` 両方未指定時に `--target-duration MIN` 相当のデフォルトとして採用される目標尺（分）。チャンネル単位で「最低尺」を宣言したいときに設定する |
| `audio.shuffle` | `false` | `tayk generate-master` で CLI `--shuffle` / `--shuffle-seed` 未指定時に `--shuffle` 相当のデフォルトとして採用される。Suno で同一プロンプトから生成した類似イントロ群がマスター後半で連続するのを避けたいときに `true` にする |
...
| `audio.pin_first_count` | `0` | `tayk generate-master` で CLI `--pin-first` / `--pin-first-count` 未指定時に `--pin-first-count N` 相当のデフォルトとして採用される。ソート済み先頭 N 件を順序固定する（`audio.shuffle: true` と併用時は残りだけシャッフル）。`0` = 固定なし。retention に強い 1 曲を冒頭に置きたいときに `1` 以上を設定する |
...
マスター音源生成は `tayk generate-master` CLI が同じ skill-config を読み込むため、`audio.crossfade_duration` は実音声のクロスフェードと metadata_generator のタイムスタンプ計算で常に一致する。
...
| `tayk generate-master --loop N` | マスター生成時に全トラックを N 回繰り返して結合 | `tayk generate-master --loop 3` |
| `tayk generate-master --target-duration MIN` | 目標尺 (分) 以上になる最小ループ回数を自動算出 | `tayk generate-master --target-duration 150` |
| `tayk generate-master --shuffle` | 連結前に MP3 リストをシャッフル（OS entropy で seed 自動生成、text 出力では `shuffleSeed: <N>`、JSON 出力では `shuffleSeed` を出力） | `tayk generate-master --shuffle` |
| `tayk generate-master --shuffle-seed N` | シャッフル順を固定（`--shuffle` を暗黙有効化、再現性検証用） | `tayk generate-master --shuffle-seed 42` |
| `tayk generate-master --pin-first <files...>` | 先頭固定する MP3 ファイル名を順番指定（`--shuffle` 併用時も pin の順序は保持） | `tayk generate-master --pin-first 00-hook.mp3 --shuffle` |
| `tayk generate-master --pin-first-count N` | ソート済み先頭 N 件を固定（`--shuffle` 併用時は残り N+1〜末尾のみシャッフル） | `tayk generate-master --pin-first-count 1 --shuffle` |
...
### フォールバック運用（手動ダウンロード → `tayk generate-master` 直叩き）
...
3. `tayk generate-master`（または `--target-duration` / `--shuffle` などのオプション付き）を **直接実行**
...
3. **件数突合チェック（必須・silent な取りこぼし禁止）**:
   ...
   - **不一致なら処理を中断**し、ユーザーへ次のメッセージで報告する:
     ...
     > 対処: (a) プレイリストを 50 曲以下に分割して再実行 / (b) 全件を手動で `02-Individual-music/` に揃えてから `tayk generate-master` を直接実行
...
**取得手段のフォールバック方針**: WebFetch は suno.com のサーバー描画分（上限 50 件）しか拾えないため、本 skill は「50 曲以下のプレイリスト」を前提運用とする。50 曲超のプレイリストは現状 50 曲単位に分割して個別実行するか、手動で `02-Individual-music/` に MP3 を揃えてから `tayk generate-master` を直接実行するワークフローへ切り替える（内部 API / 公式 API への移行は別 issue）。
...
ダウンロード完了後、`tayk generate-master` CLI でマスター音源を自動生成:

```bash
tayk generate-master                          # CWD がコレクションディレクトリのとき
tayk generate-master <collection-path>        # 明示指定
tayk generate-master --loop 3                 # 全トラックを 3 回繰り返して結合
tayk generate-master --target-duration 150    # 150 分以上になる最小ループ回数を自動算出
tayk generate-master --shuffle                # ループ展開前にトラック順をランダム化
tayk generate-master --shuffle-seed 42        # 再現性 seed 指定（--shuffle を暗黙有効化）
tayk generate-master --pin-first 00-hook.mp3 --shuffle           # 指定 1 曲を先頭固定 + 残りシャッフル
tayk generate-master --pin-first-count 1 --shuffle               # ソート済み先頭 1 件を固定 + 残りシャッフル
```
...
`tayk generate-master`（ffmpeg クロスフェード結合）は **30 秒〜2 分** 程度かかる。**必ず Bash ツールを `run_in_background=true` で起動する**。これによりユーザーは処理中も同じセッションで質問できる（Claude Code は完了時に自動でメッセージ通知するため、`sleep` ループや `until` での自前ポーリングは禁止）。

spawn 例:

```bash
tayk generate-master > /tmp/masterup-$(date +%s).log 2>&1
```
