...
- 推奨: コレクションの `main.png` (PNG, 1280×720 以上)。コレクションルートから `10-assets/main.png`
...
- セグメントごとに異なる画像を使うこともできる（呼び出しごとに `reference_image` を切り替え）。ストーリー性のある曲構造を作りたい場合に有効
...
特定セグメントのみ vocal にしたい場合は、その API リクエストだけ `mode: "vocal"` + `lyrics: "..."` を指定して呼び出す。
