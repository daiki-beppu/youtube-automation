...
>   (b) 指定した安全な skill だけ同期 (apply --sync-only で skills asset の allowlist 指定。claude-md は通常どおり同期)
>   (c) 中止して手動でマージ
> a / b / c を返してください。
```
...
`"manual"` が返ってきた場合は、自スキルは利用者に手動マージを依頼する。他に上書きしてよい skill が明確な場合だけ、Step 3-2 の (b) `--sync-only <safe-skill...>` でその skill だけ同期する（`--sync-only` は skills asset の除外指定ではなく allowlist 指定。claude-md は別 asset として通常どおり同期する）。
...
```bash
uv run yt-automation-update apply --tag v<target>                    # tag pin。Step 2-4 で同意済みの tag を必ず固定
uv run yt-automation-update apply --tag v<target> --force-sync       # tag pin + (a) 上書き
uv run yt-automation-update apply --tag v<target> --sync-only <skill1> <skill2> ...  # tag pin + (b) 指定した安全な skill だけ同期（claude-md は通常どおり同期）
...
4. `yt-skills sync --force`（デフォルト `--asset all` = skills / claude-md / docs ほか全 asset。`--force-sync` は CLI 内の local fix 差分ガードを承認済み上書きとして通過する。`--sync-only` 時は指定した安全な skill だけを skills `--only` で同期し、その後 claude-md も `--force` で同期する）
5. smoke check: `yt-skills list` / `yt-config-migrate verify --target <repo>`
