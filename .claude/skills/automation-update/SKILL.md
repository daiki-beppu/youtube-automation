...
print_channel_repo_guidance() {
  ...
  for root in "$HOME/02-yt" "$HOME/01-yt" "$HOME"; do
    ...
    while IFS= read -r pyproject; do
      ...
      if grep -q 'youtube-channels-automation' "$pyproject" \
        && ! grep -qE '^\s*name\s*=\s*"youtube-channels-automation"' "$pyproject"; then
        echo "  cd $repo_dir"
        found=1
      fi
  ...
  if [ "$found" -eq 0 ]; then
    ...
    echo "  find \"$HOME\" -maxdepth 4 -name pyproject.toml -print | xargs grep -l 'youtube-channels-automation'"
