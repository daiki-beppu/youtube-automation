...
uv run yt-automation-update apply --tag v<target> --sync-only <skill1> <skill2> ...  # tag pin + (b) local fix 解消後に指定 skill だけ同期（claude-md も同期）
...
6. smoke check: `yt-skills list` / `yt-config-migrate verify --target <repo>`
