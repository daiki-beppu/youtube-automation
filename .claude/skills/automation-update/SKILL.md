...
PY
if [ $? -ne 0 ]; then
  echo "❌ python3 は tomllib (Python 3.11+) または tomli が必要"
  exit 1
fi

classify_automation_pyproject() {
  local pyproject="$1"
...
target = "youtube-channels-automation"
...
if isinstance(project_name, str) and canonicalize_name(project_name) == target:
    print("upstream")
    raise SystemExit(0)
...
for dependency in dependencies:
    ...
    if match and canonicalize_name(match.group(1)) == target:
        print("dependency")
        raise SystemExit(0)
...
is_channel_automation_dependency_pyproject() {
  [ "$(classify_automation_pyproject "$1")" = "dependency" ]
}

is_upstream_automation_pyproject() {
  [ "$(classify_automation_pyproject "$1")" = "upstream" ]
}
...
"
  ...
  if [ "$found" -eq 0 ]; then
    ...
    echo "  各 pyproject.toml の [project].dependencies に package name が youtube-channels-automation の依存があるものを選んでください（-, _, . は同一扱い）。"
...
if is_upstream_automation_pyproject pyproject.toml; then
