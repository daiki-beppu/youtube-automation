...
5. `yt-skills sync --force`（デフォルト `--asset all` = skills / claude-md / docs / settings ほか全 asset。settings の新規 hook は全文 preview 後に人間が承認した場合だけ `apply --accept-hooks` で追加し、省略時は allow / deny の merge のみ継続する。`--sync-only` 時は指定した安全な skill だけを skills `--only` で同期し、その後 claude-md も `--force` で同期する）
