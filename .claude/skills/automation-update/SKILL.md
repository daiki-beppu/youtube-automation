...
is_channel_automation_dependency_pyproject() {
  local pyproject="$1"
  grep -q 'youtube-channels-automation' "$pyproject" \
    && ! grep -qE '^\s*name\s*=\s*"youtube-channels-automation"' "$pyproject"
}
...
print_channel_repo_guidance() {
  ...
  for root in "$HOME/02-yt" "$HOME/01-yt" "$HOME"; do
    ...
    done < <(find "$root" -maxdepth 4 -name pyproject.toml -not -path '*/.venv/*' -not -path '*/.git/*' 2>/dev/null | head -50)
  done

  if [ "$found" -eq 0 ]; then
    echo "  自動検出できませんでした。以下でチャンネルリポジトリを探してください:"
    echo "  find \"$HOME\" -maxdepth 4 -name pyproject.toml -not -path '*/.venv/*' -not -path '*/.git/*' -print | while read -r pyproject; do"
    echo "    grep -q 'youtube-channels-automation' \"\$pyproject\" && ! grep -qE '^\s*name\s*=\s*\"youtube-channels-automation\"' \"\$pyproject\" && dirname \"\$pyproject\""
    echo "  done"
...
if [ ! -f pyproject.toml ] || ! grep -q 'youtube-channels-automation' pyproject.toml; then
  print_channel_repo_guidance
  exit 1
fi
