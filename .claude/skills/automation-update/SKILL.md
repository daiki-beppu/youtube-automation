...
uv run yt-automation-update apply --tag v<target> --sync-only <skill1> <skill2> ...  # tag pin + (b) 指定した安全な skill だけ同期
...
4. `yt-skills sync --force`（デフォルト `--asset all` = skills / claude-md / docs ほか全 asset。`--force-sync` は CLI 内の local fix 差分ガードを承認済み上書きとして通過する。`--sync-only` 時は指定した安全な skill だけを skills `--only` で同期し、claude-md は同期しない）
