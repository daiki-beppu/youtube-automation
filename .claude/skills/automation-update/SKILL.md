...
| 1 | 差分あり（要追従） | 出力から pin 形式・現在（`<old>`）・最新 tag（`<target>`）を控えて Step 1-2 へ |
...
| **main 追従**（tag 無し / `branch = "main"`） | `uv.lock` の解決済み sha と upstream HEAD を比較 |
| **sha pin**（`rev = "<40 桁>"`） | 常に exit 1（自動判定対象外）。`[HUMAN STEP]` で bump 先を確認 |
...
選択に応じて `gh api repos/daiki-beppu/youtube-automation/git/refs/tags/v<target> --jq .object.sha` 等で sha を解決し、Phase 3 の `apply --rev <sha>` に使う。
...
>
> 対応を選んでください:
>   (a) upstream 版で上書き (apply --force-sync) — local fix は破棄されます
>   (b) 該当 skill だけスキップして他を同期 (apply --sync-only で個別指定)
>   (c) 中止して手動でマージ
> a / b / c を返してください。
```
...
`"manual"` が返ってきた場合は、Step 3-2 の (b) `--sync-only` で `automation-update` を除外して他スキルだけ sync し、自スキルは利用者に手動マージを依頼する。
...
`(b)` が選ばれた場合は通常の (a)/(b)/(c) 分岐へ進む（今回は直接編集を維持し、Step 3-2 で `--sync-only` 除外または `--force-sync` 維持を選ばせる）。
...
```bash
uv run yt-automation-update apply                                    # local fix 差分なし
uv run yt-automation-update apply --force-sync                       # (a) 上書き
uv run yt-automation-update apply --sync-only <skill1> <skill2> ...  # (b) 個別同期（引数は空白区切り）
...
4. `yt-skills sync`（デフォルト `--asset all` = skills / claude-md / docs ほか全 asset。`--sync-only` 時は skills `--only` + claude-md の 2 段で実行される）
