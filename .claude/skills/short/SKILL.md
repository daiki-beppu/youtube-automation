...
- **Canonical ソース**: `config/localizations.json` の `supported_languages` + `default_language`（v2 で単一 Canonical ソース）
...
（{duration} は `config/channel/audio.json` の `audio.target_duration_min` を60で割った値）
