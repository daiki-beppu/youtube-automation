...
- 本編動画のアップロードが完了した後（`/video-upload` 完了後の翌日以降）
...
いずれも無い場合はマスター動画フォールバックは**提供しない**。素材を揃えてから再実行すること（ループ動画生成: `uv run yt-shorts-generate-loop <collection-path> -y`）。
...
```bash
uv run yt-shorts-upload <collection-path>
# ドライラン（メタデータ確認のみ）
uv run yt-shorts-upload <collection-path> --dry-run
```
...
```bash
for i in 1 2 3; do
  uv run yt-shorts-upload <collection-path> --short-num $i
done
```
...
→ `/channel-status` で Analytics 監視を開始
