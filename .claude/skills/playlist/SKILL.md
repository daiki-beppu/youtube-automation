...
`bunx tayk playlist-status` と `bunx tayk playlist-manager` を 1 スキルに統合し、プレイリストの状態確認・作成・動画割り当て・クリーンアップを案内する。
...
| モード | コマンド | 説明 |
|--------|---------|------|
| **status** | `bunx tayk playlist-status` | 全プレイリストの ID・動画数・マッチングルールを表示（読み取り専用） |
| **init** | `bunx tayk playlist-manager --init [--dry-run]` | `playlists.json` 定義の全プレイリストを作成 + live/ 配下の全動画を割り当て |
| **assign** | `bunx tayk playlist-manager --assign VIDEO_ID --theme THEME [--dry-run]` | 単一動画を該当プレイリストに追加（テーマからマッチング） |
| **clean-deleted** | `bunx tayk playlist-manager --clean-deleted [--dry-run]` | 全プレイリストから削除済み/非公開動画のエントリを除去 |

`bunx tayk playlist-manager --status` も同じ `PlaylistStatusViewer` に委譲するため、`bunx tayk playlist-status` と等価。
...
```bash
bunx tayk playlist-manager --init --dry-run    # まずプレビュー
bunx tayk playlist-manager --init              # 実反映
```
...
```bash
bunx tayk playlist-manager --assign <VIDEO_ID> --theme <THEME>
```
...
```bash
bunx tayk playlist-manager --clean-deleted --dry-run    # 影響範囲をプレビュー
bunx tayk playlist-manager --clean-deleted              # 実反映
```
