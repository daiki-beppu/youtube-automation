...
# リポジトリルートから実行
export $(grep -v '^#' .env | xargs) && \
uv run yt-shorts-generate-loop <collection-path> -y
```
...
- **CTA の尺**: `config/channel/audio.json` の `audio.target_duration_min` を参照して正確な時間を入れる（例: 120分 → `Full 2-hour collection`）
- **コスト**: 画像 1 枚あたりのコストはチャンネル側の cost 設定（`config/skills/*` および `audio_costs.json` 集計）を参照（旧版でハードコードしていた値はリポジトリ全体で禁止）。ループ動画は Veo 3.1（別途）
