...
| API | call 数 / 実行 | 変動要因 |
|---|---|---|
| Vertex AI Gemini（yt-generate-image --aspect-ratio 9:16） | short.png 生成 = 1 call | 再生成回数 |
| Vertex AI Veo 3.1（yt-generate-shorts-loop） | short-loop.mp4 生成 = 1 call | ループ動画化まで行う場合のみ。不要なら 0 |

- 上限 / 承認: 画像生成は confirm_cost の y/N 確認、Veo は [y/N] プロンプトで承認する（いずれも `-y` でスキップ可）。
