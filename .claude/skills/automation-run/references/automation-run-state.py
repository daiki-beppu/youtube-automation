#!/usr/bin/env python3
"""Resolve `/automation-run` actions and maintain its lease/history state."""
        ...
        if lock_dir.exists():
            ...
            try:
                ...
                if not isinstance(expires_at, (int, float)) or isinstance(expires_at, bool) or expires_at > now:
                    raise LeaseBusyError("別の automation-run が実行中です")
                shutil.rmtree(lock_dir)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "acquire":
            ...
            result = evaluate_collection(root, collection, _load_runner_config(root))
        else:
            collection = select_collection(root, args.collection)
            record_attempt(
                root,
