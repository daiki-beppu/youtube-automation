...
**AI は `genre_line` を手書きしてはならない。** 方向性は必ずスクリプト（`bunx tayk video-analyze` の `suno_preset` 出力）由来とする。`/suno` 実行に入る最初のステップとして以下を機械的に判定し、要件を満たさない場合はパターン設計に進まず中断する:
...
2. `data/benchmark_*.json` 取得済みなら: `bunx tayk video-analyze --source benchmark --channel <slug> --top 5`
...
5. `bunx tayk generate-suno` 実行で検証（entry 数不一致・name 重複は fail-loud）
