...
コレクション用の SunoAI V5.5 音楽プロンプトを YAML で定義し、スクリプトで最終プロンプトを生成する。**インストゥルメンタル / ボーカル（歌詞あり）両モード対応**。歌詞ありの場合、`/suno` は orchestration として `/suno-lyric` → Style 設計 → `suno-prompts.json` 結合までを進める。
...
- **ボーカルモード**: 先に `/suno-lyric` で歌詞を作成し、本スキルでは Style / 情景 / タイトル設計と `suno-prompts.json` への結合を担当する
...
歌詞あり（ボーカルモード）と判定した場合、`/suno` のプロンプト設計に入る前に `/suno-lyric` の成果物を確認する。`20-documentation/suno-lyrics.json` または `20-documentation/suno-lyrics.md` が無い場合、確認待ちで停止せず、先に `/suno-lyric` を実行して歌詞を生成・レビューしてから `/suno` に戻る。

`/suno-lyric` の実行は Suno クレジットを消費しない前工程なので、現実のお金のコストが発生しない限りユーザー確認は不要。`/suno-lyric` が失敗した場合だけ `/suno` の続行を止め、歌詞成果物を作れる状態にしてから再開する。
...
2. `data/benchmark_*.json` 取得済みなら `uv run yt-video-analyze --source benchmark --channel <slug> --top 5` を全 benchmark slug で実行
...
**ボーカルモードは先に `/suno-lyric` を実行する。** 本スキル内で歌詞を直接作らない。`suno-lyrics.json` / `suno-lyrics.md` が未生成なら、自動で `/suno-lyric` を先行実行してから `/suno` に戻る。歌詞成果物をレビュー済みにしてから、Style / 情景 / タイトル設計を行う。
...
### ボーカル（歌詞あり）
→ 先に `/suno-lyric` で歌詞を生成・レビュー
→ `/suno` で Style + Lyrics の `suno-prompts.json` を生成
