...
| ボーカル | 必要 pattern 数 ≈ track_count（1 pattern = 1 採用曲。`ceil(N/2)` 不適用）+ `tracks_per_pattern` の再生成設計 | `/suno-lyric` で事前作成 | Custom Mode + **Instrumental OFF** + Lyrics 投入 |
...
- `ceil(track_count / tracks_per_pattern)` も誤り。`tracks_per_pattern` は割り算の分母ではない（次表の注記を参照）。track_count 28 のコレクションなら pattern は 9 個ではなく約 28-30 個必要
- 実績値（vocal 下流チャンネル）: pattern 55 → 最終 55 曲、pattern 40（複数 scene 込み entry 49）→ 最終 49 曲

| キー | 役割 | 既定 |
|---|---|---|
| `pattern_strategy` | `mixed` / `single` | `mixed` |
| `tracks_per_pattern` | パターンあたりの再生成回数（最終採用数には影響しない） | `3` |

- `single`: 1 つの統合情景フレーズにまとめ、同一プロンプトを `tracks_per_pattern` 回生成
- `mixed`: 感情の起伏を N 個のパターンに分割（典型 4: 静寂 → 開放 → 親密 → 動き）
- **`pattern_strategy` / `tracks_per_pattern` は 1 パターンあたりの再生成回数の設計であり、最終採用数には影響しない（採用は常に 1 グループ 1 曲）。** `tracks_per_pattern` を増やすと `(Take N)` 展開で同一曲の再生成候補が増えるだけで、コレクションの曲数（= 別々の曲の数 = pattern 数）は増えない。試聴でベストの take を選んだ残りは落選する
- ボーカルの最終 prompt entry name は `tracks_per_pattern` 分だけ展開する。単一 scene は `{name_jp} — {name_en} (Take N)`、複数 scene は `{name_jp} — {name_en} (Variation S) (Take N)` とし、`/suno-lyric` の `suno-lyrics.json` も同じ展開後 name を全件持つ
...
1. **拡張をビルドしてロード**（初回のみ）: リポジトリ root で `npx -y pnpm@9.15.9 -C extensions/suno-helper install --frozen-lockfile --ignore-workspace && npx -y pnpm@9.15.9 -C extensions/suno-helper build` → `mkdir -p ~/chrome-extensions/suno-helper && rsync -a --delete extensions/suno-helper/.output/chrome-mv3/ ~/chrome-extensions/suno-helper/` → Chrome で `chrome://extensions` → `~/chrome-extensions/suno-helper/` を選択。pnpm 9.15.9 固定の理由と release 前検証は `extensions/README.md::pnpm バージョン契約` を参照
