...
`thumbnail.jpg` は YouTube アップロード用のテキスト付きサムネイルであり、`/loop-video` の入力には使わない。`/thumbnail` で承認済み `thumbnail.jpg` から AI 再生成したテキストなし `main.png` または `main.jpg` を入力にする。
...
| コマンド | 説明 |
|---------|------|
| `uv run yt-generate-loop-video <collection-path> -y` | 指定コレクションでループ動画生成（既存 `loop.mp4` は `loop-v{n}.mp4` に退避して **Veo 再課金**） |
...
|---|---|---|---|---|
| `yt-generate-loop-video <path> -y` （素） | `loop-v{n}.mp4` に退避 | **あり**（新規生成） | あり | **フル再課金** |
...
- **本気で作り直したいとき**: 素の `yt-generate-loop-video <path> -y` を再実行。既存は `loop-v{n}.mp4` に自動退避されるため上書き事故はない。ただし **必ず Veo 再課金が発生する**ので意図的に行うこと。
...
- `10-assets/main.png` または `main.jpg` が存在すること（`/thumbnail` で承認済み `thumbnail.jpg` から AI 再生成したテキストなし動画背景）
...
2. **対象確認**: `10-assets/` にテキストなし `main.png` or `main.jpg` があることを確認。文字入り `thumbnail.jpg` しか無い場合は `/thumbnail` に戻ってテキストなし版を再生成する
