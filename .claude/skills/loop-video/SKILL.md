...
- 生成・リトライ・`--smooth` の各処理が成功するたび、`10-assets/loop.mp4` を**動画として再生できるプレビュー**で開く。静止画やキーフレームだけを提示して代替してはならない
- `/thumbnail` と同じ順序で、成果物を開いた後に `AskUserQuestion` の明示 2 択「承認する」/「受理せず修正する」を提示する
- ユーザーに承認されるまで `/videoup` などの後工程へ進まず、完了報告もしない
...
- 上限 / 承認: 実行前に [y/N] プロンプトで承認する（`-y` でスキップ）。`enabled: false` のチャンネルでは fail-loud で停止し Veo を叩かない。
...
5. **確認・プレビュー承認ゲート**: 生成された `loop.mp4` の絶対パスを解決して動画プレイヤーで開く。Codex アプリでは同じ絶対パスを動画として会話内に表示してもよい

   ```bash
   LOOP_VIDEO_PATH="$(cd <collection-path>/10-assets && pwd)/loop.mp4"
   open "$LOOP_VIDEO_PATH"
   ```

   静止画、先頭フレーム、サムネイルだけでは合否を判定できないため、必ず動画として再生する。ユーザーには次のチェック項目を提示する:

   - [ ] 再生開始後、時間経過で被写体または背景が実際に動く（静止画 fallback ではない）
   - [ ] 意図した対象だけが自然に動き、固定対象の数・形・構図が保たれる
   - [ ] 末尾から先頭へ繰り返し再生してもループの継ぎ目が不自然でない

   プレビューを開いた後、`AskUserQuestion` で `/thumbnail` と同じ並びの明示 2 択を提示する:

   1. **承認する** — 現在の `loop.mp4` を受理し、完了できる
   2. **受理せず修正する** — 修正フローへ戻り、後工程には進まない

6. **不受理時の修正 → 再プレビュー**: 受理しない場合は、観測した問題から次の経路へ戻す
...
| `omni.model` | gemini-omni-flash-preview | Gemini Developer API の Omni モデル名 |
| `omni.timeout_seconds` | 600 | Interactions / Files API の完了待ち上限（秒） |
| `omni.poll_interval_seconds` | 5 | URI delivery の Files API state 確認間隔（秒） |
...
- [Gemini Omni Flash video generation (Google AI for Developers)](https://ai.google.dev/gemini-api/docs/omni)
