...
`10-assets/loop.mp4` が生成され、`skip_preview_approval: false` ではユーザーの品質確認（「ステップ」5-6）を通過したとき、`true` では生成成功とファイル存在を確認したとき完了とする。skip 時は自動再生成や `--smooth` を行わず、生成物をそのまま確定する。
...
5. **確認・プレビュー承認ゲート**: `skip_preview_approval: true` なら生成された `loop.mp4` の存在を検証してそのまま確定し、Step 6 の自動修正は行わず後工程へ進む。`false`（既定）の場合だけ、絶対パスを解決して動画プレイヤーで開く。Codex アプリでは同じ絶対パスを動画として会話内に表示してもよい
