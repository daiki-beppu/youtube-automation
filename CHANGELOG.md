...
- `refactor(packaging)`: 旧 `cli/` 16 files と `scripts/` 64 files を `commands/{analytics,channel,collections,distrokid,media,metadata,suno,system,thumbnail,uploads,youtube}/` の 11 domain へ移し、`cli_entrypoints.py` を `entrypoints.py` へ改名。console script を 76→75 とし、`yt-distrokid-migrate` を entry / wrapper / module / test / CLI mapping / `NON_BILLED_CLIS` の 6 面から削除（DistroKid domain・extension・`yt-distrokid-prepare` は維持）。20 個の skill symlink は link path を維持したまま 14 個の新 command target へ張り替えた。あわせて `--help` が `CHANNEL_DIR` 未設定でも exit 0 で返るよう、`yt-channel-status` / `yt-playlist-manager` / `yt-playlist-status` / `yt-suno-unattended-request` / `yt-upload-auto` / `yt-upload-collection` の設定解決を引数解析より後ろへ移動し（help 表示専用に `configuration.channel_label()` を追加）、`yt-metadata-audit` のモジュールトップ `COLLECTIONS_DIR` 即時評価を解消。`yt-skills` の repo root 解決を深さ index から `youtube_automation` パッケージ基準へ変更し、移動のたびに壊れる／上位ディレクトリを誤認する事故を防いだ。この `--help` 契約は `tests/test_cli_help_contract.py` で機械担保する（全 75 console script の実バイナリを `CHANNEL_DIR` 未設定・credential 欠落の隔離 subprocess で起動し、exit 0 / `usage: <script 名>` / 認証・API client 生成の未呼び出し / 隔離 HOME への非書き込み / 15 秒以内を検証。認証呼び出しの観測は `PYTHONPATH` 注入の `sitecustomize` プローブで行い、実プロセスを 75 本起動するため slow レーンに分類）（#2308）。
