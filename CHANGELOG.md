...
- `fix(ts-rewrite/generate-master)`: review 指摘を受け、`masterup.yaml` fallback で既存 `audio.finalize.*` 形状を読めるようにし、inline / scalar `audio` は config error に統一した。`ffmpeg` の bitrate 指定では `-b:a` と `-q:a` の併用をやめ、`generate-master` public subpath から CLI 用 path resolver を外し、`yt` alias の実行 smoke と関連回帰テストを追加した（#772）
