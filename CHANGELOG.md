...
- `feat(ts-rewrite/cli)`: Python `yt-playlist-manager` 相当の playlist CRUD を ADR-0003/0004 準拠で TS 移植した（#762）。core に `playlists` service（schema / service / registry entries）を追加し、CLI は `tayk playlist` subcommand（status / create / assign / sync / clean-deleted / init）から registry 経由で呼び出す。
