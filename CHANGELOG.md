...
- `#772`: Python `yt-generate-master` 相当の導線は TS dispatcher の `tayk generate-master <collection-dir> [--json]` に移行する。per-CLI bin は追加せず、core registry entry `master.generate` 経由で `01-master/master.{mp3,wav}` を生成する。
