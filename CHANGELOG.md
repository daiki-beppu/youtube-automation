...
- `refactor(setup)`: 排他的な `/setup --channel` を追加し、旧 `/channel-new` 新規開設 Step 1〜10 と opening-only references / helpers の owner を setup へ移した。取り込み・分析・方向性検討・再生成・設定 push と共有 branding/config 資産は residual `/channel-new` に維持する（#3983）。
