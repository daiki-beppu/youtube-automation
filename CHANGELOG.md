...
- `feat(dashboard)`: デジタル庁のダッシュボード設計ガイドを基準に、初期表示を概況・データ前提・指標定義・チャンネル比較・詳細の判断順へ再構成。比較表に詳細導線を統合し、狭幅では行をカード表示に切り替えて横方向の見切れを解消。ダークテーマの背景を黒からニュートラルグレーの階調へ調整し、カードとの境界を識別しやすくした（#2546）。
