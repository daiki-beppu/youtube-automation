...
- `fix(hooks)`: git worktree / Codex 実行環境で lefthook hook が stale な Nix store 固定パスや PATH 不在により silent skip され得る導線を修正。devShell の shellHook は `lefthook install --force` を毎回実行し、`lefthook` 不在や install 失敗を `|| true` で握りつぶさず明示失敗させる。親 checkout / worktree それぞれの診断・再生成手順も docs に追加した（#1552）
