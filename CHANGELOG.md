...
- `feat(dashboard)`: 全チャンネルの日次推移を再生数・再生時間・登録者純増減・インプレッション数で切り替え、欠測値を補完せず表示できるようにする（#4351）。
...
- `feat(hybrid)`: post-publish のプレイリスト割当・固定コメント・metadata監査をcloud stageで順序付き実行し、OAuth secret、遠隔再検証、Git管理trackingの二重冪等性を追加する（#4062）。
...
- `feat(music)`: worktree/mainのマスター音源候補を標準audio player付き固定HTMLで比較し、共通brokerまたはterminal経路のdigest再検証後だけmasteredへ確定する（#4019）。
...
- `feat(documents)`: 固定名atomic review HTMLと永続HTML表示、product-neutralなsingle-use loopback候補選択broker、明示terminal fallbackを共通化する（#4015）。
...
- `feat(documents)`: skill 生成運用文書の新規作成・Markdown 明示移行・移行済み再更新を、検証済み JSON＋HTML pair の rollback 可能な一操作へ共通化する（#4024）。
...
- `feat(schema)`: リポジトリ所有 JSON Schema を Draft 7 の固定 registry でコンパイル・検証し、analytics insights・skill feedback・weekly vote log の入口で、入力を変更せず JSON pointer 付き domain error を返す（#4022）。
...
- `feat(skills)`: `/audit --video` に旧 `/video-analyze` の Vertex AI（ADC）動画解析とローカル成果物契約を統合し、`audit.video` 設定節・旧 override 互換・移行CLI・全利用者導線を更新する（#3854）。

- `feat(skills)`: `/audit --value-loop` に旧 `/value-loop-audit` の4工程横断診断を読み取り専用のまま統合し、シーン定義・制約翻訳・公開前ゲートの参照先を統合後 mode へ更新する（#3853）。

- `feat(skills)`: `/audit` を新設し、読み取り専用の `--alignment` mode に旧 `/alignment-check` の音楽ムード × サムネ × タイトル整合監査を統合する。外部サービスやチャンネル設定へ反映せず、監査レポートと改善候補だけを生成する（#3852）。
...
- `feat(skills)`: `/video` に排他 `--describe` mode を追加して旧 `/video-description` の Complete Collection 概要欄生成を吸収し、フラグなし chain を `generate` → `describe` に拡張する。設定は `video.yaml::describe` へ名前空間化し、旧 config migration と互換 loader を提供する（#3836）。

- `feat(skills)`: `/video` を新設して旧 `/videoup` のローカル MP3→MP4 生成を排他 `--generate` mode と 1-step chain manifest/state へ移し、旧 skill directory を削除する。生成設定は `video.yaml::generate` へ名前空間化し、下流 `videoup.yaml` の明示 migration と互換 loader key を提供する（#3835）。
...
- `feat(analytics)`: 全 pending 単一変数実験を 1 回の YouTube snapshot で期日・公開状態・最低日齢まで判定し、Decimal の ±20% 境界による judged 遷移と `source=experiment` insight を recoverable な 2 ファイル一体 commit で exactly-once 還流する `yt-experiment judge` を追加した（#3785）。
- `feat(analytics)`: 単一レバーの pending 実験を全 eligible 動画または同一テーマの VPD 中央値と算出根拠付きで、検証済み `data/experiments.jsonl` へ原子的かつ append-only に登録する `yt-experiment register` を追加した（#3784）。
...
- `refactor(collection-ideate)`: `yt-generate-image --help` だけで通常 flag の用途・既定値・制約を確認できるようにし、企画 skill は mode dispatch、コスト承認、TTP strict reference、終了 status・再生成、出力・cleanup 順序の orchestration / safety contract に整理した。既存の execution contract と runtime behavior は変更しない（#3518）。
...
- `docs(thumbnail)`: 配布される `references/prompt-schema.md` から下流で解決不能だった ADR と差分レポートへの 2 pointer を除去し、試験導入・実本番フロー未接続・issue #654 の再評価 gate と 14 項目 mapping / API / 段階移行の契約を reference 内に維持した。upstream docs の wheel 同梱・複製は行わず、thumbnail の実行時の振る舞いは変更しない（#3534）。
- `docs(automation-update)`: 配布される skill / references から下流で解決不能だった changelog input contract と numbered duplicate cleanup への 3 pointer を除去し、既存 Step 2-3 と post-apply checks を配布内の単一ソースとして自己完結させた。upstream docs の wheel 同梱・複製は行わず、CLI behavior・実行順・承認 gate・停止条件は変更しない（#3538）。
- `docs(automation-release)`: 配布される skill / 補助 references から下流で解決不能だった release contract への 4 pointer を除去し、Python / extension release contract と changelog promotion / prepare checklist を配布内で自己完結させた。upstream docs の wheel 同梱・複製は行わず、Python / extension release の既存挙動と #3075 の import audit contract は変更しない（#3542）。
...
- `feat(dashboard)`: collection ごとの workflow step を action・5状態・6時間指標付きで表示し、狭い画面ではカード状に折り返して keyboard でも詳細導線から確認できるようにした（#3379）。
...
- `feat(workflow)`: `config/channel/workflow.json` に optional な `manual_baseline_minutes` を追加し、action 別の手作業基準を非負有限の分単位で設定できるようにした。未設定は推測せず `None` として保持する（#2960, #3250, #3259）。
...
- `fix(system)`: notification・競合 CLI・automation update・settings merge・doctor・preflight の失敗／境界契約を実行テストで固定し、thumbnail routing の 21 件の文書固定テストを provider・reference routing・thumbnail/main 成果物・loop-video on/off・cost 拒否の実行シナリオへ置換。doctor readiness は現行の `composition_rules.text_lines` 契約へ更新した（#2807, #2808, #2809, #2810, #2811, #2812, #2813, #2814, #2815, #2816）。
...
- `refactor(packaging)`: 旧 `cli/` 16 files と `scripts/` 64 files を `commands/{analytics,channel,collections,distrokid,media,metadata,suno,system,thumbnail,uploads,youtube}/` の 11 domain へ移し、`cli_entrypoints.py` を `entrypoints.py` へ改名。console script を 76→75 とし、`yt-distrokid-migrate` を entry / wrapper / module / test / CLI mapping / `NON_BILLED_CLIS` の 6 面から削除（DistroKid domain・extension・`yt-distrokid-prepare` は維持）。20 個の skill symlink は link path を維持したまま 14 個の新 command target へ張り替えた。`agents/` に残っていた `yt-upload-auto` / `yt-upload-collection` / `yt-upload-shorts`、`utils/` の `yt-audio-visualizer-fill`、`infrastructure/auth/` の `yt-oauth` も `commands/{uploads,media,system}/` へ移し、`agents/` パッケージを退役させて、全 75 wrapper が例外なく `commands/<domain>` を target とする状態にした（`tests/test_cli_stdio.py::test_entrypoint_wrappers_only_target_command_modules` が `entrypoints.py` の AST から機械担保する）。あわせて `--help` が設定・skill-config なしで exit 0 を返すよう、`yt-channel-status` / `yt-playlist-manager` / `yt-playlist-status` / `yt-suno-unattended-request` / `yt-upload-auto` / `yt-upload-collection` / `yt-discover-competitors` の設定解決を引数解析より後ろへ移動し、`yt-metadata-audit` のモジュールトップ `COLLECTIONS_DIR` 即時評価を解消。help 文言は静的化した（parser 構築時にチャンネル名を解決する経路を作らない）。`yt-skills` の repo root 解決を深さ index から `youtube_automation` パッケージ基準へ変更し、移動のたびに壊れる／上位ディレクトリを誤認する事故を防いだ。この `--help` 契約は `tests/test_cli_help_contract.py` で機械担保する（全 75 console script の実バイナリを `CHANNEL_DIR` 未設定・credential 欠落の隔離 subprocess で起動し、exit 0 / `usage: <script 名>` / `load_config()`・`channel_dir()`・認証・API client 生成の未呼び出し / 隔離 HOME への非書き込み / 15 秒以内を検証。設定解決まで観測点に含めるのは、`ConfigError` を握り潰す fallback を挟めば「設定なしで exit 0」は保ててしまい、help 文言の設定依存を検出できないため。観測は `PYTHONPATH` 注入の `sitecustomize` プローブで行い、実プロセスを 75 本起動するため slow レーンに分類）（#2308）。
- `refactor(packaging)`: command adapter に残っていた共有ロジックを business layer へ移し、`commands/` を thin adapter に揃えた。benchmark の鮮度更新（`ensure_benchmark_fresh`）と JSON クエリ（`find_latest_benchmark_json` / `load_benchmark_videos`）を `application/analytics/`、DistroKid の disc 列挙を `application/distrokid/`、live / short 判定などの動画ポリシーを `domains/analytics/benchmark.py`、Suno の route 定数群を `domains/suno/downloaded/models.py`、`--target` ディレクトリ解決を `utils/channel_target.py`、setup ディレクトリ契約を `utils/setup_directory_contract.py` へ移設（`application/` / `domains/` / `utils/` から `commands/` への import は 0 件）。`ensure_benchmark_fresh` は collector / analyzer / reporter を Protocol + factory で受け取る形にし、収集結果が空・サムネイル取得失敗のときに JSON 保存とレポート生成へ進まないことを `tests/test_benchmark_collector_no_fallback.py` で固定した。`yt-skills` の repo root 解決は「`pyproject.toml` と `src/youtube_automation` と `.claude/skills` を備え、実行中のパッケージを含む checkout」を探索する方式へ変更し、深さ非依存かつ無関係な親ディレクトリを root と誤認しないことをテストで機械担保（#2308）。
...
- `docs(config)`: 配布 skill・README・認証/GCP 文書からチャンネルルート `.env` の作成・読込手順を除去し、ADC quota project・用途別 location・process env / OAuth file / 1Password の実経路へ統一（#2476）。
- `refactor(config)`: production Python からチャンネルルート `.env` の自動読込と `python-dotenv` 依存を削除し、Vertex AI client の location を用途別の内部固定値として必須化。`GOOGLE_CLOUD_PROJECT` / ADC の project 解決は維持（#2477）。
...
- `docs(live-chat)`: 配信中チャットの設定・人間認証・1Password 保存・VPS 配備・検証・障害復旧を一気通貫で案内する `/live-chat-reply` skill を追加し、`/comments-reply`・`/streaming` との相互排他、設定サンプル、主要モジュール文書を整備（#2376）。
...
- `feat(auth)`: multi-channel workspace の OAuth client secret 解決を channel 配下優先・workspace ルート `auth/client_secrets.json` fallback に拡張した。複数チャンネルが個別の client secret を持つ場合、`yt-doctor` はルート共有へ統合可能であり全チャンネルの再認証が必要なことを info レベルで案内する。強制・自動統合は行わず、単一チャンネル構成の挙動は維持する（#1950）。
...
- **skills**: `/analytics` を追加し、Analytics の収集・分析・最新レポート表示を成果物鮮度に基づいて一括実行・途中再開できるようにした (#1823)
...
- `feat(analytics)`: HTML レポートのテーマ色を `analytics/config.default.yaml::theme.colors` に移し、`config/skills/analytics.yaml` の channel override で差し替えられるようにした。未設定チャンネルでは既存パレットを維持する（#1691）
...
- 新規 skill 13 件: `/wf-auto`, `/analytics`, `/post-publish`, `/flop-analysis`, `/market-research`, `/creative-constraints`, `/value-loop-audit`, `/thumbnail-iterate`, `/thumbnail-test`, `/thumbnail-research`, `/feedback`, `/live-chat-reply`, `/automation-schedule`
...
- `docs(skills)`: 分析系 8 スキル（analytics / analytics / analytics / benchmark / channel-status / video-analyze / postmortem / alignment-check）の SKILL.md を 7 観点チェックリストでレビューし、意味・機能を変えない記述改善を適用した（#1705）。全 8 スキルの冒頭に「## 完了条件」を明示（既存の出力・終了記述の再掲）、analytics / analytics の「最新ファイル」検出基準を更新時刻順（`ls -t`）に一意化、analytics の Quick Reference にモード別実行コマンド対応を明記、benchmark / alignment-check の Read / Agent ツール記述に Codex 読み替え注記を追加、alignment-check Phase 5 の「必要に応じて更新」を AskUserQuestion 承認条件に明確化した。sonnet-safe 群（#1512〜#1524）適用済み修正は保持
...
- `fix(skills)`: 分析系 8 スキル（analytics / analytics / analytics / benchmark / channel-status / video-analyze / postmortem / alignment-check）の整合性監査で検出した記述ドリフトを修正（#1687）。`analytics/SKILL.md` の出力例を実装の実ファイル名 `data/analytics_data_YYYYMMDD_HHMMSS.json` と `config/channel/meta.json` の `channel.name` 参照に更新、`channel-status/SKILL.md` から `yt-channel-status` が実装していない「いいね数・コメント数」「制作中コレクション（workflow-state.json）表示」の記述を削除して実出力（再生数・総視聴時間・平均視聴時間）と `/wf-status` への案内に置換、`postmortem/SKILL.md` の実装参照 3 件を現行実体に更新（`strategic_analytics.py::_collect_top_videos`（不存在）→ `get_combined_analytics`、`benchmark_collector.py:378-394`（現在は collect_playlists の行域）→ `benchmark_collector.py::collect_channel`、schema_version=3 の生成元 `collection_uploader.py` → `agents/_tracking_io.py`（分離済み））。analytics / analytics / benchmark / video-analyze / alignment-check はドリフト検出なし
...
- `fix(skills)`: lifecycle skill の起動文字列を `bunx tayk <cmd>` から `uv run yt-<cmd>` へ戻した（#1625）。tayk は次世代版 CLI で現時点では運用未使用（実装は別リポジトリへ分離予定）のため、#965 で置換した表記のままではスキルを読んだ AI 実行者が未実装 subcommand（suno: `video-analyze` / `suno-verify`、masterup: `suno-audio-cleanup` / `suno-verify-playlist` / `fix-timestamps` / `finalize-master` / `apply-rain-layers` / `suno-select-tracks` ほか）で実行不能になっていた。対象は masterup / suno / suno-lyric / suno-helper / thumbnail / video-upload / video-description / wf-new / wf-next / playlist / analytics / distrokid-helper の SKILL.md・references・config.default.yaml（wf-new が references として同梱する `yt-init-collection` の復旧ヒント文言も同様に戻した）。起動文字列のみの置換で、周辺の手順・節構成・TS 実装（packages/）は変更しない。回帰テストは方針を反転し、tayk cutover まで lifecycle skill に `tayk` 表記が入らないことを担保する `tests/test_lifecycle_skills_no_tayk.py` へ改称・書き換えた
...
- skill-config 機構の適用を6 スキル(discover-competitors / live-clean / postmortem / video-upload / analytics / metadata-audit)へ拡大(#1669)
- videoup の ffmpeg フィルタグラフバグ、analytics の重複取得・TZ 境界バグなど計 12 件の Fixed
...
- `feat(skills)`: dogfood ライフサイクルが踏む 12 skill（wf-new / wf-next / suno / suno-helper / masterup / videoup / video-upload / thumbnail / video-description / analytics / playlist / distrokid-prep）の `uv run yt-*` 呼び出しを `bunx tayk <cmd>`（ADR-0007 rebrand / ADR-0004 単一 dispatcher）へ置換した（#965）。TS 版を pin した下流でも skill 経由で Python が実行され dogfood が始まらない問題を解消する。対象 skill の `uv run` 残骸ゼロを `tests/test_lifecycle_skills_no_uv_run.py` で機械担保。lifecycle 外の skill の置換は #966 で対応予定。
...
- `fix(analytics)`: `analytics/SKILL.md` の「CTR 値の解釈」が「整数値 2606」から意味不明な式の否定を経て「そのまま使用」に至る自己矛盾した記述になっており、コード実態（`reporting_api.py` の `total_weighted / total_impressions` が返す百分率 float。例 `4.2` = 4.2%、`tests/test_ctr_resolver.py` のフィクスチャも 4.2）と食い違っていた。百分率 float である旨・表示フォーマット（小数 1〜2 桁 + `%`）・100 で割る/掛ける/整数再解釈の禁止・`None` 時の表示を一義的な規則として書き直した（#1514）
...
- `docs(skills)`: wf-next / wf-status / analytics / wf-new / channel-setup / video-upload / community-post / collection-ideate の記述を現行実装に同期し、optional config 一覧を README / AGENTS / CLAUDE に追記（#1173, #1174, #1175, #1176, #1177, #1178, #1179, #1180）
