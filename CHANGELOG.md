...
- `fix(hooks)`: git worktree / Codex 実行環境で lefthook hook が stale な Nix store 固定パスや PATH 不在により silent skip され得る導線を修正。devShell の shellHook は trusted な flake 側 `.lefthook/install.sh` を呼び出して `lefthook install --force` を毎回実行し、生成済み `pre-commit` / `pre-push` は install 時の lefthook 絶対パス + PATH fallback の wrapper へ置き換え、どちらも解決できない場合は exit 1 で fail-closed する。親 checkout / worktree それぞれの診断・再生成手順も docs に追加した（#1552）
