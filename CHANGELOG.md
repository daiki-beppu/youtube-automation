...
所要時間の目安: 5〜15 分（独自 Python import の修正がある場合は追加確認が必要）

local fix 衝突注意:
- automation / channel-research / music / setup / thumbnail / video / wf-new

サマリ:
- fal の 1080P 指定を利用可能にしました。既定値の 768P と既存設定は維持します。
- Windows の起動フックと秘密ファイルの保護、Suno の曲採用・停止・ダウンロード処理を修正しました。拡張機能は別途更新してください。
- 通常の CLI entry point の撤去・改名はありません。独自コードの直接 import は以下の移動先を確認してください。

Python module 移動: あり
互換 facade: なし

| 旧 module / 公開入口 | 新 module / 入口 |
| --- | --- |
| `youtube_automation.domains.metadata.service` | `youtube_automation.application.metadata.service` |
| `youtube_automation.domains.metadata.BAHMetadataGenerator` | `youtube_automation.application.metadata.service.BAHMetadataGenerator` |
| `youtube_automation.domains.uploads.collection` | `youtube_automation.application.uploads.collection` |
| `youtube_automation.domains.uploads.shorts` | `youtube_automation.application.uploads.shorts` |
| `youtube_automation.domains.uploads.youtube.YouTubeAutoUploader` | `youtube_automation.application.uploads.youtube.YouTubeAutoUploader` |
| `youtube_automation.infrastructure.analytics.dashboard_refresh` | `youtube_automation.application.analytics.dashboard_refresh` |
| `youtube_automation.domains.uploads._preflight` | `youtube_automation.application.uploads.preflight` |
| `youtube_automation.domains.uploads._complete_collection_executor` | `youtube_automation.application.uploads._complete_collection_executor` |

移動監査: v6.0.0 との差分を rename 検出および削除・追加の組み合わせで照合しました。`youtube_automation.domains.uploads.description_document` の読み取り処理は `youtube_automation.domains.uploads._complete_collection_strategy.CompleteCollectionStrategy.load_description_document` に統合され、独立 module は削除されています。`youtube_automation.core.adapters.google` は空の package marker の撤去です。その他の新規 module は既存 module を残した責務分割で、module 全体の移動ではありません。
...
[6.0.1]: https://github.com/daiki-beppu/youtube-automation/releases/tag/v6.0.1
