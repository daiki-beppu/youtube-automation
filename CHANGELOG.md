...
- `fix(suno-helper)`: Suno の表示言語を日本語にしていると Exclude Styles 欄の placeholder が「スタイルを除外」になり、英語決め打ちの placeholder セレクタ（`input[placeholder*="Exclude" i]` 等）に一致せず「その他のオプションを開いてください」という誤案内の `FatalRunError` で run 全体が停止する問題を修正（#1840）。`shared/dom.ts` の `excludeStyles` セレクタに日本語 placeholder 対応（`input[placeholder*="除外"], textarea[placeholder*="除外"]`）を追加し、曲名欄（`input[placeholder*="Song Title" i], input[placeholder*="曲名"]`）と同じ英日並記に揃えた。既存の英語 placeholder / aria-label セレクタは従来どおり維持（後方互換）
