...
- `feat(ts-rewrite/core)`: `yt-collection-serve` を TS core service `collectionServeService` と `tayk collection-serve` thin adapter として追加した（#776）。Suno helper / DistroKid helper の契約値は `extensions/shared/constants.ts` を直接 import し、`collection.serve` registry entry 経由で起動する。
...
- `feat(suno-helper, collection-serve)`: channel-agnostic な Suno playlist capture endpoint を実装した（#893）。`tayk collection-serve` に `--playlist-capture-root <PATH>` / `--playlist-capture-prefix <SLUG>` を追加し、両方指定時のみ POST `/suno/playlists` を有効化する（片方だけ指定は schema validation で fail-loud、silent 無効化しない）。サーバー側は `packages/core/src/collection-serve/` の TS service として `normalizePlaylistTitle` / `writeCapturedPlaylists` などをスペック化し、POST は 200（許可 Origin）/ 403（Origin 未設定・不許可）/ 404（capture 無効・別パス）/ 400（非 list / 不正 JSON）を返す。prefix フィルタは拡張側でやらずサーバー側に閉じて channel-agnostic を担保する。suno-helper 拡張には overlay 下部に Capture / Send to localhost の 2 ボタンを持つ `PlaylistCaptureTab` を追加し、`shared/playlist-scrape.ts::scrapePlaylistsFromMe`（Suno `/me` の `a[href^="/playlist/"]` を aria-label 優先 + textContent fallback で抽出、空 title skip / url dedup）と `shared/api.ts::postCapturedPlaylists` を新設。連続実行の playlist 化完了時には background が bg `/me` tab を開いて scrape→POST→close する自動 capture を fail-soft で走らせる（`tabs` 権限を追加、過剰な広域権限は混入させない）。`buildCollectionsIndex` は `mapped: bool` を返すよう拡張し、拡張側は `excludeMappedCollections` で未マッピング collection のみをドロップダウンに出す。SSOT は `extensions/shared/constants.ts::PLAYLISTS_CAPTURE_ROUTE` で、TS server が直接 import する。`packages/core/test/collection-serve.test.ts` / `extensions/suno-helper/tests/playlist-capture.test.ts` / `playlist-scrape.test.ts` / e2e `playlist-capture.spec.ts` / `manifest.test.ts` で担保。仕様は `docs/tasks/suno-playlist-capture.md`
