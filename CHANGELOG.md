...
- `refactor(legacy-utils)`: canonical owner と同一だった `legacy_utils/{worktree,profile}.py` を削除し、確認済み下流 facade 5 本の import・symbol identity・挙動と配布互換は維持したまま、facade への関数・class・実行文・任意代入の再導入を recursive 契約テストで拒否する（#3896）。
